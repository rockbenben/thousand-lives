...
**关键设计（已确认）**：
- **apex 走 climb（不走渡劫赌命）**：成「一代名角/宗匠」是三十年技艺与声名的积累终点，非二元生死赌（同 officialdom 拜相）。3 个 passive 巅峰结局改 `maxTurns &` 门控，稀有性来自攀爬难度。**survive 登顶不强求 0**。
- **长度保持软上限 `maxTurns:30`**（「三十年粉墨春秋」，不改）。
- **tierLabel: '名位'**；名位印记四个、按序：`搭班`→`挑梁`→`名伶`→`宗匠`，不得越级。
...
新建 4 个专属升艺事件（同 officialdom 做法：既有 唱对台戏/压箱绝活/堂会受辱 是两难/风险事件，留作攒技艺/声名来源；专属升艺事件作闸门，技艺纯熟+机缘到时晋一阶）。各 `keyMoment:true, once:true, weight:3`，「登台/拜师/成名」选项授 `flagsSet:['下一名位']` + 技艺冲新上限，按 搭班→挑梁→名伶→宗匠 串链。窗口含足够 key 回合（keyMomentTurns(30)）。
...
| `宗匠` | 开宗立派 | `has(名伶) & art>=88 & fame>=70` | 晚期 24 |
...
apex 稀有由攀爬难度保证：爬到 `宗匠` 印记 + 三十年守住声名安稳，天然极少；乱点/早死/纯避险者绝无可能登顶。**唱对台戏保留为高风险事件**（打对台败→声名/安稳重挫或 endTone 横死），但只是凶险事件之一，**非 apex 闸门**。
...
`liyuan` 加 ceilingUnlocks + opening flag 后 `scenarioUsesFlags` 自动 = true，AI 提示**自动**注入【当前印记】【晋阶之序】+ flagsSet/endTone 契约 + 词表（名位印记 = ceilingUnlocks flags 搭班/挑梁/名伶/宗匠；隐藏 tone = endings 中 `condition==='safety<=-1'` 的 tone）。**prompt.ts 已通用化，无需再改**。

需：① 加 `tierLabel: '名位'`（晋阶之序读「本剧名位依次为：搭班→挑梁→名伶→宗匠」）；② `liyuan systemPrompt` 补名位晋阶（技艺到顶须真升艺→flagsSet 名位印记，按 搭班→挑梁→名伶→宗匠 顺序、不越级）+ 梨园横祸暴毙极稀。
...
- **apex 无赌命如何保稀**：靠攀爬难度 + 升艺担代价 + maxTurns 门控。若 sim 显示 survive 白嫖（登顶过高/非攀爬所得），收紧末阶升艺 requires（如宗匠需 fame 更高）。
