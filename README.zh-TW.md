...
> 社交分享入口頁（`public/s/`）已預先生成並隨倉庫提交，`npm run build` 會一併打包；改動內建劇本標題/開局後，重跑 `npx vite-node scripts/gen-og-pages.mjs` 重新整理 OG 預覽頁。
...
- **極端命運**：極小機率觸發「大開大合」的命運瞬間——天降橫財或飛來橫禍，一段生動文案 + 大幅後果，定格為本局的命運高光，可帶入命運卡分享
...
- **命運卡 + 挑戰連結**：對局中任意時刻都能生成「命運卡」——當下命運高光配圖 + 一句鉤子文案 + 二維碼，並附挑戰連結（`?s=&o=` 讓好友直達同題材、同開局），把你的此刻一鍵帶出
- **分享面板 + 社交直達**：結局卡與命運卡均走所見即所得的預覽彈窗，可複製圖片、儲存圖片，或一鍵分享到微博 / QQ 空間 / X / Telegram
...
- **可讀性 & 無障礙**：閱讀字級可自由調節並持久化；彈窗支援鍵盤操作（Esc 關閉、焦點鎖定）與 ARIA 標註，照顧不同裝置與讀屏使用者
- **社交 OG 預覽**：分享連結指向預先生成的靜態入口頁（`/s/<題材>-<開局>/`），社交平台抓取時顯示對應題材封面與鉤子文案，真人造訪再經 JS 重新導向回 SPA 開局——純靜態、無需邊緣函式，GitHub Pages / EdgeOne 等任意靜態託管即可
...
├── ui/          # React 介面：首頁 / 遊戲頁 / 設定 / 結局卡 / 命運卡分享彈窗 / 社交分享 / 挑戰連結 / 命途留影 / 成就 / AI 生成彈窗
├── assets/      # 內建劇本封面、結局圖、節點插畫、成就徽章（webp）
├── storage.ts   # localStorage 存檔與設定讀寫
└── App.tsx      # 路由與全域狀態
```

> 另有 `scripts/gen-og-pages.mjs` 預先生成社交分享入口頁 → `public/s/`，題材 OG 封面置於 `public/og/`。
