...
**10 個劇本 · 28 家 AI 服務商預設 · 31 枚成就 · 純前端 · 無需 Key 即玩**
...
內建 **10 個劇本**，涵蓋末世、官場、諜戰、仙俠、武俠、科幻等題材，每個均自帶本地事件池，**無需 API Key 即可試玩**：

| 劇本 | 題材 | 簡介 |
|------|------|------|
| ☢️ 末世求生 | 末世 | 病毒爆發後的現代都市，撐過三十天等待軍方救援 |
...
| 欄位 | 類型 | 說明 |
|------|------|------|
| `id` | string | 唯一識別符，不能與內建劇本的 id 相同（`wasteland` / `book` / `officialdom` / `spy` / `xian` / `wuxia` / `scifi` / `voyage` / `liyuan` / `sanguo`） |
| `title` | string | 劇本名稱 |
| `emoji` | string | 劇本圖示 |
| `intro` | string | 開場介紹文字 |
...
- **成就系統**：31 枚成就，涵蓋局數里程碑、結局收集、S 評級、各劇本通關與傳說結局、無傷通關、劇本創作等維度
