...
| `title` | string | 劇本名稱 |
| `genre` | string? | 可選，題材標籤（如「仙俠」「穿越」），作選關卡與設定頁題名之上的鎏金眉標 |
| `intro` | string | 開場介紹文字 |
...
- **事件**：`keyMoment`（命運抉擇關鍵節點，卷文題簽上就地鎏金標記 +「落子問命」金框選項 + gemini 專屬圖）、`requires`/`requiresItem`（門控）、`once`/`weight`/`minTurn`/`maxTurn`/`wildcard`（奇遇亂入）；選項可帶 `outcomes:[{weight,effects,flagsSet,itemsGained,endTone}]`（加權分支 + 隱藏彩蛋結局）、`reaction`（他人即時反饋）。
...
**敘事 & 決策**
- **沉浸呈現**：命途以整屏場景插畫為幕、卷文浮層承載劇情的視覺小說式佈局，一鍵「看全圖」可隱去文字純賞畫面；正文寫完選項才淡入，一寫一答自成呼吸
...
- **命途留影**：把這一生走過的**每一回合**匯成可回溯的圖冊——每格含場景插畫、正文、你的抉擇與他人回響，命運抉擇關鍵節點鎏金高亮，一局即是一卷可通覽全程的命運長卷
