import type { Scenario } from './schema'

export const wasteland: Scenario = {
  id: 'wasteland',
  title: '末世求生',
  genre: '末世',
  intro:
    '不明病毒爆发的那年，城市在尖叫中陷落。许诺的军方救援始终没有到来，官方安全区一个接一个失守、沦为新的坟场。废墟成了世界的全部，感染者昼伏夜出，幸存者之间既有相濡以沫，也有彼此吞噬。三年，你要在这片崩坏的土地上活下去——并从瓦砾里，亲手重建一处属于活人的据点与秩序。',
  attributes: [
    {
      key: 'hp',
      name: '生命',
      initial: 80,
      max: 100,
      deathBelow: 0,
      // 末世环境无时不在消耗你——饥渴、严寒、辐射与旧伤，
      // 生命每日自然流失（-1/天）；须靠进食、医疗、庇护所恢复，坐以待毙则油尽身死。
      decayPerTurn: 1,
      bands: [
        { upTo: 20, label: '濒死', severity: 'critical', directive: '身负重伤、行动迟缓，叙事中体现剧痛与失血；耗体力的行动只能勉力完成、收益缩水且代价加重，但仍可尝试。' },
        { upTo: 50, label: '负伤', severity: 'low', directive: '带伤行动，剧烈动作会加重伤势。' },
        { upTo: 85, label: '康健', severity: 'normal' },
        { upTo: 100, label: '强健', severity: 'high' },
      ],
    },
    {
      key: 'sanity',
      name: '理智',
      initial: 70,
      max: 100,
      deathBelow: 0,
      bands: [
        { upTo: 20, label: '濒临崩溃', severity: 'critical', directive: '叙事中必须出现幻觉、幻听或误判现实；至少一个选项是基于错觉的危险冲动。' },
        { upTo: 45, label: '动摇', severity: 'low', directive: '判断力下降，叙事流露焦虑、多疑与犹豫。' },
        { upTo: 80, label: '清醒', severity: 'normal' },
        { upTo: 100, label: '坚定', severity: 'high' },
      ],
    },
    {
      key: 'supplies',
      name: '物资',
      initial: 50,
      max: 100,
      // 据点印记逐级解锁物资天花板：孤身流浪屯不住（卡 50），唯建据点方能层层囤积、缓冲生命与理智
      ceiling: 50,
      ceilingUnlocks: [
        { flag: '落脚点', max: 65 },
        { flag: '据点', max: 80 },
        { flag: '堡垒', max: 92 },
        { flag: '营地', max: 100 },
      ],
      bands: [
        { upTo: 15, label: '枯竭', severity: 'critical', directive: '断粮缺水，叙事体现饥饿与绝望；本回合所有选项的 effects 额外扣减 hp（约 -10）与 sanity（约 -5）。' },
        { upTo: 40, label: '紧张', severity: 'low', directive: '物资见底，必须精打细算，叙事流露匮乏的压力。' },
        { upTo: 80, label: '尚可', severity: 'normal' },
        { upTo: 100, label: '充足', severity: 'high' },
      ],
    },
  ],
  openings: [
    { name: '便利店店员', prompt: '值夜班时被困的店员，熟悉店内物资与周边街区。', flag: '店员' },
    { name: '退役军医', prompt: '懂急救与武器，但左腿有旧伤，体力消耗更快。', flag: '军医' },
    { name: '高中生', prompt: '体力好、跑得快，但缺乏生存经验，容易冲动。', flag: '高中生' },
  ],
  ambitions: ['在末世里长久地活下去', '搜罗物资建立一处安全据点', '找到并守护其他幸存者', '查明病毒爆发的真相', '攒足补给，远走他乡寻一片净土', '研制出遏制病毒的解药', '在废土上建立自己的秩序与势力', '为惨死的亲人讨一个交代', '末世之中守住做人的底线', '组建一支生死与共的幸存者队伍'],
  turnUnit: '月',
  tierLabel: '据点',
  maxTurns: 44,
  finale: '你熬过了末世最长的寒冬，废土之上，终见微光。',
  systemPrompt: `你是一个末世生存文字游戏的主持人（GM）。世界观：不明病毒爆发后的现代废土，军方救援早已成泡影、官方安全区接连失守，感染者昼伏夜出，幸存者之间既有互助也有掠夺。叙事风格：紧张、写实、有压迫感，偶尔留一丝人性微光。
规则：
- 每回合代表一个月，剧情按月推进、可有跨度更大的经营与变故，不要原地打转
- 感染者、其他幸存者、物资短缺、天气与伤病都是威胁来源
- 物资（supplies）持续消耗：每个选项的 effects 都必须包含 supplies 的负数变化（通常 -3 至 -10）；觅食与经营类选项可在扣除消耗后净增加 supplies
- supplies 为 0 时玩家陷入饥饿，本回合所有选项的 effects 必须额外扣减 hp（约 -10）与 sanity（约 -5），剧情中体现饥饿与绝望
- 末世环境持续侵蚀生命（饥渴、严寒、辐射、旧伤），生命会自然流失，须靠进食、医疗、据点等主动恢复，坐以待毙则油尽身死；紧要关头可给出「耗命搏一线」的险选
- 玩家理智（sanity）过低时，剧情中应出现幻觉与误判
- 救援已成泡影，长期求生与经营据点才是出路；随回合推进可铺陈据点的兴建、幸存者的聚散与废土秩序的重建
- 据点分「落脚点→据点→堡垒→营地」数级，唯有觅地清场、加固扩建、筑墙设防、聚拢幸存者的重大机缘（关键抉择中肯投入经营的那一手）方能晋阶，据点越高越能囤积物资、缓冲生命与理智
- 泯灭人性的僭越（同流合污食人、弃同伴独活）或舍身护住希望的极难抉择，皆可能通向隐藏的结局`,
  endings: [
    // —— 中途死亡（死亡级）——
    // 「据点殉地」靠 salience 特判（建过据点者守土而亡）；其余裸 hp<=0 是一组「同等具体」的死法，
    // 引擎在它们里随机取一——使数值相同也可能饿死/渴死/病死/中毒死，死法各异、不再千篇一律「力竭身亡」。
    { condition: 'hp<=0 & has(据点)', tone: '力竭殉地·寸土未弃', art: '1o7keip', gen: 'gemini', epilogue: '你守着一砖一瓦垒起来的据点，到最后一刻也没退。围墙是你加固的，陷阱是你布的，这片瓦砾里好不容易才有了点活人的模样。你倒在自己守了许久的门口，手还攥着那杆早打空了的家伙。没能看到更好的明天，可这方你拼命护住的寸土，会替你记着——有人曾在这死城里，认真地活过，也认真地守过。' },
    { condition: 'hp<=0', tone: '力竭身亡', art: '1bntq8p', gen: 'gemini', epilogue: '你倒在冰冷的瓦砾上，伤口不再灼痛，世界也终于安静了下来。最后映入眼底的，是灰白天光从断墙的裂缝斜斜照进来，像旧世界临别前递来的一点余温。你想抬手，却再没有力气。风卷着尘土漫过你的脸，街上那些拖沓的脚步声渐渐远去——这一次，你不必再逃了。' },
    { condition: 'hp<=0', tone: '饿毙街头', art: '1mdl4iw', gen: 'gemini', epilogue: '到最后，杀死你的不是感染者的獠牙，是空荡荡的肠胃。能翻的瓦砾都翻遍了，能咽的东西早咽尽了。你撑着墙根一点点滑下去，眼前阵阵发黑，连抬一根手指的力气都没了。饥饿是这死城里最有耐心的猎手，它不急，它只是等——等到你再也站不起来的那一天。' },
    { condition: 'hp<=0', tone: '渴死荒途', art: '1fsdlay', gen: 'gemini', epilogue: '水。最后那点念头里翻来覆去只剩这一个字。补给断了，能喝的早喝光了，连泥坑里的脏水都成了奢望。喉咙干得像要裂开，嘴唇结了硬痂，废墟在烈日下扭曲成一片白晃晃的幻影。你朝着想象中的水声爬过去，爬着爬着，就再没动过——原来在这世道，渴，比什么都来得快。' },
    { condition: 'hp<=0', tone: '病亡无医', art: '1gsvig6', gen: 'gemini', epilogue: '一开始只是低烧，你没当回事。可这年月没有医院、没有药，一点小病也能拖成索命的绳。高烧一天天熬干了你，伤口发黑流脓，整个人烧得糊里糊涂。你蜷在角落里发抖，听着自己的喘息越来越弱。没有人能救你，连你自己都救不了——熬过了刀光和獠牙，最后却败给了一场本不致命的病。' },
    { condition: 'hp<=0', tone: '中毒暴毙', art: '1awux9t', gen: 'gemini', epilogue: '那点东西看着没什么异样，饿极了的你也顾不上多想，就那么吃了下去。没过多久，腹中绞痛如刀割，冷汗瞬间湿透后背。你这才明白吃错了东西，可一切都晚了。毒在血里烧，视线一阵阵发花，你扶着墙剧烈干呕，终究还是软软倒了下去——废土什么都缺，唯独不缺让人毫无防备就送命的陷阱。' },
    { condition: 'hp<=0', tone: '失血而亡', art: '1cq1chv', gen: 'gemini', epilogue: '伤口比你以为的更深。起初还能用布条胡乱扎住，可血就是止不住，顺着指缝一下下往外涌，在身下积成一小滩温热。力气随着血一点点流走，手脚渐渐变得冰凉，眼前的废墟也褪成了灰白。你想喊人，喉咙却只挤得出气音。没有缝合的针线，没有止血的药——在这没有医者的世道，一道伤口，就足够要了一条命。' },
    { condition: 'sanity<=0', tone: '疯癫失智·消失在废墟', art: '1vmjdmo', gen: 'gemini', epilogue: '理智的最后一道堤坝悄无声息地塌了。你站起身，朝那个一声声唤你名字的方向笑着走去，废墟在你眼里开成了花，嘶吼成了久违的欢迎。没有人知道你最终走向了哪里，只有空荡的街道记得你渐行渐远的背影。你的人还在喘息，那个「你」却已散在风里，再也回不来——你成了死城里又一个再也回不来的影子。' },
    {
      condition: 'sanity<=-1',
      tone: '同流合污·食人自保', art: '1wg0edu', gen: 'gemini',
      epilogue:
        '你咽下了那几口明知是什么的「肉」，也咽下了自己作为人的最后一点东西。从那以后，饥饿不再让你恐惧，因为你已经知道了一条谁都不愿说破的「活路」。你在废土上越走越远，吃得越来越「饱」，可镜子里那张脸，你已渐渐认不出——你没死在感染者口中，却亲手把自己变成了比它们更让人作呕的东西。',
    },
    {
      condition: 'sanity<=-1',
      tone: '弃众独生·孤鬼游荡', art: '1r3nr5m', gen: 'gemini',
      epilogue:
        '你把信任你的同伴推向了死亡，踩着他们的惨叫独自活了下来。命是保住了，可那一双双临死前不可置信的眼睛，从此夜夜在你梦里睁着。你再不敢与任何人结伴，因为你比谁都清楚自己会怎样背叛——你像一缕孤魂，独自游荡在空荡的废土上，活着，却比那些死去的人更像个鬼。',
    },
    {
      condition: 'sanity<=-1',
      tone: '以命护苗·废土微光', art: '1656wjy', gen: 'gemini',
      epilogue:
        '你押上了自己的一切，护着那个对病毒免疫的孩子，一路躲过各方势力的围追堵截，把他和他血管里那点足以拯救世界的希望，亲手送到了还在运转的研究所。他们说，疫苗有望了。你或许看不到那一天，可你知道——在这片人性几近熄灭的废土上，你用一条命，替所有人守住了一簇不肯灭的微光。',
    },
    // —— 三年末世后的归宿（maxTurns 变体，子句越多越靠前）——
    { condition: 'maxTurns & hp>=54 & sanity>=78 & supplies>=70', tone: '末世王者·重塑秩序', art: 'a55elf', gen: 'gemini', epilogue: '三年了。当最后一批掠夺者也退出你的势力范围，你站在据点最高的墙头俯瞰脚下——堆满给养的库房、安睡的人、井然的秩序，都是你从瓦砾里一寸寸立起来的。废土在你脚下俯首，你不只是活了下来，你重新立起了秩序。风掀动你褴褛却干净的衣角，墙下有人低声唤你的名字，那声音里带着敬意——新的世界，将从你站立的地方开始。' },
    { condition: 'maxTurns & hp>=49 & sanity>=70 & supplies>=63', tone: '王者归来·近乎无伤', art: '1d3n4me', gen: 'gemini', epilogue: '三年过去，你几乎是毫发无损地走完了这场浩劫，眼神清明，脚步沉稳，仿佛末世从未在你身上留下半道伤痕。投奔来的人打量着你，不敢相信有人能把这地狱走得如此从容。阳光重新落在你脸上，你回望身后那片狼藉的城，没有恐惧，只有一种历劫归来的笃定——你赢了，赢得干净而彻底。' },
    { condition: 'maxTurns & hp>=44 & sanity>=62 & supplies>=56', tone: '从容立足·重建有望', art: '1tysmz0', gen: 'gemini', epilogue: '三年的厮杀与饥寒，没在你身上留下太重的痕迹。你带着尚算充裕的给养、清醒的头脑，立在你一手撑起的落脚处，看着废墟之上第一缕炊烟袅袅升起。活下来只是开始，而重建一切的种子，早已握在你手里——这片死地，终究被你熬出了一点活气。' },
    { condition: 'maxTurns & hp>=44 & supplies>=62', tone: '满载而归的幸存者', art: 'xk79ve', gen: 'gemini', epilogue: '你背着鼓鼓囊囊的行囊走过这片废土，身板硬朗，步履有力，仿佛末世只是磨砺了你的筋骨。罐头、清水、药品——这些拿命换来的家当，此刻成了你在废土上立身的底气。有人投来艳羡的目光，你只是淡淡一笑。活到第三年，你不仅熬了过来，还熬得有滋有味。' },
    { condition: 'maxTurns & hp>=50 & sanity>=66', tone: '身心俱全·迎来曙光', art: '8fmdx4', gen: 'gemini', epilogue: '三年的厮杀与饥寒，没能压垮你的身子，也没能蚀空你的心。你站在废墟的边缘，看着东方那道终于撕开夜色的曙光，身上虽有伤，眼里却仍有光。你迎着晨曦走过去，胸腔里那颗心还在稳稳地跳着——它告诉你，你完整地、像个人一样，活到了三年之后。' },
    { condition: 'maxTurns & supplies>=66 & hp>=42 & sanity>=50', tone: '重建据点·重燃文明', art: '1i7nsjz', gen: 'gemini', epilogue: '你守住的不只是一处藏身之所，而是一簇不肯熄灭的火。库房里堆满给养，墙头有了哨位，破败的角落被你一点点拾掇成了家的模样。三年熬下来，你回头看了看这片亲手撑起的天地。文明的灯火曾在这座城里熄灭，而你，让它在废土上重新亮了起来。' },
    { condition: 'maxTurns & supplies>=66 & sanity>=54', tone: '据点之主·守得云开', art: 'dh7od5', gen: 'gemini', epilogue: '你曾在无数个不眠的夜里死守这方寸之地，把每一罐补给、每一道门闩都看得比命还重。三年熬下来，云开了——投奔的人在你墙下扎了营，孩子的笑声第一次盖过了远处的嘶吼。你拍了拍身旁囤满物资的墙垛，像在与一位并肩到底的老友相视而笑。这据点是你的疆土，而你，是它当之无愧的主人。' },
    { condition: 'maxTurns & hp>=48 & supplies>=56', tone: '安稳扎根的幸存者', art: '1o2j7up', gen: 'gemini', epilogue: '没有惊天动地的搏杀，也没有壮烈的牺牲，你只是稳稳当当地、一步一个脚印地，把自己安全地扎下了根。背包里还有余粮，身上的伤不深不浅，三年走到今天，你长长地呼出一口气——平安活着，本身就是这乱世里最了不起的胜利。' },
    { condition: 'maxTurns & has(阿黄) & sanity>=45', tone: '不离不弃·荒野相伴', art: '1696zy8', gen: 'gemini', epilogue: '那条你从废墟里收留的忠心狗阿黄，陪你熬过了整个末世。在这吃人的世道里，是它那点不离不弃的暖，让你没有活成一具行尸。重建的日子终于到来，你蹲下身摸了摸它花白的头——熬过末世的从来不只是物资与武器，还有这份连绝境都没能割断的相依为命。' },
    { condition: 'maxTurns & has(店员)', tone: '凡躯不屈·熬过末世', art: 'v0rnqt', gen: 'gemini', epilogue: '开局「便利店店员」——末世前不过是个最普通的打工人，没受过训、没有特长。可你就凭着一股不肯倒下的韧劲，硬是用这具凡人之躯熬过了最惨烈的末世。没有英雄的光环，你活下来靠的是小心、是坚持、是一次次咬牙撑住——平凡人的不屈，本身就是末世里最了不起的奇迹。' },
    { condition: 'maxTurns & has(军医) & sanity>=55', tone: '悬壶济世·废土良医', art: '1us311m', gen: 'gemini', epilogue: '开局「退役军医」——一身救人的本事，在这草菅人命的末世里成了最珍贵的东西。你本可以独善其身，却选择了一次次为伤者缝合、为绝望者点灯。废土良医的名声传开，多少人因你而多活了一口气。在人性崩坏的末世，你用那把手术刀，守住了「人」该有的样子。' },
    { condition: 'maxTurns & has(高中生) & supplies>=55', tone: '少年长成·废土新生', art: 'x7gyot', gen: 'gemini', epilogue: '开局「高中生」——末世降临时，你还是个懵懂少年。这几年的血与火，把你从一个躲在课桌后的孩子，逼成了能扛起一片据点的大人。你攒下了立足的根基，也把自己活成了废土上新一代的脊梁。旧世界塌了，可你这样的少年长成了，新世界便有了希望。' },
    { condition: 'maxTurns & hp<=20 & sanity<=20', tone: '苟延残喘·险死还生', art: 'gwge33', gen: 'gemini', epilogue: '你是从废墟里半拖着自己爬出来的。身上没有一处是完好的，神志也在崩溃的边缘游荡，连眼前的景物都看不真切。每一口呼吸都像在刀尖上挣扎，你早已记不清自己究竟是怎么熬到三年的。当你终于瘫倒在一片相对安全的废土上，你终于松开了死攥着的手——这条命，是从死神指缝里硬生生抠回来的，残破，却滚烫。' },
    { condition: 'maxTurns & sanity<=20 & supplies>=60', tone: '行尸走肉·虽生犹死', art: '1ah7ert', gen: 'gemini', epilogue: '你囤下了足够吃上许久的物资，却唯独没能保住自己的灵魂。三年了，有人走近你时，你眼神空洞地望着远方，嘴里反复念叨着没人听得懂的话。身体活着，背包满着，可那个会哭会笑、会害怕也会盼望的你，早已在某个无名的夜里悄悄死去了。你活成了一具被掏空的躯壳，在风里，木然地看着废土上来来往往的人。' },
    { condition: 'maxTurns & sanity<=20 & supplies<=20', tone: '形神俱疲·惨胜余生', art: '5s4sez', gen: 'gemini', epilogue: '你两手空空，神志恍惚，是拖着一具几乎散架的身心爬到三年尽头的。饥饿掏空了你的肚子，恐惧蛀空了你的脑子，能撑到这一刻，靠的早已不是力气，而是一股连你自己都说不清的执念。有人递来一口水，你抖着手都快端不稳。这是一场惨烈的胜利——你赢了，却几乎什么都没剩下。' },
    { condition: 'maxTurns & sanity<=20', tone: '苟活·却已精神崩坏', art: '1dm8f5c', gen: 'gemini', epilogue: '三年过去，你却分不清这是现实还是又一场幻觉。你冲着空气傻笑，又对着虚无尖叫，把废土上走近的人当成了死去的亲人，又把伸来的手看作索命的鬼影。身体还在喘息，可你的神志，永远地留在了那座吞噬一切的死城里。你苟活了下来——以一种比死去更让人不忍直视的方式。' },
    { condition: 'maxTurns & hp<=20', tone: '油尽灯枯·勉力撑住', art: '119hrw4', gen: 'gemini', epilogue: '你像一盏快要熬干的灯，在最后一丝光亮熄灭前，硬生生撑过了第三十六个月。浑身的伤口早已麻木，血几乎流尽，每一次心跳都像在向身体讨要它再也付不出的力气。瘫倒在废土的某个角落时，你连睁眼的力气都没有了。你赢得了这场马拉松，却是用尽了生命里最后一滴灯油换来的。' },
    { condition: 'maxTurns & supplies<=15', tone: '饿殍边缘·勉强熬过', art: '1owk9m7', gen: 'gemini', epilogue: '你是踩着饿死的边线挪过三年的。胃早已绞痛得没了知觉，四肢轻飘飘的像不属于自己，眼前的废土一阵阵发黑发花。有人塞进你手里一块干粮，你抖着手撕开包装，狼吞虎咽，眼泪却不争气地滚了下来。这一程，你没死在感染者口中，差一点，却死在了空空如也的肚子里。' },
    { condition: 'maxTurns & sanity>=80', tone: '心如磐石的幸存者', art: '10h03gm', gen: 'gemini', epilogue: '三年过去，你站在断墙下静静环视这片你熬下来的废土，神色平静，目光清澈而坚定——身上带着伤，那颗心却从头到尾稳如磐石。' },
    { condition: 'maxTurns & hp>=56', tone: '钢筋铁骨的幸存者', art: '12eifo', gen: 'gemini', epilogue: '感染者的撕咬、掠夺者的拳脚、严寒与饥饿的轮番折磨，都没能在你这副铁打的身板上凿出致命的裂痕。活到第三年，你肌肉里仍蓄着用不完的力气，伤痕成了你披在身上的勋章。投奔来的人不敢相信你是一路血战着走过来的。废土很硬，可你比它更硬——这具身体，扛住了它能扔来的一切。' },
    { condition: 'maxTurns & supplies>=70 & hp<=55', tone: '坐拥余粮·带伤脱困', art: 'i1lrk3', gen: 'gemini', epilogue: '你拖着一身的伤，却背着一囊沉甸甸的给养，一瘸一拐地走过这片废土。每一步都牵动伤口隐隐作痛，可你舍不得丢下任何一罐拼命攒来的物资。有人要替你卸下重担，你却摇了摇头——这些家当，是你在血与火里换来的命根子。带伤又何妨，你终究是满载着积攒，活到了三年之后。' },
    { condition: 'maxTurns & hp>=52', tone: '体魄尚健·安然立足', art: 'iohpcf', gen: 'gemini', epilogue: '你的身子还算硬朗，在这片废土上立足的步子也还稳当。这一路虽磕磕绊绊，添了些伤、受了些惊，可筋骨未损，气力尚足。你抬手揉了揉发酸的肩膀，望着你扎下根的这处所在，心头一阵踏实。在这把无数人拖垮、放倒的废土上，你护住了自己的身体，也护住了重新开始的本钱。' },
    { condition: 'maxTurns & sanity>=60', tone: '神志清明·坚持到底', art: '1eqkulg', gen: 'gemini', epilogue: '三年过去，你仍能逐一认清废土上每张走近的脸，清楚地记得自己是谁。你站在废墟前，眼神清亮、脚步沉稳——身子或有损耗，那盏灯却一直亮着。' },
    { condition: 'maxTurns & hp>=40', tone: '伤痕累累·熬到今日', art: '16zwxq', gen: 'gemini', epilogue: '你浑身是伤地走过这三年，旧创未愈，新痕又添，每一道疤都记着一次死里逃生。脚步虽虚浮，你却没有倒下。活到今日，你扯出一个疲惫的笑——能站着走到这里，已是天大的幸运。废土在你身上刻满了印记，可它终究没能把你彻底留下。你带着满身伤痕，熬了下来。' },
    { condition: 'maxTurns', tone: '熬过末世', art: '1gh9049', gen: 'gemini', epilogue: '第三十六个月的月光照进断墙时，你忽然意识到自己真的撑了下来。感染者的嘶吼还在远处此起彼伏，可你已不再是那个在便利店废墟里惊惶醒来的人。你拖着疲惫的身躯走过这座吞噬了无数生命的城，它没放过别人，却独独漏下了你。前路依旧未知，但至少，你还活着，还能呼吸这劫后的空气——对一个幸存者而言，这就够了。' },
  ],
  localEvents: [
    {
      narrative:
        '你蹲在一截倒塌的水泥墙后躲避正午的毒日，无意间瞥见墙缝里塞着一只发黑的旅行箱。撬开锈死的锁扣，你倒吸一口凉气——里头不是吃的，是一沓沓用橡皮筋捆得整整齐齐的钞票，少说也有几十万。在这个面包比命金贵的世道，这堆花花绿绿的纸早成了一文不值的废物，可它静静躺在那儿，仍透着旧世界一去不返的荒诞与讽刺。',
      choices: [
        { text: '抽几张当引火纸，剩下原样塞回', effects: { sanity: 4, supplies: -4 }, reaction: '钞票卷成的引信「腾」地窜起火苗，你就着这把旧世界的余烬烤热了罐头——曾经能买下半条街的钱，如今只值这一口暖。' },
        { text: '怔怔数完，苦笑着空手离开', effects: { sanity: -3, supplies: -3 }, reaction: '你把橡皮筋一根根重新捆好，像在替一个死去的时代收殓。走出去老远，那荒诞的一幕还黏在眼底，分不清是想哭还是想笑。' },
        { text: '翻遍夹层，看有没有值钱的真家伙', effects: { supplies: 5, sanity: -2 }, reaction: '钞票底下竟压着半板止痛片和一把瑞士军刀——你嗤笑一声：这年头，废纸里裹着的实在玩意儿，才是真正的财。' },
      ],
      summary: '废箱里的钞票', art: '1lp9c3v', gen: 'gemini',
      weight: 0.4,
    },
    {
      narrative:
        '夜里你被一阵奇异的光晕惊醒，推开窗，整片天幕正翻涌着诡谲的绿紫色极光，像有谁打翻了天上的颜料，无声地灼烧着云层。这座城从没在这个纬度见过这种天象，街上零星几个幸存者也都仰着头，呆呆望着那片不祥又壮丽的天空。没人知道这是病毒爆发后大气出了什么乱子，还是别的什么征兆——只是那片诡光照下来，连感染者的嘶吼都莫名安静了下去。',
      choices: [
        { text: '爬上楼顶，静静看完这场天象', effects: { sanity: 10, supplies: -3 }, reaction: '冷风里你看了整整一夜，那片诡丽的光像在替这座死城办一场无声的葬礼——天亮时你心头竟空空地静了，连日的惊惶淡了几分。' },
        { text: '认定是凶兆，连夜收拾细软转移', effects: { hp: -4, supplies: -5, sanity: -2 }, reaction: '你背着包深一脚浅一脚地逃，回头那片绿光仍在天上烧。事后什么都没发生，可那一夜的仓皇，让你笑自己也信了回邪。' },
      ],
      summary: '诡异极光', art: '1k8pb1u', gen: 'gemini',
      weight: 0.4,
      minTurn: 10,
    },
    {
      narrative:
        '你在一栋写字楼的茶水间避雨，转角的自动售货机却突然「嗡」地活了过来，屏幕幽幽亮起——残存的电力让它兀自运转着。更离奇的是，它似乎卡了程序，出货口正一罐接一罐地往外吐着冰可乐，「哐当哐当」滚了一地，怎么也停不下来。在这个连口净水都要拿命换的世道，一台发了疯、免费派发饮料的机器，荒诞得让你站在原地愣了好久，几乎要笑出声来。',
      choices: [
        { text: '抱回所有罐子，能存多少存多少', effects: { supplies: 12, hp: -3 }, reaction: '你脱下外套兜着叮当作响的战利品离开，那台机器还在身后徒劳地吐着货——这是末世给你开的唯一一个甜得发慌的玩笑。' },
        { text: '只灌饱肚子，痛快喝个够', effects: { hp: 6, sanity: 6, supplies: -2 }, reaction: '你坐在一地罐子中间，仰头灌下久违的甜与凉，气泡在喉咙里炸开。那一刻饥渴和恐惧都退了潮，你像回到了从前某个寻常的下午。' },
      ],
      summary: '发疯的售货机', art: '8rs4t4', gen: 'gemini',
      weight: 0.4,
    },
    {
      narrative:
        '搜寻物资时，你在一间被翻烂的画室角落发现一只没人稀罕的旧木匣。掀开盖子，里头不是吃的喝的，而是一只小巧的八音盒，铜质的发条还能拧动。你鬼使神差地上紧发条，一段清越的旋律便流淌出来，在死寂的废墟里叮叮咚咚地响着，干净得不像属于这个世界。你怔怔听着，恍惚间饥饿、恐惧、那些灰扑扑的日子，都被这串音符轻轻托了起来。',
      choices: [
        { text: '揣进怀里，留作支撑自己的念想', effects: { sanity: 9, supplies: -3 }, reaction: '此后每个撑不下去的夜，你都拧两圈发条。那串细弱的旋律成了你揣在胸口的一簇火，提醒你还记得自己是个人。' },
        { text: '太占地方，听完便放回原处', effects: { sanity: 4, supplies: -2 }, reaction: '你让最后一个音符消散在空气里，才合上匣盖。东西没带走，那点久违的柔软却悄悄跟着你出了门。' },
      ],
      summary: '废墟里的八音盒', art: '1yrnyar', gen: 'gemini',
      weight: 0.4,
      itemsGained: ['八音盒'],
    },
    {
      narrative:
        '你正翻找一处院落，头顶忽然传来「嘎吱」一声闷响——一只硕大的花盆从三楼的窗台滚落，挟着风声直直砸下，擦着你的肩头轰然碎裂，碎瓷溅了你一脸，疼得你眼前发黑。你惊魂未定地抬头怒视，那扇空窗里却空无一人，只有一只野猫晃着尾巴蹲在窗沿，居高临下、漫不经心地睨着你。在这个连死法都透着随机的世道，你差点就栽在一只猫和一盆枯花手里。',
      choices: [
        { text: '骂骂咧咧揉着肩，自认倒霉', effects: { hp: -7, sanity: -2 }, reaction: '你冲着那只猫啐了一口，它毫不在意地舔起爪子。你揉着青肿的肩膀苦笑——熬过感染者，没想到差点死得这么窝囊。' },
        { text: '上楼查看，那猫窝里说不定有货', effects: { hp: -3, supplies: 7, sanity: -1 }, reaction: '你顺着楼梯摸上去，那野猫的窝边竟堆着它叼来的几包未拆的零食。你哭笑不得地收进包里——这一盆花，砸出条财路来了。' },
      ],
      summary: '飞来的花盆', art: '4pivdv', gen: 'gemini',
      weight: 0.4,
    },
    {
      narrative:
        '黄昏时你拐进一条窄巷，迎面撞见一个佝偻的老头支着块木板，板上歪歪扭扭写着「以物易物，童叟无欺」。他面前摆着些稀奇古怪的玩意儿：缺角的怀表、生锈的指南针、半瓶香水、一副老花镜。见你过来，他咧开没剩几颗牙的嘴笑了，操着一口慢悠悠的腔调招呼你，仿佛外头根本没有什么末世，他只是又支起了经营了半辈子的小摊。这份与世界格格不入的从容，看得你一阵恍惚。',
      choices: [
        { text: '拿余粮换一只还能用的指南针', effects: { supplies: -6, sanity: 5 }, reaction: '老头郑重地把指南针放进你掌心，叮嘱「往北别迷路」。揣着这玩意儿，你头一回觉得脚下乱糟糟的路，好像有了个方向。' },
        { text: '陪他唠几句，听听巷子外的传闻', effects: { sanity: 8, supplies: -4 }, reaction: '老头嘬着烟袋，断断续续讲起东边的庇护所、北边的吃人帮派。这些真假难辨的闲话，却让你像久旱逢甘霖般贪婪地听着。' },
        { text: '不与陌生人纠缠，绕开走', effects: { supplies: -2, sanity: -2 }, reaction: '你贴着墙根快步绕过那摊子，老头也不恼，只在身后悠悠道了句「后会有期」。走出巷口，你竟莫名有点舍不得那点人气。' },
      ],
      summary: '废土杂货摊', art: '1svb312', gen: 'gemini',
      weight: 0.4,
    },
    {
      narrative:
        '断壁残垣间，你被一片不合时宜的鲜活绿意绊住了脚——一小块被人精心侍弄过的菜畦，藏在两堵塌墙的夹缝里，几株番茄正红得发亮，藤蔓上还挂着水珠。畦边歪着一块木牌，上面用炭笔写着：「种菜的人没能等到收成，路过的朋友，摘了别糟蹋，记得给后来人留几颗。」字迹已被雨水洇得发淡，可那份隔着生死递来的善意，却让你站在原地，鼻子一阵阵发酸。',
      choices: [
        { text: '只摘熟透的几颗，照木牌留下种子', effects: { supplies: 6, sanity: 10 }, reaction: '你把摘剩的番茄籽细细埋回土里，又替菜畦松了松土。做完这些，胸口那点被末世磨得发硬的东西，软了下来。' },
        { text: '顾不上许多，连根带果全拔走', effects: { supplies: 14, sanity: -8 }, reaction: '你抱着满怀的收成匆匆离开，那块木牌被你踩进了泥里。填饱了肚子，可那行被你辜负的字，夜里总在心头隐隐发烫。' },
      ],
      summary: '夹缝里的菜畦', art: 'ws5s5p', gen: 'gemini',
      weight: 0.4,
    },
    {
      narrative:
        '连日来，你在好几处藏身点的墙上都见过同一个用红漆草草涂下的符号——一个圈住的螺旋。今夜围着篝火，几个萍水相逢的幸存者压低嗓门议论起它：有人说那是某个神秘组织的标记，专门收容走投无路的人；有人说凡是画了这记号的街区，去的人就再没回来过；还有人赌咒发誓，说自己亲眼见过顺着螺旋走能找到一处地下避难所。真假混作一团，可那符号偏偏就刻在你今夜歇脚的这面墙上。',
      choices: [
        { text: '宁可信其有，循着螺旋探一探', effects: { hp: -6, supplies: -5, sanity: 6 }, reaction: '你顺着一路的螺旋记号摸进地下，尽头竟真有个废弃的人防工事，墙上贴满陌生人留的字条。谜底没全解开，可这一夜你睡得格外踏实。' },
        { text: '邪门得很，明早绕开这片街区', effects: { supplies: -3, sanity: -3 }, reaction: '你天不亮就背包走人，把那个螺旋甩在身后。是福是祸再没人知道，可宁可错过、绝不踏进未知，正是你活到今天的本分。' },
      ],
      summary: '螺旋符号传闻', art: '6z8vgi', gen: 'gemini',
      weight: 0.4,
      minTurn: 14,
    },
    {
      narrative:
        '便利店的卷帘门被你用尽全身力气只拉下半截，铁皮在轨道里发出刺耳的呻吟，又卡死不动。借着从门缝漏进来的灰白天光，你看清货架早被洗劫一空，只剩散落的包装袋和打翻的货物在脚边滚动。收银台后那扇储物间的小门却仍上着锁，锁芯完好——也许里头还藏着没被发现的东西。门外那条街上，拖沓黏腻的脚步声始终没停，时远时近，提醒你留给你的时间不多了。',
      choices: [
        { text: '撬开储物间，赌里面有补给', effects: { supplies: 8, hp: -3, sanity: -2 }, reaction: '锁芯崩开的脆响里，门外一个躲着的瘦影探头瞄了一眼，咽了口唾沫——它早盯上这扇锁了，却没你这份豁出去的胆量。' },
        { text: '只拿眼前能拿的，尽快离开', effects: { supplies: 2, sanity: -3 }, reaction: '街角阴影里一双眼睛盯着你匆匆离去的背影，撇了撇嘴——这么大一间店，被你白白撂下了。' },
        { text: '原地屏息，等脚步声远去', effects: { supplies: -4, sanity: -5 }, reaction: '没人看见你这一夜，只有你自己听着那拖沓脚步声远去又折返，把这一夜熬成了煎熬。' },
      ],
      summary: '便利店搜寻', art: '1oewd7f', gen: 'gemini',
      minTurn: 1,
      once: true,
      itemsGained: ['撬棍'],
    },
    {
      narrative:
        '入夜，远处先是一声嘶哑的长嚎划破死寂，紧接着是接二连三的回应，那种不属于活人的吼叫像潮水一样朝这边漫过来，越来越近。你蜷在公寓楼道的拐角，连呼吸都不敢放大，黑暗厚得仿佛能攥在手里。就在你以为自己听错时，楼梯井深处传来极轻的、布料摩擦墙皮的窸窣声——有什么东西正在那片伸手不见五指的黑里缓慢挪动，方向，正对着你。',
      choices: [
        { text: '退回房间反锁，硬撑一夜', effects: { supplies: -6, sanity: -4 }, reaction: '门那头的窸窣声停了一瞬，仿佛也在掂量这道反锁的门——它没再靠近，黑暗里你赢下了这场无声的对峙。' },
        { text: '摸黑下楼，赌它们还没上来', effects: { hp: -12, supplies: -3 }, reaction: '楼梯井里那东西扑了个空，发出一声不甘的低吼——你贴着墙根冲下去的那股狠劲，连它都没追上。' },
        { text: '点亮手电查看——也许只是错觉', effects: { sanity: -8, supplies: -4 }, reaction: '光柱亮起的刹那，黑暗里无数张灰败的脸齐刷刷转向你——你把自己活活照成了它们眼里的猎物。' },
      ],
      summary: '夜半异响', art: '1cgmrju', gen: 'gemini',
      weight: 1.5,
    },
    {
      narrative:
        '你撬开一户敞着门的居民楼，屋里弥漫着积尘和无人居住的霉味，家具却还摆得整整齐齐，像主人只是临时出门。厨房的橱柜竟逃过了之前的搜刮，你拉开柜门，几罐贴着完整标签的罐头码在最里层，旁边是半袋还没受潮的大米，最让你心跳加快的是角落那瓶未开封的矿泉水——在这个连干净水都成了奢望的世道，这一柜子简直是救命的横财。',
      choices: [
        { text: '全部搬空，多多益善', effects: { supplies: 16, hp: -4 }, reaction: '你走后，下一个摸进来的幸存者望着空空如也的橱柜，恶狠狠啐了一口——这年头，先到的就是王法。' },
        { text: '只取够用的，留点给后来人', effects: { supplies: 8, sanity: 4 }, reaction: '后来人拉开柜门，看见你留下的那几罐，怔了半晌——这片废土上，竟还有人肯替素不相识的人留一口活路。' },
      ],
      summary: '居民楼觅食', art: '1k9cahn', gen: 'gemini',
      itemsGained: ['罐头'],
    },
    {
      narrative:
        '拐过街角，一个瘦削得几乎脱了形的身影猛地从废墟后探出来，吓得你后退半步。他立刻高高举起双手，掌心朝你，声音抖得不成调：「别开枪……求求你，我只是饿了三天了，三天没吃东西了。」他的嘴唇干裂起皮，眼窝深陷，目光却死死黏在你鼓鼓囊囊的背包上。末世里，怜悯有时是善意，有时是被人拿捏的破绽——你一时分不清眼前这人究竟是哪一种。',
      choices: [
        { text: '分他一些物资，结伴同行', effects: { supplies: -10, sanity: 8 }, reaction: '他接过干粮的手抖得厉害，眼眶一下红了：「这年月……还有人肯分我一口。」从此他走在你侧后半步，替你盯着死角。' },
        { text: '警惕地拒绝，让他走开', effects: { supplies: -3, sanity: -4 }, reaction: '他垂下举起的手，眼里的光一点点灭了，转身蹒跚走进废墟——没骂你，那份沉默的失望反倒更扎心。' },
        { text: '假意答应，趁机搜他的包', effects: { supplies: 6, sanity: -10 }, reaction: '他反应过来时已被你按住，瞪大的眼里满是难以置信：「你……你居然……」这一眼，你往后许多个夜里都甩不掉。' },
      ],
      summary: '街角的幸存者', art: '4pentz', gen: 'gemini',
      weight: 1.2,
    },
    {
      narrative:
        '守了大半夜的收音机里突然窜出一阵电流杂音，随即一个略带沙哑的女声断断续续地浮了上来，一遍遍循环播放着同一段话：「……所有幸存者请向城西体育馆集结，那里有……军方……」声音随即被一阵更大的杂讯吞没，再没出来。你死死盯着收音机的指示灯，心跳如鼓——这是这些天来你听到的第一个具体的、像样的方向，可它也可能只是一段早已无人值守的循环录音。',
      choices: [
        { text: '记下方位，决定动身前往', effects: { supplies: -5, sanity: 10 }, reaction: '若有同伴在旁，他会重重拍你肩膀，声音发颤：「总算有个奔头了！」——绝望里抓住方向的人，眼里的光骗不了人。' },
        { text: '不轻信，继续守在原地', effects: { supplies: -6, sanity: -3 }, reaction: '没人来评判你的谨慎，可那段循环录音一遍遍重播，像在无声嗤笑你白白错过了唯一一个像样的方向。' },
      ],
      summary: '电台讯号', art: '1gqv5zk', gen: 'gemini',
      once: true,
      minTurn: 5,
      maxTurn: 20,
    },
    {
      narrative:
        '连日的奔波终于在身上结了账。手臂上那道前几天被铁丝划开的口子开始红肿发烫，渗出黏稠的脓液，一碰就钻心地疼，额头也烧得发昏，眼前的景物时不时叠出重影。你瘫坐在墙根，颤抖着翻遍背包，只摸出最后一小块硬邦邦的干粮。胃里像被一只手狠狠攥成了一团，绞着痛——可这点东西吃了，明天又该拿什么续命？身体和理智在为这最后一口粮各执一词。',
      choices: [
        { text: '硬扛，把干粮留到更危急时', effects: { hp: -10, supplies: -2, sanity: -6 }, reaction: '若有同伴看见你攥着干粮却饿得发抖，会皱眉摇头：「你这是拿命省粮，留得住口粮，留不住命啊。」' },
        { text: '吃光干粮，先撑过今天', effects: { hp: 4, supplies: -8 }, reaction: '没人苛责一个饿到极致的人填饱肚子，可空荡荡的背包就摆在脚边，无声提醒着你：明天的账，迟早要还。' },
      ],
      summary: '墙根分食干粮', art: 'hu4b98', gen: 'gemini',
      weight: 1.3,
    },
    {
      narrative:
        '一场毫无预兆的暴雨把整座城浇成了水帘，你狼狈地躲到天桥下，雨水顺着发梢往脖子里灌。透过密织的雨幕，你看见前方不远处的空地上，三五个感染者正一步步缓慢地聚拢过来，歪着头，似乎被空气里某种你闻不到的气味牵引着。它们还没发现你，可这条路是唯一的去向，而雨声盖住了你的脚步，也盖住了它们逼近的动静——你得趁它们围拢成形之前做个了断。',
      choices: [
        { text: '冒雨绕远路撤离', effects: { hp: -6, supplies: -5, sanity: -2 }, reaction: '那几个感染者在雨幕里茫然转着圈，始终没能锁定你——你绕的这一大圈远路，把它们和危险一并甩在了身后。' },
        { text: '躲进废弃车厢屏息等待', effects: { supplies: -7, sanity: -6 }, reaction: '感染者贴着车窗缓缓挪过，鼻尖几乎抵到玻璃上——你死死咬住袖子不让自己发抖，赌赢了，也熬干了一身的胆气。' },
      ],
      summary: '雨中的围拢', art: '1q1u3g4', gen: 'gemini',
    },
    {
      narrative:
        '街边一间药房的招牌还挂着，玻璃门居然只是虚掩，没被砸碎——这在被翻了个底朝天的街区里实属罕见。你贴着橱窗往里看，柜台后的玻璃陈列柜里隐约码着退烧药的盒子和成卷的绷带，正是你伤口急需的东西。可门后那片店堂黑得发沉，连白天的光都照不进几尺，里面安静得过分，安静得反而让人头皮发麻：这么好的地方还没被搜空，到底是运气，还是有什么东西守在暗处。',
      choices: [
        { text: '冲进去快速搜刮', effects: {}, outcomes: [
          { weight: 1, effects: { hp: -5, supplies: 4, sanity: -3 }, itemsGained: ['退烧药'], reaction: '黑暗里果然蛰伏着东西，扑上来时被你一把撂开——你抱着药盒夺门而出的狠劲，连暗处的危险都没料到。' },
        ] },
        { text: '先在门口听动静再进', effects: { supplies: -4, sanity: 2 }, reaction: '你贴门听了许久才迈步，这份耐性救了你——里头的动静证明，急着冲进去的人，多半已经没机会后悔了。' },
      ],
      summary: '废弃药房', art: '1dcyb6d', gen: 'gemini',
      once: true,
    },
    {
      narrative:
        '一个自称「老猎人」的中年男人在你搜寻物资时悄无声息地出现，肩上斜挎着一把改装猎枪，腰间别着几把磨得发亮的刀。他打量了你一番，沙哑地开口邀你加入他的小队，条件是共享所有物资、轮流守夜放哨。他眼角的皱纹里写满了在末世里熬过来的疲惫与警觉，可那双眼睛深处又藏着某种说不清的算计——抱团能多一分活路，也可能是把自己交到不知底细的人手里。',
      choices: [
        { text: '加入，人多力量大', effects: { supplies: -6, sanity: 10 }, reaction: '老猎人咧嘴一笑，拍了拍你的肩，把守夜的好位置让给了你——他眼里的算计淡了几分，多了点接纳同类的踏实。' },
        { text: '婉拒，独行更自由', effects: { supplies: -4, sanity: -5 }, reaction: '老猎人耸耸肩，转身没入街角的阴影，临走撂下一句：「独狼……在这世道活不长，小子。」语气里说不清是惋惜还是讥讽。' },
      ],
      summary: '老猎人的邀约', art: '1tym5co', gen: 'gemini',
      once: true,
      minTurn: 8,
      maxTurn: 22,
    },
    {
      narrative:
        '跋涉了不知多少天，城西体育馆那庞大的灰色轮廓终于撞进你的视野尽头，墙头上一面被风雨褪成惨白的军旗有气无力地耷拉着——早没了驻军，只剩一群幸存者勉力据守着这处摇摇欲坠的孤岛。通往大门的那片开阔广场上，横七竖八地躺着数十具身影，分不清是熬不住睡死过去的幸存者，还是早已停止呼吸的尸体。死寂笼罩着一切，你不知道贸然走过去，会惊醒怎样的东西。',
      choices: [
        { text: '绕到侧门，悄悄靠近', effects: { hp: -4, supplies: -5, sanity: 6 }, reaction: '侧门后探出一个面黄肌瘦的哨兵，见你没惊动广场上的尸群，低声赞了句「是个老手」，放你进了门——里头挤满了同样走投无路的人，配给紧巴、人心惶惶，这据点自己也撑不了多久了。' },
        { text: '高喊求救，赌里面有活人', effects: { hp: -10, supplies: -3, sanity: 8 }, reaction: '喊声惊得满广场身影一齐抬头，墙头也亮起了灯——有人冲下来把你拽进门，骂咧咧又佩服你这一嗓子的孤注一掷；可进了门才看清，这据守的体育馆早已弹尽粮绝，不过是又一处朝不保夕的避难所。' },
      ],
      summary: '抵达体育馆', art: 'muujmr', gen: 'gemini',
      once: true,
      minTurn: 22,
      weight: 2,
    },
    {
      narrative:
        '一辆抛锚的货车斜停在路中央，车头撞瘪在电线杆上。你用撬棍别开后厢的卷门，下一秒整个人都怔住了——成箱的瓶装水和压缩饼干整整齐齐码到车顶，足够一个人吃上小半个月。这简直是天上掉下来的横财，你几乎要笑出声。可撬门时金属碰撞发出的「哐当」巨响，在这条死寂得能听见自己心跳的街区里被无限放大，余音还在楼宇间打转——你不知道这声响已经招来了多少双眼睛。',
      choices: [
        { text: '抓紧时间，能搬多少搬多少', effects: { supplies: 22, hp: -4, sanity: -2 }, reaction: '巨响招来的几道身影刚摸近，就只看见你背着满满当当的战利品扬长而去——他们攥着空拳，眼里又是眼红又是忌惮。' },
        { text: '只拿一部分，速战速决', effects: { supplies: 10, sanity: 2 }, reaction: '你拎着东西干脆利落地撤了，引来的觊觎者扑了个空，只在心里暗骂一句：这人下手快，撤得更快。' },
      ],
      summary: '货车横财', art: '1knqda6', gen: 'gemini',
      requiresItem: '撬棍',
      once: true,
    },
    {
      narrative:
        '理智的堤坝在某个瞬间无声地裂开了缝。墙上那片陈年的水渍正一点点扭曲、蠕动，渐渐拼凑成一张张狰狞咧笑的脸；走廊尽头，那个早已死去的同伴竟好端端地站在那儿，朝你温柔地招手，用你最熟悉的声音一声声唤你过去，仿佛只要走过去，一切苦难就能结束。你死死掐着自己的手心，可你已经分不清眼前的这一切，究竟是真的，还是你濒临崩溃的脑子编织出的、最致命的温柔陷阱。',
      choices: [
        { text: '咬破舌尖，用剧痛把自己拽回现实', effects: { hp: -6, sanity: 14, supplies: -4 }, reaction: '血腥味在舌尖炸开，那些咧笑的脸轰然碎成水渍——能在崩溃的悬崖边把自己生生拽回来的，整片废土也没几个。' },
        { text: '跟着「同伴」走过去看看', effects: { hp: -15, sanity: -6, supplies: -3 }, reaction: '你伸向「他」的手，触到的却是冰冷的锈铁与裂开的伤口——幻象散去时，连你自己都分不清是哭还是笑了。' },
        { text: '蜷缩在角落，捂耳尖叫到天明', effects: { sanity: 4, supplies: -6 }, reaction: '没有人来安慰这一夜的崩溃，只有你自己的尖叫在空楼里回荡，天亮时嗓子哑了，那声呼唤却总算淡去了。' },
      ],
      summary: '崩溃边缘的幻象', art: 'xopghu', gen: 'gemini',
      requires: 'sanity<=22',
      weight: 2,
    },
    {
      narrative:
        '断粮已经是第三天了。胃像被一只无形的手攥成一团死结，每动一下都泛起酸水，饥饿啃噬着你的每一根神经，连思考都成了奢侈。你翻遍背包，只剩下一片早已过期、嚼起来味同嚼蜡的口香糖。就在你盯着它出神时，墙角「嗖」地窜过一道灰影——一只老鼠，肥硕的、活的老鼠。那一瞬间它在你眼里不再是脏东西，而是一块会跑的、热乎的肉。生存的本能压倒了一切体面。',
      choices: [
        { text: '设陷阱抓鼠，好歹是口肉', effects: { hp: -2, supplies: 8, sanity: -6 }, reaction: '没人看着你生火烤鼠，可那份饿到不挑食、还能冷静设套的求生本能，是熬过末世的人才有的硬气。' },
        { text: '嚼口香糖骗骗肠胃，继续赶路', effects: { hp: -8, supplies: -2, sanity: -4 }, reaction: '到手的活物被你白白放走，胃里那阵更凶的绞痛像在替你后悔——体面，在饿到第三天时是最廉价的奢侈。' },
      ],
      summary: '墙角觅食的老鼠', art: '16n682h', gen: 'gemini',
      requires: 'supplies<=15',
      weight: 1.8,
    },
    {
      narrative:
        '前方那间超市还算完整，可你刚靠近就听见「哗啦」一声枪栓上膛，几道黑洞洞的枪口从沙袋工事后探了出来，对准了你的脑门。一伙武装幸存者占据了这里，为首的啐了一口，冷冷开口：「这地盘是我们的。识相的，要么把身上的东西全留下滚蛋，要么……就把命留下。」他们足有五六个人，个个面带凶相，你形单影只，这场对峙从一开始就毫无胜算可言。',
      choices: [
        { text: '交出部分物资，息事宁人', effects: { supplies: -14, hp: 0, sanity: -3 }, reaction: '为首的掂了掂抢来的东西，鄙夷地朝你扬扬下巴：「算你识相。」手下几个哄笑着，把你当成了又一个软骨头。' },
        { text: '佯装离开，夜里再回来偷', effects: { supplies: 10, hp: -8, sanity: -6 }, reaction: '天亮时那伙人发现物资少了一截，气得破口大骂，却怎么也想不到，那个白天灰溜溜走掉的家伙竟敢摸回来。' },
        { text: '硬闯，拼一条生路', effects: { hp: -18, supplies: 6, sanity: -8 }, reaction: '你浑身是血却真撕开了一道口子冲出重围，沙袋后那几个人愣在原地——他们头回见着这么不要命的孤身一人。' },
      ],
      summary: '武装据点', art: '1fhpm3b', gen: 'gemini',
      weight: 1.2,
    },
    {
      narrative:
        '一夜之间暴雪封了城，气温像断了线一样直坠冰点。你蜷缩在一栋写字楼的角落，每呼出一口气都在面前凝成白雾，又瞬间在围巾上结成霜花。手脚早已冻得失去知觉，再不取暖，今夜怕是熬不过去。环顾四周，办公区里成堆的桌椅、文件、隔板都是现成的燃料——可一旦点起火来，那升腾的浓烟和跳动的火光，在死寂的雪夜里无异于一座灯塔，会把多远的感染者都引到你这儿来。',
      choices: [
        { text: '烧家具取暖，赌烟不会暴露位置', effects: { hp: 6, supplies: -8, sanity: -2 }, reaction: '火光在雪夜里跳动，远处几道踉跄的身影被引了过来又被你提前转移甩开——这一把火，你赌赢了体温也赌赢了命。' },
        { text: '硬扛严寒，靠意志撑过去', effects: { hp: -12, supplies: -4, sanity: -5 }, reaction: '没有谁见证你冻得发紫的嘴唇和咬碎的牙关，只有清晨第一缕天光照见你还在发抖——你硬是用意志熬退了死神。' },
      ],
      summary: '暴雪封城', art: '1euasya', gen: 'gemini',
      minTurn: 12,
    },
    {
      narrative:
        '收音机里破天荒地传来一段清晰得近乎奢侈的人声：一个嗓音温厚的男人正不紧不慢地念着一长串名字，替失散的人报着平安，间或插一句「城东有个互助社区还在收人，有吃的，有热水」。在这片只剩嘶吼与杂音的荒芜世界里，这平静的、属于活人的声音像一束光，让你鼻头一酸，眼眶不受控制地发起热来——你已经太久没听过有人好好说话了。可这声音背后，是真的庇护所，还是又一个精心设下的诱饵？',
      choices: [
        { text: '循着频率去城东找互助社区', effects: { supplies: -6, sanity: 12 }, reaction: '那温厚的男声仿佛隔着电波也察觉了你的脚步，念名字的间隙轻轻添了句：「再多撑一会儿，路上的朋友，门一直开着。」' },
        { text: '将信将疑，先回个讯号试探', effects: { supplies: -4, sanity: 5 }, reaction: '电波那头沉默片刻，随即传来一声温和的轻笑：「警惕是对的，这年头肯先敲门的人，才活得久。」' },
      ],
      summary: '幸存者电台', art: '1suvy0v', gen: 'gemini',
      once: true,
      minTurn: 6,
      maxTurn: 22,
    },
    {
      narrative:
        '路边用废家具垒起的街垒后头，蜷着一对母子。母亲早已没了气息，身体冰凉僵硬，可她至死都把孩子紧紧搂在怀里护着。那孩子约莫五六岁，烧得满脸通红，嘴唇干裂，呼吸又急又浅，眼看就要随母亲而去。你的手不由自主摸向背包里那瓶退烧药——那是你拼了命才从黑漆漆的药房里抢出来的，是你自己病倒时的救命稻草，也是这孩子此刻唯一的生机。给，还是不给？',
      choices: [
        { text: '把退烧药给孩子，带他上路', effects: {}, outcomes: [
          { weight: 1, effects: { sanity: 14, supplies: -8, hp: -2 }, itemsLost: ['退烧药'], reaction: '药喂下去，孩子滚烫的小手渐渐攥住了你的手指。后来听说此事的人都摇头叹服：拿救命药换个素不相识的娃，疯，却疯得让人敬重。' },
        ] },
        { text: '留药自保，默默离开', effects: { sanity: -12, supplies: -3 }, reaction: '没人会指责一个想活命的人，可那孩子追在身后渐弱的喘息，和母亲僵冷怀抱的那一幕，会在你往后的梦里反复审判你。' },
      ],
      summary: '高烧的孩子', art: 'g2o4bl', gen: 'gemini',
      requiresItem: '退烧药',
      once: true,
    },
    {
      narrative:
        '你这些天辛苦囤下的物资，终究还是引来了恶狼。深夜，你被一阵极轻的脚步声惊醒，借着窗外的微光，几个黑影正猫着腰、屏着息，从藏身处的几个方向悄悄合围过来，手里的家伙隐隐反着冷光。你这才彻底明白那句在末世里口耳相传的老话——富有，在这世道里不是底气，而是一张催命符。你藏得越好的东西，越是把你架在了刀尖上。今夜，这些家当和你的命，只能保一样。',
      choices: [
        { text: '设伏反杀，守住家当', effects: { hp: -14, supplies: -6, sanity: -8 }, reaction: '黑影们包抄进来时反被你一一撂倒，最后一个落荒而逃的边跑边回头，眼神里全是惊骇——这窝点的主人，根本是头藏起爪子的狼。' },
        { text: '弃一半物资金蝉脱壳', effects: { supplies: -30, hp: -2 }, reaction: '那几个掠夺者扑向你撂下的物资，争得头破血流，谁也没顾上去追——你用一半家当，聪明地买回了整条命。' },
      ],
      summary: '物资引狼', art: '1gkub7n', gen: 'gemini',
      requires: 'supplies>=70',
      weight: 1.5,
    },
    {
      narrative:
        '一处惨烈的厮杀过后，你身负重伤，跌跌撞撞地倒进了一座加油站的便利店，再也站不起来。血正顺着指缝一滴滴渗进地砖的缝隙里，视线像隔了一层水雾，怎么也聚不拢焦。四肢沉重得像灌了铅，意识一阵阵地往下沉，那是死亡温柔的召唤。你能感觉到生命正一寸寸从身体里抽走——要么咬碎牙关，拼上最后一口气替自己救命，要么……就闭上眼，让这一切就此安静地结束。',
      choices: [
        { text: '撕衣束伤、生火止血，搏一线生机', effects: { hp: 16, supplies: -10, sanity: -4 }, reaction: '烙铁般的剧痛里，你硬是把自己从鬼门关拽了回来。倘若有人撞见这一幕，定会倒吸一口凉气：这人的求生欲，比死神的镰刀还硬。' },
        { text: '保存体力静卧，赌身体自愈', effects: { hp: 4, supplies: -6, sanity: -6 }, reaction: '你把命交给了运气，听着血一滴滴渗进地砖缝。这一回侥幸没死，可那种把生死全押给身体的赌法，连你自己都后怕。' },
      ],
      summary: '濒死自救', art: '1g17zkd', gen: 'gemini',
      requires: 'hp<=20',
      weight: 1.6,
    },
    {
      narrative:
        '黄昏的余晖把街道染成一片血色。就在这时，原本各自游荡的感染者像接到了某种无声的号令，齐刷刷地抬起头，停顿了一瞬，随即调转方向，成群结队、踉踉跄跄地朝城北的方向涌去，黑压压一片，蔚为壮观又透着诡异。你贴着墙看着这骇人的一幕，心里盘算开来：尾随它们，或许能查明是什么在牵引这庞大的兽群、究竟城北发生了什么；可反过来，它们清空的街区此刻正大门洞开，是难得的搜刮良机。',
      choices: [
        { text: '尾随感染者潮，查个究竟', effects: { hp: -8, supplies: -4, sanity: -4 }, reaction: '没人敢跟着尸群走，你却贴着它们的尾巴探进了城北——这份要么找死、要么揭开真相的胆魄，寻常人想都不敢想。' },
        { text: '反向搜刮被清空的街区', effects: { supplies: 14, hp: -4, sanity: -2 }, reaction: '别人都盯着尸群惊慌，你却看准它们腾空的街区反向扎了进去——这份冷静算计的脑子，在废土上比力气还值钱。' },
      ],
      summary: '感染者迁徙', art: '91otiu', gen: 'gemini',
      minTurn: 8,
    },
    {
      narrative:
        '你推开一栋写字楼的玻璃门，头顶的应急灯竟还幽幽亮着——这栋楼的应急电源居然撑到了现在！久违的电流嗡鸣声让你一阵恍惚，仿佛旧世界从未崩塌。电梯指示灯在闪，监控探头还在缓缓转动，更让你心头一跳的是大堂角落那台屏幕还亮着的电脑：它竟然连得上残存的网络。这是你陷落以来，第一次有机会窥见这座城、乃至整个外界究竟变成了什么模样，甚至发出一声求救。',
      choices: [
        { text: '上网搜寻救援信息与安全区坐标', effects: { sanity: 12, supplies: -6 }, reaction: '屏幕亮起的瞬间，你像抓住了与旧世界相连的最后一根线。换作旁人，多半只顾着填肚子，你却先去抢那条能救命的情报——这眼界，救得了不止今天。' },
        { text: '撬开自动售货机，补充给养', effects: { supplies: 12, sanity: -2 }, reaction: '你三下五除二撬开了机器，饮料零食哗啦啦滚出来——眼前的实惠到了手，只是那台还连着网的电脑，被你头也不回地撂下了。' },
      ],
      summary: '残存电力', art: '1fb1qew', gen: 'gemini',
      once: true,
      minTurn: 4,
    },
    {
      narrative:
        '你沿着另一家便利店积满灰尘的货架间缓慢挪动，空气里漂浮着腐坏食物发酵出的酸馊气味，呛得人直反胃。每一步落下，碎玻璃和包装袋都在脚底发出细碎的声响。就在货架尽头的拐角后，传来一阵轻微而持续的摩擦声——像是有什么东西，或者什么人，正在那头不动声色地翻找。你下意识攥紧了手里的家伙，背包里那点见底的存粮提醒着你：不管前面是人是鬼，你都没有掉头空手离开的余地。',
      choices: [
        { text: '深入仓库搜寻', effects: { hp: 0, sanity: -3, supplies: 10 }, reaction: '拐角后翻找的那道身影僵了一下，见你不退反进、径直摸进仓库深处，悄没声地避到一旁——它也掂出了你不好惹。' },
        { text: '见势不妙撤离', effects: { hp: 0, sanity: -1, supplies: -4 }, reaction: '拐角后的动静停了，仿佛在等你露头。你却已悄然退出门外——那东西扑了个空，而你的存粮又少了一格。' },
        { text: '大声试探对方', effects: { hp: -6, sanity: -4, supplies: -3 }, reaction: '你一声喝问刚出口，拐角后猛地扑出个红了眼的家伙——这一嗓子等于自报家门，把藏在暗处的危险全勾了出来。' },
      ],
      summary: '废弃便利店', art: '1tybsna', gen: 'gemini',
    },
    {
      narrative:
        '傍晚的光线一点点黯淡下去，你正打算寻处地方过夜，远处一栋居民楼里却隐隐飘来一声声求救——那声音又细又弱，断断续续，像是个上了年纪的老人，每喊一句都得喘上半天。你循声靠近，那栋楼的楼道黑得像口深井，更让你头皮发麻的是墙上那一道道暗红的拖拽血迹，从楼梯口一路蜿蜒着延伸进黑暗深处。救，可能是救一条命，也可能是踏进别人布下的、用人声做饵的陷阱。',
      choices: [
        { text: '摸黑上楼查看', effects: { hp: -4, sanity: -2, supplies: -3 }, reaction: '黑井般的楼道里，那个虚弱的老人听见你的脚步，呜咽着哭出了声：「真的有人来了……真的有人肯上来……」' },
        { text: '留下食物离开', effects: { hp: 0, sanity: 2, supplies: -8 }, reaction: '你把食物搁在楼梯口便退了，那声求救渐渐弱下去——既没冒险，也没全然狠心，这一点分寸，夜里能让你睡得踏实些。' },
        { text: '无视求救继续走', effects: { hp: 0, sanity: -5, supplies: -2 }, reaction: '那声呼救在你身后一点点变细、断绝。没人会怪你保命，可那道戛然而止的余音，会黏在你心里好些天。' },
      ],
      summary: '楼上传声', art: '1exdg63', gen: 'gemini',
    },
    {
      narrative:
        '连续几天的低温像一把钝刀，慢慢割着这座城。你在清晨醒来时，发现水管早已冻得硬邦邦，拧开龙头一滴水也没有，手指更是僵得几乎弯不动，关节一动就钻心地疼。临时据点那扇没关严的窗户被凛冽的寒风灌得「咣咣」直响，整个人仿佛泡在冰水里。环顾四周，能生火的燃料只剩墙角那一小堆，火柴也没几根了——这一夜该怎么熬，眼下就得拿主意，再迟疑下去，怕是要被活活冻僵。',
      choices: [
        { text: '烧掉部分家具', effects: { hp: 2, sanity: -2, supplies: -6 }, reaction: '火苗窜起，僵硬的手指总算缓过了知觉——没人看着你，可这份当机立断地拆桌生火，正是会过日子的人才有的果断。' },
        { text: '硬撑到天亮', effects: { hp: -8, sanity: -4, supplies: -2 }, reaction: '你抱着膝盖在墙角抖了一整夜，省下的那点燃料压根不值这一身冻伤——硬扛得了寒，扛不住自己这股拧巴的倔。' },
      ],
      summary: '刺骨寒夜', art: '1bqrgph', gen: 'gemini',
    },
    {
      narrative:
        '走到一处十字路口，一辆侧翻的货车横在路中央，车头扭曲变形，车厢的卷门半开着，露出黑黢黢的内里。奇怪的是，四下里安静得反常，连平日里无处不在的感染者嘶吼都听不见一声，这种刻意的寂静反而让你后背发凉。你低头一看，货车周围的浮灰上印着一串清晰的脚印，边缘还很利落，显然就在不久前才有人来过——他们是搬空了货车走了，还是仍藏在附近某个角落，正盯着你？',
      choices: [
        { text: '迅速翻找货物', effects: { hp: -3, sanity: -2, supplies: 14 }, reaction: '远处暗中盯着货车的那双眼睛眼睁睁看你抢先翻空了车厢，气得直跺脚——犹豫的那一刻，猎物就成了别人的。' },
        { text: '观察后离开', effects: { hp: 0, sanity: 0, supplies: -3 }, reaction: '你盯着那串新鲜脚印想了想，终究没上前。藏在附近的人见你识趣离开，也松了口气——这一回，两边都没动手。' },
        { text: '设伏等待来人', effects: { hp: -5, sanity: -5, supplies: -4 }, reaction: '你蹲守了大半天，等来的却是更熟门熟路的对手——废土上想算计别人的，往往自己也正被人盯着。' },
      ],
      summary: '侧翻货车', art: '1b8hp84', gen: 'gemini',
    },
    {
      narrative:
        '夜幕即将合拢前，你又一次拧开收音机碰运气。一阵剧烈的电流杂音过后，竟跳出一段断断续续的军方频道——一个公事公办的男声反复提到，东区目前仍有一处隔离点在运作，接收并安置幸存者。可信号实在太弱，每说几个字就被汹涌的杂讯淹没，关键的方位和细节断断续续，拼不成完整的一句话。你屏住呼吸，把耳朵几乎贴到了喇叭上，心里却怎么也压不下那个疑问：这是真正的生路，还是一段早已作废的旧录音？',
      choices: [
        { text: '记录坐标前往', effects: { hp: -2, sanity: 1, supplies: -7 }, reaction: '你贴着喇叭一字字记下那断续的方位，攥紧了拳头——若有同伴在旁，会被你这股抓住一线生机就死不松手的劲头感染。' },
        { text: '怀疑是陷阱', effects: { hp: 0, sanity: -2, supplies: -2 }, reaction: '没人来评说你的多疑，只有那段含糊的录音在杂讯里反复打转，像在嘲弄你既不敢信、又舍不得彻底关掉。' },
      ],
      summary: '军方频段', art: '1bevstb', gen: 'gemini',
      once: true,
      maxTurn: 22,
    },
    {
      narrative:
        '正午的烈日把柏油路晒得发软，蒸腾的热浪里夹着一股刺鼻的焦糊味，连呼吸都觉得灼人。你已经记不清上一次喝到干净水是什么时候了，嗓子干得像要冒烟，吞咽时喉咙火辣辣地疼。头晕一阵接着一阵袭来，耳朵里嗡嗡作响，眼前的街景开始扭曲晃动，脚步也越来越沉，每抬一次腿都得耗尽全身力气。再这样硬撑下去，怕是没等遇上感染者，就先要倒在这空荡荡的马路中央了。',
      choices: [
        { text: '寻找阴凉休息', effects: { hp: 2, sanity: -1, supplies: -5 }, reaction: '没人看着你蜷进阴影里喘息，可这份不跟自己身体较劲、懂得见好就收的清醒，正是熬过酷暑的人才学得会的。' },
        { text: '继续赶路', effects: { hp: -7, sanity: -3, supplies: -3 }, reaction: '你晃晃悠悠硬撑着往前挪，脚步越来越虚浮——这股不要命的赶路劲，空荡的马路看在眼里都替你捏一把汗。' },
      ],
      summary: '酷热天气', art: '1l67dwm', gen: 'gemini',
    },
    {
      narrative:
        '一个抱着孩子的女人忽然从路边的废墟里踉跄着走出来，挡住了你的去路。她头发凌乱，脸上沾着泥灰，一双眼睛里盛满了化不开的疲惫与哀求，颤声问你能不能匀一点吃的。她怀里那个孩子瘦得只剩一把骨头，小脸蜡黄，虚弱得连眼睛都快睁不开了，脑袋无力地耷拉在母亲肩头。你的目光在她哀求的脸和孩子奄奄一息的模样之间徘徊，背包里那点口粮的分量，此刻沉甸甸地压在心上。',
      choices: [
        { text: '分出部分口粮', effects: { hp: 0, sanity: 4, supplies: -10 }, reaction: '女人捧着口粮，塞进孩子嘴里，泪水砸在地上，反复念着「好人，你是好人」——这世道还肯掰口粮给陌生人，她记你一辈子。' },
        { text: '拒绝并离开', effects: { hp: 0, sanity: -4, supplies: -2 }, reaction: '女人没再纠缠，只是抱紧了孩子退到一旁，那双盛满绝望的眼无声送你离开——你没错，可那一眼让你脚步发沉。' },
        { text: '索要物品交换', effects: { hp: 0, sanity: -2, supplies: -1 }, reaction: '女人怔了怔，默默从怀里摸出仅剩的一点值钱物什递来，眼神复杂——这买卖公平，却也凉薄得让你自己都别开了脸。' },
      ],
      summary: '饥饿母子', art: '1mkgpfq', gen: 'gemini',
    },
    {
      narrative:
        '夜色浓得化不开，你借着零星的星光赶路，一脚却踩中了横在地上的细绳——「哗啦啦」一串系在绳上的铁罐应声炸响，刺耳的金属碰撞声在死寂的街区里传出去老远。你这才发现自己一头扎进了别人精心布置的警戒区，四周的绳索和空罐头连成一张网。报警声还没散尽，不远处的黑暗里就传来一阵急促杂乱的脚步声，正朝你这个方向逼近——布下这片陷阱的人，已经被惊动了。',
      choices: [
        { text: '立刻逃离现场', effects: { hp: -2, sanity: -3, supplies: -4 }, reaction: '布陷阱的人循声赶到时，只看见空荡的街和还在晃的铁罐——你撤得干脆，他扑了个空，骂了句：「跑得倒快。」' },
        { text: '寻找埋伏者', effects: { hp: -8, sanity: -5, supplies: -6 }, reaction: '你迎着脚步声反扑过去，布陷阱的人没料到惊动的猎物竟敢回头找上门，惊得后退半步——可对方人多，这胆量代价不小。' },
      ],
      summary: '简易陷阱', art: '1hseqg0', gen: 'gemini',
    },
    {
      narrative:
        '断粮已经整整两天，胃部绞痛得让你冷汗直冒，弯着腰才能勉强站住。你拖着发软的双腿在巷子里搜寻，最后停在一处垃圾堆旁——发霉长毛的罐头和烂成一摊的水果混在一起，散发出令人作呕的腐臭，引来一群嗡嗡乱飞的苍蝇。理智在尖叫着警告你这东西吃下去多半要闹出人命，可饥饿这头野兽正啃噬着你的五脏六腑，让你的目光怎么也从那堆烂食上挪不开。吃，是赌命；不吃，是慢慢饿死。',
      choices: [
        { text: '勉强进食，赌肠胃扛得住', effects: { hp: -8, sanity: -2, supplies: 8 }, reaction: '没人看着你就着腐臭往嘴里塞，可那股饿到拿肠胃赌命的狠劲，是被饥饿逼到墙角的人才懂的孤注一掷。' },
        { text: '继续忍耐，保住肠胃', effects: { hp: -6, sanity: -3, supplies: 0 }, reaction: '你咽着口水转身离开了那堆烂食——空着肚子，却守住了不被一口脏东西放倒的清醒，这忍耐也是一种本事。' },
      ],
      summary: '垃圾堆翻找烂罐头', art: '25u8q3', gen: 'gemini',
      requires: 'supplies<=20',
    },
    {
      narrative:
        '深夜独自巡查据点时，那个声音又来了——已经死去的同伴，正用你再熟悉不过的语调，一声声地在某个角落唤着你的名字，温柔，清晰，仿佛就在耳边。你猛地回头，空荡荡的走廊里只有自己被拉得老长的影子，和你急促呼吸的回响。可那声音又响了起来，从黑暗的更深处。你的心跳如擂鼓，理智告诉你这是熬不住的脑子在作祟，可那呼唤实在太真，真得让你几乎要相信，他还在那里等你。',
      choices: [
        { text: '追寻声音来源', effects: { hp: -3, sanity: -8, supplies: -3 }, reaction: '你循声追进黑暗，墙角空无一人，那呼唤却又从背后响起——没有旁人，只有你越陷越深的脑子在编织温柔的陷阱。' },
        { text: '强迫自己冷静', effects: { hp: 0, sanity: -2, supplies: -2 }, reaction: '你死死掐着掌心，逼自己别去理那声音。没人见证这场和幻听的拉锯，可你没追过去，就是赢了它一回。' },
      ],
      summary: '空走廊里的回头', art: '1rrp3cr', gen: 'gemini',
      requires: 'sanity<=20',
    },
    {
      narrative:
        '你远远望见一栋楼的楼顶上，另一伙幸存者正忙着加固据点——他们用沙袋、铁皮和废木料把女儿墙垒高，搭起简易的瞭望和防御工事，看上去颇有章法。一个领头模样的人注意到你，朝你喊话，说他们手头缺趁手的工具，正缺人搬运沉重的建筑材料，问你愿不愿意搭把手。这是个结交人脉、换取信任的好机会，可在这世道，主动凑上去帮忙，也意味着把自己暴露在一群陌生人面前，福祸难料。',
      choices: [
        { text: '帮忙换取信任', effects: { hp: -2, sanity: 3, supplies: -4 }, reaction: '你扛起沉重的建材搭把手，领头那人看你卖力，眼神渐渐由戒备转为信任，临了塞给你一壶水：「往后这据点，有你一口。」' },
        { text: '索取报酬再帮', effects: { hp: -1, sanity: 0, supplies: 3 }, reaction: '领头的盯着你看了两秒，笑了：「明白人。」痛快地分了你一份物资——这世道，先谈好价钱反倒省了往后的猜忌。' },
        { text: '拒绝合作', effects: { hp: 0, sanity: -2, supplies: -2 }, reaction: '楼顶那群人耸耸肩，转回头继续垒墙，再没多看你一眼——一次结交人脉的机会，就这么从你指缝里溜走了。' },
      ],
      summary: '楼顶据点', art: '1f9ty72', gen: 'gemini',
    },
    {
      narrative:
        '你摸进一处地下停车场，霉味和机油味混杂着扑面而来。借着应急灯残存的微光，你在角落发现了一辆轿车，钥匙竟还插在点火孔上——它或许还能发动，能载你逃离这片死地。可仪表盘上的油表指针几乎贴着底，最要命的是，车子周围横七竖八地「睡」着好几只感染者，胸口微微起伏，随时可能被一点动静惊醒。要发动这辆车，等于在它们耳边敲响一记惊雷。',
      choices: [
        { text: '尝试启动车辆', effects: { hp: -4, sanity: -3, supplies: -5 }, reaction: '引擎一声轰鸣，睡着的感染者齐刷刷睁眼扑来——你猛踩油门冲出停车场的那一下，惊险得连后视镜里的它们都甩远了。' },
        { text: '拆下可用零件', effects: { hp: 0, sanity: -1, supplies: 4 }, reaction: '你没去碰那声等于自杀的引擎，而是悄无声息地卸下了能用的零件——这份不被横财冲昏头的冷静，是活下去的本钱。' },
      ],
      summary: '废弃轿车', art: '1dd0qq4', gen: 'gemini',
    },
    {
      narrative:
        '你踏进一座医院的急诊大厅，眼前的景象让你倒吸一口凉气：成排的病床被遗弃在过道里，床单凌乱地拖在地上，墙壁和地面上喷溅着早已发黑结痂的血迹，无声地诉说着这里曾经历过怎样的恐慌与厮杀。空气中弥漫着消毒水残味与腐败气息交织的怪味。就在大厅尽头的一间病房里，竟还透出一丝微弱摇曳的灯光——是有幸存者守在那儿，是还没断电的应急灯，还是别的什么在等着你。',
      choices: [
        { text: '进入病房查看', effects: { hp: -5, sanity: -4, supplies: 8 }, reaction: '那点幽光后藏着的东西扑出来时，被你硬生生挡了回去——敢往血迹斑斑的病房深处探的人，这医院里你不是头一个，却是少数活着出来的。' },
        { text: '只搜大厅区域', effects: { hp: -1, sanity: -1, supplies: 3 }, reaction: '你只在过道里翻拣了一通便收手，没去碰那间透光的病房——拿得不多，却也没把命搭进未知的黑暗里，稳当。' },
        { text: '立即撤离', effects: { hp: 0, sanity: 0, supplies: -2 }, reaction: '你扫了一眼那片血污就退了出来。空荡荡的急诊厅没人评判你的胆怯，可那间亮着灯的病房里究竟有什么，你再也不会知道了。' },
      ],
      summary: '医院大厅', art: '1bvjdce', gen: 'gemini',
    },
    {
      narrative:
        '一座公交站台的角落里，缩着一个白发苍苍的老人，他用脏污的衣襟死死捂着腿上的伤口，可暗红的血还是渗了出来，伤口边缘已经红肿溃烂，散发出隐隐的腐臭——再不处理，怕是性命难保。见你走近，他浑浊的眼睛里燃起一点光，颤巍巍地伸出手哀求你帮他包扎，喘着气说自己知道附近一处还没被发现的储藏点，愿意拿来交换。救他费时费力还要搭上药品，可那处储藏点的诱惑，也实在让人难以拒绝。',
      choices: [
        { text: '帮助老人', effects: { hp: 0, sanity: 3, supplies: -6 }, reaction: '老人看着你认真替他包扎，浑浊的眼里泛起泪光，哆嗦着握住你的手：「好人有好报……那地方，我一定原原本本告诉你。」' },
        { text: '逼问位置离开', effects: { hp: 0, sanity: -5, supplies: 5 }, reaction: '老人被你逼着吐露位置，看你转身就走、留他独对溃烂的伤口，眼里的光彻底熄了——你得了情报，丢了点别的东西。' },
        { text: '装作没看见', effects: { hp: 0, sanity: -3, supplies: -2 }, reaction: '老人伸出的手僵在半空，你已侧身走过。他没出声，可那储藏点的秘密和他这条命，都被你一并留在了身后的角落里。' },
      ],
      summary: '受伤老人', art: '1bn8uez', gen: 'gemini',
    },
    {
      narrative:
        '凌晨最深的那段黑暗里，远处忽然传来一声闷响——像是爆炸，又像是楼体坍塌。这声巨响仿佛捅了马蜂窝，整个街区的感染者瞬间被惊动，发出此起彼伏的嘶吼，开始无头苍蝇般在街道上乱窜游荡。很快，你藏身处那扇本就不算结实的门板上，便传来了「砰、砰」的撞击声，一下重过一下，门轴在剧烈晃动，门闩发出不堪重负的吱呀声。它们闻到你了，这道门到底还能撑多久，谁也说不准。',
      choices: [
        { text: '死守据点', effects: { hp: -5, sanity: -4, supplies: -6 }, reaction: '门板被你用身子和家具死死顶住，撞击声一下下砸在心上。天亮时门外的嘶吼散去，你瘫坐在地——硬是守住了这方寸活路。' },
        { text: '趁乱转移', effects: { hp: -3, sanity: -2, supplies: -5 }, reaction: '你趁着满街混乱悄然溜出后窗，把那群撞门的家伙甩在了空屋里——审时度势、该走就走，这份果断比死守更难得。' },
      ],
      summary: '夜间骚动', art: '1crzvrr', gen: 'gemini',
    },
    {
      narrative:
        '在一栋废弃的办公楼里，你意外发现一台还能工作的短波电台，机身指示灯一闪一闪。你戴上耳机，里面正不知疲倦地循环播放着一段录制好的求援信息：一个疲惫的声音报着遇险人数，恳求任何听到的人施以援手。然而无论你怎么调试，那个声音的来源位置始终模糊不清，方位飘忽，仿佛发自这座死城的任何一个角落，又仿佛哪里都不是。是回应这素未谋面的呼救，还是不卷入这桩前途未卜的麻烦，你一时拿不定主意。',
      choices: [
        { text: '尝试回应', effects: { hp: 0, sanity: 2, supplies: -3 }, reaction: '你对着话筒报出自己的位置时，耳机里那个疲惫的声音骤然拔高，带着哭腔：「有人……真的有人听见了！」' },
        { text: '关闭设备省电', effects: { hp: 0, sanity: -2, supplies: -1 }, reaction: '你伸手关了电台，那段循环的求救声戛然而止。没人怪你精打细算，可那句没人回应的呼救，就这么沉进了死城的寂静里。' },
        { text: '追查信号来源', effects: { hp: -3, sanity: -1, supplies: -6 }, reaction: '你扛着耳机一路追那飘忽的方位，绕了大半座死城——明知是趟前途未卜的麻烦，还肯为一声陌生的呼救奔走，这心肠不多见了。' },
      ],
      summary: '求援信号', art: '1fg7drn', gen: 'gemini',
    },
    {
      narrative:
        '路边一座破败的便利亭里，藏着一个瘦得脱了相的小男孩，约莫十来岁，浑身脏污，正死死攥着一把削水果的小刀，刀尖微微发颤。他显然在这里躲了很久。一看见你出现，他像受惊的小兽般猛地往墙角缩去，把刀举在胸前胡乱挥舞，喉咙里发出呜咽般的威胁声，那双过早染上恐惧与戒备的眼睛里，分不清是求生的凶狠，还是无助的哀求。一个孩子，在这吃人的世道里，既可能是你的拖累，也可能是仅存的一点暖意。',
      choices: [
        { text: '安抚并提供食物', effects: { hp: 0, sanity: 3, supplies: -7 }, reaction: '男孩举着小刀的手慢慢垂下，盯着你递来的食物迟疑半晌，终于扑过来狼吞虎咽，泪水混着饭粒——他眼里那点凶狠，终究化成了对人的信。' },
        { text: '强行搜身', effects: { hp: -2, sanity: -6, supplies: -2 }, reaction: '你按住挣扎的孩子翻他口袋，小刀划破了你的手。他哭嚎着的眼神里满是被背叛的惊恐——连个孩子都防，你心里也不是滋味。' },
        { text: '转身离开', effects: { hp: 0, sanity: -2, supplies: -2 }, reaction: '你转身离去，男孩举着刀缩在墙角，既松了口气又透着说不出的失落——这吃人的世道，连点头之交的暖意他都没敢奢望。' },
      ],
      summary: '孤身孩童', art: '1d2yyiq', gen: 'gemini',
    },
    {
      narrative:
        '走着走着，脚踝深处那道旧伤毫无征兆地复发了，一阵尖锐的刺痛顺着小腿窜上来，痛得你倒吸冷气，每往前迈一步都像踩在刀尖上。你试着放慢节奏、改用脚掌外侧着地，可疼痛非但没减轻，反而越走越重，整条腿都开始发沉发木。环顾四周，尽是断壁残垣，没有一处看着安全、能让你停下来好好歇脚处理的地方。更糟的是，西边的天色正以肉眼可见的速度暗下去，夜晚——感染者最活跃的时刻，就要来了。',
      choices: [
        { text: '停下处理伤口', effects: { hp: 3, sanity: -1, supplies: -4 }, reaction: '你寻了处避风的角落仔细缠好脚踝，没逞强硬撑——天色虽暗得快，可懂得在夜幕前给自己留条退路的人，才走得到最后。' },
        { text: '咬牙继续前进', effects: { hp: -7, sanity: -2, supplies: -2 }, reaction: '你咬着牙一瘸一拐往前挪，每一步都踩在刀尖上，旧伤越拖越重——夜色追了上来，你这股硬撑，赌得实在凶险。' },
      ],
      summary: '旧伤发作', art: '1edyi65', gen: 'gemini',
    },
    {
      narrative:
        '拐过一道矮墙，几名持枪的幸存者不知从哪儿冒了出来，呈扇形散开，堵死了你前行的所有去路。为首的男人面无表情地朝你的背包扬了扬下巴，声音冷硬：「把包打开，我们检查检查。」他们少说有四五个人，枪口虽未直指你的要害，却也始终没有半分放下的意思，黑洞洞的枪口在暮色里泛着冷光。对方人多势众，占尽上风，这场遭遇你毫无还手的本钱，怎么应对，全在自己一念之间。',
      choices: [
        { text: '交出部分物资', effects: { hp: 0, sanity: -2, supplies: -12 }, reaction: '为首的男人翻检着抢来的东西，冷哼一声挥手放你走——在他眼里，你不过是又一个乖乖掏空口袋的过路羔羊。' },
        { text: '寻找机会反抗', effects: { hp: -9, sanity: -5, supplies: -6 }, reaction: '你瞅准空当暴起反抗，虽吃了亏，却也让那几个掠夺者实打实地慌了一下——他们没想到这落单的猎物竟敢咬人。' },
        { text: '低头谈判', effects: { hp: -1, sanity: -1, supplies: -7 }, reaction: '你不卑不亢地讨价还价，为首的眯眼打量你半晌，终是收了部分东西放行——能在枪口下谈成条件，这口才也是保命的本事。' },
      ],
      summary: '路遇掠夺者', art: 'qyxyu2', gen: 'gemini',
    },
    {
      narrative:
        '没有一丝预兆，瓢泼大雨砸了下来，雨点密得像一张连绵的网，瞬间把整条街浇成了泽国。雨水顺着头发、脖颈往衣服里灌，浑身上下没一处干的地方，冰凉刺骨。脚下，堵塞的下水道口正咕嘟咕嘟往外翻涌着浑浊发臭的污水，迅速漫过了脚踝。你必须尽快找到能遮风避雨的地方，否则背包里那点干粮和好不容易攒下的火柴、绷带一旦被彻底浸透，就全成了废物，而你也会在湿冷里迅速失温。',
      choices: [
        { text: '躲进地下通道', effects: { hp: -2, sanity: -2, supplies: -4 }, reaction: '你护着背包钻进通道避雨，保住了那点火柴和干粮——没旁人看着，可这份先顾物资再顾自己的精明，正是熬过雨季的门道。' },
        { text: '冒雨前进', effects: { hp: -5, sanity: -3, supplies: -5 }, reaction: '你顶着瓢泼大雨硬闯，等找到落脚处，背包里的火柴绷带早泡成了一团烂泥——逞这一时之快，把家底都赔进了雨里。' },
      ],
      summary: '暴雨来袭', art: '1eud36b', gen: 'gemini',
    },
    {
      narrative:
        '你爬上一栋楼的屋顶，借着高处的视野观察周遭动静，正打算规划明天的路线，目光却被远方天际一缕笔直升起的浓烟攫住了。那烟很黑、很浓，在灰白的天空下格外扎眼。浓烟意味着火，而火，在这座早已断水断电的死城里，几乎只可能是活人留下的痕迹——也许是有幸存者在生火做饭、取暖，是难得的同类；可也可能是一场冲突或厮杀刚刚平息，那升起的，是劫掠后焚烧留下的硝烟。',
      choices: [
        { text: '前往侦察', effects: { hp: -2, sanity: -1, supplies: -6 }, reaction: '你摸向那缕浓烟，没人知道前头是同类的炊烟还是劫后的余烬——可愿意为一线同伴的可能去探虚实，这胆色在废土上稀罕。' },
        { text: '保持距离', effects: { hp: 0, sanity: 0, supplies: -2 }, reaction: '你盯着那缕浓烟看了片刻，终究没去——稳妥是稳妥，只是那头若真有个能搭伙的同类，也随这决定一起飘散了。' },
      ],
      summary: '远方浓烟', art: '1kvnjos', gen: 'gemini',
    },
    {
      narrative:
        '一连几天的阴雨让据点里潮气弥漫，你这才发现墙角和天花板的缝隙里悄悄爬满了大片大片的黑绿色霉斑，毛茸茸的，看着就让人胃里发紧。更糟的是，你囤在角落的一部分食物也未能幸免——罐头封口处沁出了可疑的霉点，袋装的干粮结了块、长了毛，散发出一股酸腐的怪味。整个空间又闷又湿，黏腻的空气让人喘不过气，躺下去翻来覆去怎么也睡不着，再放任下去，怕是连身子都要被这环境拖垮。',
      choices: [
        { text: '彻底清理环境', effects: { hp: 1, sanity: -1, supplies: -5 }, reaction: '你忍着恶心把霉斑和坏食一一清掉，据点重新透了气——这份在末世里还肯收拾家、不肯让自己活成猪窝的讲究，撑住了人的体面。' },
        { text: '暂时放任不管', effects: { hp: -4, sanity: -2, supplies: -2 }, reaction: '你嫌费事懒得动，任那片霉斑越爬越大。没人催你，可没过几天，潮湿的霉气就先一步在你嗓子和肺里安了家。' },
      ],
      summary: '据点霉变', art: '1e6bsj1', gen: 'gemini',
    },
    {
      narrative:
        '一辆军用卡车歪斜地停在路边，车身布满弹孔，轮胎瘪了大半。你绕到车旁，发现驾驶座边的地上倒着一具早已风干的士兵遗骸，他至死都保持着戒备的姿势，怀里还死死搂着一卷地图和几个空空如也的弹匣——子弹想必是打到了最后一刻。四下里安静得反常，连风都仿佛屏住了呼吸，那种死寂压得人喘不过气，让你不由自主地猜想：这名士兵临死前，究竟在抵挡着什么样的东西。',
      choices: [
        { text: '检查遗物', effects: { hp: -2, sanity: -3, supplies: 7 }, reaction: '你翻出那卷地图和弹匣时，士兵风干的面容仿佛在无声注视——没人评判生者翻检逝者，可你拿走遗物时，心头到底沉了一下。' },
        { text: '默默离开', effects: { hp: 0, sanity: -1, supplies: -2 }, reaction: '你朝那具戒备至死的遗骸默默颔首，转身离去。没碰他一分一毫，可那份本可救急的物资，也随你的脚步留在了死寂里。' },
        { text: '掩埋遗体', effects: { hp: 0, sanity: 2, supplies: -4 }, reaction: '你费力刨开硬土，把士兵和他怀里的地图一道掩埋。倘若有同伴在旁，会默默帮你培土——这世道还肯为陌生人立座坟的，没几个了。' },
      ],
      summary: '遗弃军车', art: '1kx1c4g', gen: 'gemini',
      weight: 0.7,
    },
    {
      narrative:
        '你沿着高架桥下方阴影里穿行，忽然在坍塌的混凝土堆里发现了一节脱轨的地铁车厢，半截被压在断梁碎石之下，车身扭曲变形。车门有被人硬生生撬开过的痕迹，门缝里飘出一股混着腐臭与潮湿霉味的浊气，让人胃里翻腾。车厢深处黑得看不真切，隔一会儿就传来一声极轻的金属碰撞回响，不知是风，是塌落的零件，还是别的什么。天色正一点点暗下来，你得赶在天黑前决定，要不要钻进那片黑暗里去赌一把。',
      choices: [
        { text: '钻入车厢搜寻', effects: { supplies: 8, sanity: -4 }, reaction: '你钻进那片散着腐臭的黑暗，摸出了别人不敢取的东西——敢往脱轨车厢的肚子里探的，寻常人光是想想就两腿发软。' },
        { text: '绕开继续前进', effects: { supplies: -3 }, reaction: '你看了眼那黑黢黢的车厢，摇摇头绕了过去。没人笑你胆小，可天黑前少了这点进项，夜里的肚子会替你后悔。' },
        { text: '在入口观察片刻', effects: { supplies: -4, sanity: -1 }, reaction: '你在车门口蹲守观望，那阵金属轻响始终摸不清是风是鬼——既没冒进也没全退，这份小心，起码让你没栽进黑暗里。' },
      ],
      summary: '脱轨的地铁车厢', art: '1uglvej', gen: 'gemini',
    },
    {
      narrative:
        '你抬头打量一栋高耸的写字楼时，注意到光洁的外墙上垂着一串崭新的绳索勒痕，深深嵌进墙缝——显然就在近期，有人借着这绳子从高层垂降下来过，身手不凡。那整面玻璃幕墙在阴沉的天光下泛着冷光，偶尔有一两点微弱的反光在某扇窗后一闪而逝，你说不清那是天光的折射，还是有人正躲在暗处用望远镜窥视着你。大风卷起楼下残破的广告布，「猎猎」作响，吹得人神经从头到脚都绷得紧紧的。',
      choices: [
        { text: '攀上低层查看', effects: { hp: -6, supplies: 5 }, reaction: '你攀着墙缝往上爬，若窗后真有人用望远镜窥着，定会暗暗心惊——明知可能有埋伏还敢登门的，不是愣头青就是真高手。' },
        { text: '远离此地', effects: { supplies: -2 }, reaction: '你避开那面泛着冷光的幕墙绕道而行——窗后若真藏着猎人，这一退正好让他扑了空，警觉救了你一命。' },
        { text: '留下记号再走', effects: { supplies: -3 }, reaction: '你在墙根留下个只有自己看得懂的暗记便走了——不冒进、不贪功，却为日后留了线索，这份盘算比莽撞高明。' },
      ],
      summary: '绳索痕迹', art: '1ieypig', gen: 'gemini',
    },
    {
      narrative:
        '一处地下停车场的入口被一道生锈的铁链半封着，链条松松垮垮地挂在卷门上，留出一道仅容一人侧身的缝隙。里头黑沉沉的，安静得异乎寻常。你借着门外的光往里探看，地面的一汪积水里清晰地印着车辙和拖拽的痕迹，一道道延伸进黑暗深处——分明是有人在不久前才往这儿运过、或是搬走过一批物资。混着机油和霉味的阴冷空气从缝里渗出来，扑在脸上，让你心里直犯嘀咕：这地方，到底是宝库，还是别人的窝点。',
      choices: [
        { text: '撬开铁链进入', effects: { supplies: 10, hp: -4 }, reaction: '铁链应声断开，你侧身钻进那片漆黑——别人见了车辙拖痕只会绕道，你却敢往别人的窝点里探，这胆识够硬。' },
        { text: '守在外面观察', effects: { supplies: -4 }, reaction: '你蹲在缝隙外盯了许久，始终没贸然进去——里头到底是宝库还是陷阱，你宁可饿一顿也不拿命去赌，稳。' },
        { text: '立刻离开', effects: { supplies: -2 }, reaction: '你看了眼那道渗着阴冷的门缝，转身就走。没人笑你避事，可那拖进黑暗里的物资是福是祸，从此与你无关了。' },
      ],
      summary: '地下车库', art: '1c5h9o9', gen: 'gemini',
    },
    {
      narrative:
        '路过一间宠物医院时，一阵断断续续、急促的抓挠声从笼舍区的深处传了出来，指甲刮过金属的声音听得人牙根发酸。玻璃门早已碎裂，散落一地的病历纸被穿堂风吹得簌簌翻动，几张还沾着暗褐色的痕迹。医院里通常会备着不少罐装的宠物食品和消毒用品，对眼下的你都是好东西。可那笼子里关着的，究竟是侥幸活下来、饿坏了的小动物，还是早已在病毒里变了形、成了别的什么东西，你心里一点底都没有。',
      choices: [
        { text: '进入寻找食物', effects: { supplies: 7, hp: -5 }, reaction: '笼舍深处扑出的东西被你撂翻在地，你抹了把脸继续翻找罐头——这份不被诡异抓挠声吓退的定力，寻常人早掉头跑了。' },
        { text: '封住入口离开', effects: { supplies: -3, sanity: 2 }, reaction: '你拖来杂物把笼舍那头堵死才离开——拿不到里头的东西，却也没让那未知的危险跟出来，这分寸拿捏得稳妥。' },
        { text: '不予理会', effects: { supplies: -2 }, reaction: '你听着那抓挠声越来越急，头也不回地走了。没人评判你的取舍，可那笼里到底是猫狗还是别的，成了永远的悬念。' },
      ],
      summary: '宠物医院', art: '1czizt7', gen: 'gemini',
    },
    {
      narrative:
        '夜幕降临前，你撞见一所幼儿园，那道彩绘的院墙竟被人用沙袋和铁皮重新加固过，显然曾有人想把这里改造成据点。可推开虚掩的小门，操场上却是一片死寂：五颜六色的小鞋子东一只西一只地散落着，滑梯和木马上积满了厚厚的灰，被遗弃的玩偶歪倒在沙坑里，那种属于孩子的鲜艳与眼前的荒凉撞在一起，压抑得让人喘不过气。就在这令人窒息的安静中，某间教室里，传来一下、又一下，规律得诡异的敲击声。',
      choices: [
        { text: '搜索教学楼', effects: { supplies: 9, sanity: -5 }, reaction: '你循着那诡异的敲击声摸进教室，搜空了一间间散落童鞋的屋子——能压着头皮在这种地方翻找的人，神经比常人粗得多。' },
        { text: '迅速撤离', effects: { supplies: -3 }, reaction: '那规律的敲击声听得你头皮发麻，你没逞强，扭头便撤——有时候，听从直觉掉头走，本身就是最聪明的选择。' },
        { text: '在外围等待', effects: { supplies: -4 }, reaction: '你守在加固的院墙外，既不进也不走，听那敲击声一下下响着——这份既谨慎又不甘的犹豫，把宝贵的工夫都耗在了门口。' },
      ],
      summary: '废弃幼园', art: '1dcrk34', gen: 'gemini',
    },
    {
      narrative:
        '沿着河道走了许久，你在浅滩处发现一艘搁浅的小型巡逻艇，半个船身斜插进淤泥里。船体侧面有一道狰狞的撞击凹痕，金属皮都翻卷了起来，驾驶舱的玻璃碎了一地，内壁上溅着大片早已凝固发黑的血迹，无声地诉说着这里曾发生过的惨剧。落日的余晖把河面染成一片橙红，浑浊的河水一下下拍打着倾斜的船壳，发出空洞的回响，四下里再看不见任何活人的踪影，只有你和这艘载着死亡气息的残船。',
      choices: [
        { text: '登船搜查', effects: { supplies: 11, hp: -3 }, reaction: '你踩着倾斜的船壳爬进那间溅满黑血的驾驶舱，翻出了不少好东西——敢登一艘载满死亡气息残船的人，胆子练得够大了。' },
        { text: '拆取零件备用', effects: { supplies: 4 }, reaction: '你只在船外卸了些用得上的零件就收手，没钻进那阴森的舱里——拿得不多，却也没去招惹船上那桩没说完的惨剧。' },
        { text: '离开河岸', effects: { supplies: -2 }, reaction: '你望着那艘搁浅的残船，终究没上前，沿着染成橙红的河岸走远了——稳是稳，只是船里若藏着什么，你再不会知道。' },
      ],
      summary: '搁浅巡艇', art: '1e8maib', gen: 'gemini',
    },
    {
      narrative:
        '一座废弃影院的大厅里，褪色破损的电影海报还歪歪斜斜地挂在墙上，定格着旧世界某个寻常的周末。一台自动售货机倒在角落，玻璃碎裂，里头的零食却好像还有剩。你正打算上前，放映厅那扇厚重的隔音门后，却传来一阵「吱嘎——吱嘎——」的座椅摩擦声，缓慢而沉重，仿佛有什么东西正在那片绝对的黑暗里，一排排地摸索着、移动着。空气里弥漫的潮湿霉味，混进了一丝若有若无的腥气。',
      choices: [
        { text: '摸黑进入放映厅', effects: { supplies: 8, sanity: -6 }, reaction: '你推开那扇隔音门，迎着座椅摩擦的怪响一排排摸过去——敢往伸手不见五指、还混着腥气的黑里走的，胆魄不是一般人有的。' },
        { text: '只搜大厅', effects: { supplies: 3 }, reaction: '你只在大厅拣了点零食便罢手，没去碰那扇透着腥气的放映厅门——拿得不多，却也稳稳避开了藏在黑暗里的东西。' },
        { text: '离开影院', effects: { supplies: -2 }, reaction: '你听着门后那缓慢沉重的摩擦声，二话不说退了出去——这份当机立断的撤退，把一场未知的凶险隔在了门外。' },
      ],
      summary: '旧电影院', art: '1ekcghg', gen: 'gemini',
    },
    {
      narrative:
        '天桥中段横着一辆被烈火吞噬过的公交大巴，车身焦黑扭曲，玻璃窗无一幸免地碎成了齿状的残片。你扒着车窗往里看，过道和座椅间堆着不少没来得及带走的行李箱包，或许藏着有用的东西；可在那些行李之间，几具被烧得只剩轮廓的焦黑人形蜷缩着，保持着死前挣扎的姿态，看得人头皮发麻。远处那群感染者还沉浸在白日的呆滞里没有苏醒，可这片刻的安宁随时会被打破，留给你翻找的时间，少得可怜。',
      choices: [
        { text: '翻找遗留行李', effects: { supplies: 9, sanity: -4 }, reaction: '你在焦黑的人形之间翻拣行李，手都没怎么抖——能在烧焦的尸骸堆里冷静搜刮的人，这份心理素质叫旁人后背发凉。' },
        { text: '快速通过', effects: { supplies: -3 }, reaction: '你屏着气快步穿过焦黑的车厢，没去碰那些行李，赶在尸群苏醒前脱了身——分得清轻重缓急，命比物资金贵。' },
        { text: '推下障碍开路', effects: { hp: -4, supplies: 2 }, reaction: '你使劲把烧毁的大巴推下天桥开道，轰隆一声响——动静虽大，可这股说干就干、亲手清路的狠劲，让人不敢小瞧。' },
      ],
      summary: '焚毁大巴', art: '1g82ue3', gen: 'gemini',
      minTurn: 15,
    },
    {
      narrative:
        '凌晨最沉的时分，你正在黑暗里赶路，眼角余光忽然瞥见一栋办公楼的顶层窗口亮起一束光——那光只闪了短短几秒，便倏地熄灭，干净利落，像是有人在刻意发送某种信号。你停下脚步，屏息凝神地盯着那片漆黑的楼影，四周却再无半点动静，唯有夜风在鳞次栉比的楼宇间盘旋呜咽。那束转瞬即逝的光让你心头百感交集：它可能是另一个孤独的幸存者在试探同类，也可能是猎人为引诱猎物精心布下的诱饵。',
      choices: [
        { text: '尝试靠近联络', effects: { supplies: -4, sanity: 3 }, reaction: '你朝那扇黑窗谨慎地回了个光信号，片刻后，那束光又闪了两下作答——茫茫死城里，两个孤独的人，隔着夜色认出了彼此。' },
        { text: '隐藏观察整夜', effects: { supplies: -6 }, reaction: '你猫在暗处守了整宿，那束光再没亮过。没人见证你的耐心，可不轻易把自己暴露给一束来历不明的光，这份沉得住气救命。' },
        { text: '直接离开', effects: { supplies: -2 }, reaction: '你看了眼那片漆黑的楼影，转身没入夜色。是同类还是诱饵，从此再无从分辨，这谜底被你利落地留在了身后。' },
      ],
      summary: '短暂灯光', art: '1h8pa0c', gen: 'gemini',
      requires: 'sanity>=60',
    },
    {
      narrative:
        '路过一家面包工坊，一缕极淡的、久违的焦香钻进了你的鼻腔，恍惚间竟让你想起了灾难前清晨街角的烟火气，鼻子一阵发酸。后厨发酵室的门半掩着，里头还残存着一些尚未彻底腐坏的面粉和原料。可让你警觉的是，断电这么久的设备旁，竟有被人近期擦拭、摆弄过的痕迹，电源线被重新接驳了——显然有人曾试图在这里重新生火做饭，重拾一点旧日的烟火。那个人，现在还在不在附近？',
      choices: [
        { text: '搜集剩余食材', effects: { supplies: 10 }, reaction: '你麻利地把没坏的面粉原料一扫而空——没去多想那个想重拾烟火的人，只把实打实的口粮装进了包，这取舍干脆。' },
        { text: '检查设备来源', effects: { supplies: 3, sanity: -2 }, reaction: '你顺着那截重新接驳的电线查探，想弄清这里还有没有人——比起食物先摸清环境，这份警觉省得日后被人从背后捅刀。' },
        { text: '立即离开', effects: { supplies: -2 }, reaction: '你嗅着那缕焦香，心里发虚，转身就走——也许只是个想重拾旧日的可怜人，可宁可错过也不犯险，你走得果断。' },
      ],
      summary: '面包工坊', art: '1lxod6j', gen: 'gemini',
    },
    {
      narrative:
        '钻进一条狭窄的背街小巷，你发现两侧斑驳的墙面上用喷漆和粉笔画满了密密麻麻的箭头标记，一个接一个地指向巷子深处，显然是某个幸存者团体为方便后来者认路而留下的暗号。可正当你打算循着记号前进时，却发现其中好几个关键的箭头被暗红的血迹覆盖、涂改过，原本的指向变得模糊难辨——是有人在仓皇逃命时蹭上的血，还是有恶意者故意篡改了路标，要把后来人引进死路？天边的最后一点亮光，正迅速褪去。',
      choices: [
        { text: '沿标记前进', effects: { supplies: 6, sanity: -3 }, reaction: '你赌那些箭头还指着活路，一路追了下去——明知有的记号被血改过，还敢顺藤摸瓜，这份胆子赌赢了便是收获。' },
        { text: '自行选择路线', effects: { supplies: -3 }, reaction: '你看穿那些被血迹涂改的箭头不能全信，索性自己择路而行——不被别人留的记号牵着鼻子走，这份独立的判断救命。' },
        { text: '擦除部分记号', effects: { supplies: -4 }, reaction: '你抹掉了几个被恶意篡改的箭头，免得后来人被引进死路——大难当头还肯替素不相识的人擦去陷阱，这心肠难得。' },
      ],
      summary: '墙上箭头', art: '1cd3pft', gen: 'gemini',
    },
    {
      narrative:
        '你翻进一处社区泳池，池水早已干涸见底，深坑里积满了腐烂的落叶、被风刮进来的杂物和一层污浊的淤泥，散发着隐隐的霉味。可就在那堆杂物之间，你瞥见了几个码放整齐的蓝色防水箱——在末世里，会用防水箱密封保存的，准是要紧的物资。要拿到它们，你就得攀着满是青苔、湿滑无比的池壁爬下那道深坑，稍有不慎就会摔伤。四下里安静得诡异，连只虫鸣都没有，那种空旷的死寂让你后颈直冒寒气。',
      choices: [
        { text: '下池取箱子', effects: { supplies: 12, hp: -5 }, reaction: '你攀着滑腻的池壁一点点挪下深坑，几次险些摔下去，终究把那几个防水箱拖了上来——这股不要命的执拗，看得人心惊。' },
        { text: '放弃风险', effects: { supplies: -2 }, reaction: '你掂量了那道湿滑深坑的凶险，摇头作罢——眼前的物资虽诱人，可摔伤在这世道等于送命，懂得割舍才走得远。' },
        { text: '寻找安全路线', effects: { supplies: -4 }, reaction: '你绕着泳池找了半天稳妥的下法，耗了不少工夫——既想要箱子又不肯冒险，这份瞻前顾后，把时间全磨在了犹豫上。' },
      ],
      summary: '干涸泳池', art: '1debig2', gen: 'gemini',
      minTurn: 20,
    },
    {
      narrative:
        '穿过一片居民区时，你忽然停下了脚步——几只灰扑扑的鸽子正扑棱着翅膀，聚集在某栋楼的阳台附近，咕咕地啄食着什么。在这连麻雀都几近绝迹的死城里，活物的出现稀罕得近乎神迹。鸽子聚集，往往意味着那里还残留着可供啄食的粮食，或许那户人家的橱柜里还藏着没被搜刮的存货；可成群的活物同样会引来别的觊觎者，甚至别的东西。傍晚浓重的阴影正一寸寸爬上街道，把一切都染上不安的暗色。',
      choices: [
        { text: '进入楼内查看', effects: { supplies: 8, hp: -3 }, reaction: '你读懂了鸽群聚集的暗示，摸进那户人家翻出了没被搜空的存货——能从几只鸟身上看出门道，这观察力是活下来的本钱。' },
        { text: '捕捉鸽子充饥', effects: { supplies: 5 }, reaction: '你屏息逼近，猛地扑出抓住一只扑棱的鸽子——在这连麻雀都绝迹的死城，能徒手逮着口活肉，这身手实打实地管用。' },
        { text: '保持距离', effects: { supplies: -2 }, reaction: '你警惕着成群活物会招来的觊觎者，没敢靠近便绕开了——稳是稳，只是那点唾手可得的口粮也随之飞了。' },
      ],
      summary: '异常鸽群', art: '1df2ubk', gen: 'gemini',
    },
    {
      narrative:
        '路过一间乐器店，本该死寂的店堂里却飘出几个低沉、缓慢的音符——一架立式钢琴的琴键，正在没有任何人触碰的情况下，自顾自地缓缓下沉、抬起，弹奏出零落而诡异的旋律。店里没有一丝风，落地玻璃完好无损，也看不见任何人影或动物的踪迹。那不成调的乐声在空荡的店里回荡，每一个音符都像有只冰凉的手抚过你的脊背，激起一层鸡皮疙瘩。理智告诉你这世上没有鬼，可眼前这一幕，怎么也找不出合理的解释。',
      choices: [
        { text: '检查声音来源', effects: { sanity: -7, supplies: 4 }, reaction: '你硬着头皮凑近那架自顾自弹奏的钢琴一探究竟——明知毛骨悚然还偏要弄个明白，这股不信邪的劲头，寻常人早吓得腿软。' },
        { text: '封门离开', effects: { supplies: -3 }, reaction: '你拉上店门把那诡异的琴声关在里头——不去深究，也不慌不忙，这份遇到说不清的事就敬而远之的分寸，稳当。' },
        { text: '匆忙逃离', effects: { supplies: -2, sanity: -1 }, reaction: '那不成调的琴声激得你后颈发麻，你慌不择路地夺门而出——逃是逃了，可那点没解开的恐惧，还黏在你心里。' },
      ],
      summary: '钢琴异响', art: '1lo4kru', gen: 'gemini',
      requires: 'sanity>=40',
    },
    {
      narrative:
        '一辆白色的冷链运输车孤零零地停在路边，车厢门紧紧闭着。供电中断了这么久，车里的制冷设备早就停摆，可这种专门运送生鲜的厢式货车，内部隔热密封得严实，里头那些速冻的肉类、食材说不定还没有彻底腐坏变质，对眼下断粮的你而言无异于一座宝库。然而车厢门的缝隙周围，黑压压地爬满了苍蝇，嗡嗡作响，一股令人作呕的腐臭气味从里头隐隐渗出——这味道，让人既贪恋那点可能的食物，又对开门后将面对的景象心生恐惧。',
      choices: [
        { text: '打开车厢检查', effects: { supplies: 9, hp: -4 }, reaction: '你捂着口鼻拉开车门，在扑面的腐臭里抢出了还没坏透的食材——敢顶着这股恶心钻进苍蝇堆里翻找的，饿狠了也得有股狠劲。' },
        { text: '只拆取工具', effects: { supplies: 3 }, reaction: '你没去开那扇渗着腐臭的车门，只卸了点车上的工具——拿得不多，却也没拿肠胃去赌一车说不准的腐肉，聪明。' },
        { text: '离开恶臭区域', effects: { supplies: -2 }, reaction: '你被那股腐臭熏得直反胃，捂着鼻子快步走开。没人笑你挑剔，可那一车或许还能吃的食材，你到底没敢碰。' },
      ],
      summary: '冷链货车', art: '1bn9y2v', gen: 'gemini',
    },
    {
      narrative:
        '你走进一座公园的深处，在一片树荫下意外撞见了一小片临时墓地：十几个隆起的土堆排列着，每一座前都插着一块粗糙的木牌，上面用刀或炭笔潦草地刻着名字，有的还歪歪扭扭写着一两句话。看得出，在这人命如草芥的末世里，曾有人不顾危险，认认真真地把同伴一个个安葬在了这里。几束早已枯萎发黑的野花仍倔强地摆在土堆前，无声诉说着生者的不舍。一股沉甸甸的压抑感攫住了你，连呼吸都变得艰难。',
      choices: [
        { text: '搜寻周边遗物', effects: { supplies: 5, sanity: -6 }, reaction: '你在一座座坟前翻拣可用之物，枯萎的野花被你拨到一旁——没人会拦你，可在逝者安息地搜刮的滋味，自己心里堵得慌。' },
        { text: '默默离开', effects: { supplies: -2 }, reaction: '你朝那片简陋的坟茔默默鞠了一躬，转身离去，什么也没动——这世道还肯对陌生人的安息地保留一分敬意的，不多了。' },
        { text: '整理墓地', effects: { supplies: -4, sanity: 4 }, reaction: '你扶正了歪倒的木牌，把枯花重新摆好。倘有逝者的同伴远远看见，定会红了眼眶——乱世里替素不相识的死者拢一拢坟，是天大的善意。' },
      ],
      summary: '临时墓园', art: '1avymq7', gen: 'gemini',
      once: true,
    },
    {
      narrative:
        '你推开一间自助洗衣店的玻璃门，一排排滚筒洗衣机静静停在原地，有几台的圆形舱门后还塞着洗到一半的衣物，早已捂得发臭，结成了硬块——它们的主人，怕是再也回不来取了。这画面荒诞得让人心头发紧，仿佛时间在某一刻被猛地掐断。你的目光扫过收银柜台，底下的小抽屉有被人撬开过的痕迹，零钱和杂物散落一地，看样子之前有人匆忙搜刮过，却又像是被什么打断，没能把所有东西都带走。',
      choices: [
        { text: '仔细翻找', effects: { supplies: 7 }, reaction: '你在散落的零钱杂物里细细翻拣，把前人匆忙间漏下的都收进了包——这份不嫌琐碎、连边角都不放过的细致，攒得下家底。' },
        { text: '检查后门', effects: { hp: -3, supplies: 5 }, reaction: '你留了个心眼去探后门，撞上守在那儿的东西也没慌——别人只顾着翻柜台，你却懂得先把退路和暗处摸清，老到。' },
        { text: '直接离开', effects: { supplies: -2 }, reaction: '你扫了眼这间被人匆忙搜过的洗衣店，没多停留便走了——稳是稳，可那点没被带走的东西，也随你的脚步留在了原地。' },
      ],
      summary: '洗衣店', art: '3pxst2', gen: 'gemini',
    },
    {
      narrative:
        '远处传来一阵断断续续的口哨声，奇怪的是，那旋律的节奏从头到尾一成不变，机械地、固执地循环重复着，听久了让人头皮发麻。你循着声音摸过去，来到一处烂尾的废弃工地，半成品的楼体上裸露着锈蚀的钢筋，高耸的脚手架在风中发出令人不安的「吱呀」晃动声。可那口哨声却像是从四面八方同时传来，你转了好几圈，始终无法确认它究竟出自哪个角落、是人是鬼。暮色正一点点吞没工地，把那些钢架的影子拉得又长又狰狞。',
      choices: [
        { text: '深入工地探查', effects: { supplies: 8, sanity: -5 }, reaction: '你迎着那机械循环的口哨声钻进锈钢林立的工地——明知诡异还敢深入虎穴一探究竟，这胆量让躲在暗处的人都不敢轻举妄动。' },
        { text: '撤回原路', effects: { supplies: -3 }, reaction: '那一成不变的口哨听得你头皮发麻，你果断原路退了回去——分得清哪是机会哪是陷阱，这份警觉比贪那点物资值钱。' },
        { text: '藏身等待', effects: { supplies: -5 }, reaction: '你躲进暗处守着，想等吹哨的人现身，口哨却始终从四面八方传来——既不敢进又不肯走，这一等，白白耗光了天光。' },
      ],
      summary: '重复口哨', art: '1l41vvu', gen: 'gemini',
      weight: 0.8,
    },
    {
      narrative:
        '一间照相馆的橱窗里，依旧整整齐齐地陈列着许多裱好的全家福——一张张笑脸定格在灾难降临前的某个幸福瞬间，如今全蒙上了一层灰白的尘埃，无人认领。这些再普通不过的合影，此刻看来却透着说不出的凄凉。你的目光落在后室那扇门上：它被人从里面用木板和钉子封得死死的，唯有门缝里，丝丝缕缕地渗出一股令人作呕的腐臭气味。封住这扇门的人，是想把什么东西关在里面，还是把什么东西，永远地隔绝在了外面？',
      choices: [
        { text: '撬开封板', effects: { supplies: 6, sanity: -5 }, reaction: '你硬撬开那扇被钉死的门，腐臭扑面而来也没让你退缩——别人见这阵仗早跑了，你却偏要看个究竟，胆识过人。' },
        { text: '收集可用材料', effects: { supplies: 3 }, reaction: '你只拆了些钉子木板就罢手，没去碰那扇渗着腐臭的封门——拿得不多，却也聪明地把里头的未知隔在了门后。' },
        { text: '离开照相馆', effects: { supplies: -2 }, reaction: '你看着满墙蒙尘的全家福，又瞥了眼那扇渗臭的封门，默默退了出去——那门后封着什么，你宁可一辈子不知道。' },
      ],
      summary: '旧照相馆', art: '1eju01d', gen: 'gemini',
    },
    {
      narrative:
        '街心一座古旧的钟楼竟还在运转，每隔一个小时，便会「铛——铛——」地敲响沉闷悠长的钟声，声波在空寂的街区里荡开，传出去老远。这本是旧世界残存的一丝秩序感，可你很快就毛骨悚然地发现，附近的感染者对这钟声格外敏感——每当钟响，它们便如被无形的丝线牵引着，纷纷停下脚步，转而踉踉跄跄地朝钟楼方向缓慢聚拢。继续在这片区域逗留，无异于把自己困在一个会定时召唤死亡的牢笼里，可周边或许也正因此被清空出搜刮的空当。',
      choices: [
        { text: '趁乱搜集周边', effects: { supplies: 9, hp: -4 }, reaction: '你掐着钟点，趁感染者被钟声引走的空当扫荡街区——能把这定时召唤死亡的牢笼反过来当成搜刮的时机，这脑子够活。' },
        { text: '迅速撤离', effects: { supplies: -2 }, reaction: '你看出这片区域会被钟声反复引来死亡，二话不说撤了——不在危险里多留一刻，这份当机立断，救了你一命。' },
        { text: '观察感染者动向', effects: { supplies: -4 }, reaction: '你蹲在远处摸钟声召唤的规律，想找出空当，却迟迟没敢动手——盘算得太久，机会和天光都从你眼皮底下溜走了。' },
      ],
      summary: '钟楼回响', art: '1lm8qsr', gen: 'gemini',
      minTurn: 26,
    },
    {
      narrative:
        '在一座加油站的后院，你发现了一个被水泥粗暴封死的地下入口，可水泥的边缘却有大片新近被凿开、挖掘过的痕迹，碎块还散落在四周，显然有人想重新打开它。入口附近的地上，丢着几个空空的罐头盒和一截截还没烧尽的烟头——这些都是新鲜的，说明就在不久前，还有人在这里逗留、忙碌过。被封死又被重新挖开的地下空间，往往藏着原主人最珍视、最不愿示人的东西，这强烈的诱惑背后，也潜藏着同样强烈的危险。',
      choices: [
        { text: '挖开入口', effects: { supplies: 11, hp: -5 }, reaction: '你接着别人凿到一半的痕迹奋力挖开了封口，抢出了原主人藏得最深的家底——敢虎口夺食、把别人的心血据为己有，这胆魄不小。' },
        { text: '留下记号', effects: { supplies: -3 }, reaction: '你没贸然下手，只在附近留了暗记打算日后再来——眼下烟头还热，稳住贪念先避其锋芒，这份冷静比莽撞高明。' },
        { text: '避开此地', effects: { supplies: -2 }, reaction: '你看着那些新鲜的烟头和罐头，掂出了背后的凶险，默默绕开了——舍得放下唾手可得的诱惑，才不会栽在别人的窝里。' },
      ],
      summary: '封死入口', art: '1d0fls9', gen: 'gemini',
    },
    {
      narrative:
        '清晨的浓雾把整座城裹得严严实实，能见度不足十米，一切都浸在乳白色的、湿冷的死寂里。你摸索着走到一处路口，眼前的景象却让你猛地停住了脚步：数十辆共享单车被人整整齐齐地堆叠、拼接成了一个巨大而规则的几何形状，像某种祭坛，又像某种警告。阵列的正中央，一面残破的灰布挂在最高处，随着雾里若有若无的气流，无声地缓缓晃动着。这分明是人为的造物，可那刻意的、不知所云的诡异，却透出一股说不清道不明的森然寒意。',
      choices: [
        { text: '进入中心查看', effects: { supplies: 7, sanity: -4 }, reaction: '你拨开浓雾，一步步走进那座诡异祭坛的正中央——明知这是人为的警告还敢往里探，这股不信邪的胆子，看得人脊背发凉。' },
        { text: '绕路前进', effects: { supplies: -3 }, reaction: '你看出那单车阵列透着说不清的森然，索性绕道而行——读得懂别人留下的警告并避其锋芒，这份直觉，常常就是活路。' },
        { text: '推倒障碍', effects: { hp: -3, supplies: 2 }, reaction: '你不管那阵列是祭坛还是警告，上手就把单车推倒开道——这股不被诡异唬住、偏要破而后立的莽劲，倒也透着股狠。' },
      ],
      summary: '单车堆阵', art: '1buxtp7', gen: 'gemini',
    },
    {
      narrative:
        '路过市图书馆，你发现这座知识的殿堂有一部分被大火烧毁了，残存的梁柱熏得焦黑，空气里至今还飘着纸张燃烧后特有的呛人焦糊味。所幸的是，地势较低的地下阅览室似乎逃过一劫，依旧保存完整。一排排高大厚重的书架在那片昏暗里林立着，既能成为遮蔽身形、抵御外敌的天然屏障，让你得以喘息，可那纵横交错、宛如迷宫的狭窄过道，同样也可能把你死死困住，让你与潜伏在书架阴影里的未知存在狭路相逢，无处可逃。',
      choices: [
        { text: '进入地下室', effects: { supplies: 8, sanity: -3 }, reaction: '你钻进书架如迷宫般纵横的地下阅览室，在黑暗里摸索前行——敢往这种进退两难的窄道里探的人，心里得有底气才行。' },
        { text: '搜集地面物资', effects: { supplies: 3 }, reaction: '你只在熏黑的地面层拣了点东西，没钻进那座书架迷宫——拿得不多，却也聪明地没把自己困进可能出不来的窄道里。' },
        { text: '离开图书馆', effects: { supplies: -2 }, reaction: '你望着那片焦黑的废墟和黑沉沉的地下室，摇头退了出去——稳是稳，可那一排排书架后藏着的东西，从此与你无缘。' },
      ],
      summary: '烧毁书馆', art: '1g6xxvd', gen: 'gemini',
      requires: 'sanity>=50',
    },
    {
      narrative:
        '一栋楼的天台上，竟奇迹般地保留下了一小片菜园——尽管大半已经枯死，但缠绕的藤蔓间，仍能看见几颗顽强挂着的青菜和番茄，那一点鲜活的绿意，在满目疮痍的灰败里刺得人眼睛发热。这是有人曾在绝境里用心耕耘过的痕迹。可那只浇水的水箱早已干涸见了底，更让你心头一紧的是，通往天台的楼道里，墙上、地上散布着激烈搏斗后留下的血迹与凌乱痕迹——这片小小菜园的主人，如今是生是死，又遭遇了什么，没有人知道。',
      choices: [
        { text: '采集剩余作物', effects: { supplies: 10 }, reaction: '你只摘走了藤蔓上顽强挂着的菜果，没去深究主人的下落——拿得干脆，却也没贪心去碰那条血迹斑斑的楼道，稳。' },
        { text: '彻底搜查楼层', effects: { supplies: 13, hp: -5 }, reaction: '你顺着血迹把整层楼翻了个遍，连凶险的角落都没放过——别人只敢摘几棵菜，你却敢追着搏斗的痕迹深挖，胆大心也细。' },
        { text: '立即撤离', effects: { supplies: -2 }, reaction: '你瞥见楼道里那些激烈搏斗的血痕，没敢久留，采都没采便撤了——这份见势不妙就走的警觉，把未知的凶险挡在了身后。' },
      ],
      summary: '天台菜园', art: '1ch7c5j', gen: 'gemini',
    },
    {
      narrative:
        '走进一座公交总站，你在站台一隅发现了几顶废弃的帐篷，松垮垮地塌在地上。周围散落着一副下到一半的象棋、几个空了的水瓶，还有用粉笔在地上画的格子游戏——种种迹象都在诉说着，这里曾经聚集过不少幸存者，他们也曾在这避难所里苦中作乐，试图维系一点正常生活的模样。可如今，人去棚空，唯有穿堂的风掀动着破损的塑料布，发出「哗啦哗啦」的声响，那空荡荡的凄凉，比满地的尸骸更让人心里发堵。',
      choices: [
        { text: '翻找遗留物资', effects: { supplies: 7 }, reaction: '你在塌陷的帐篷和散落的杂物里仔细翻拣，把前人留下的好东西收进了包——这份不浪费废墟里每一分余粮的精打细算，实在。' },
        { text: '检查周围痕迹', effects: { supplies: 4, sanity: -2 }, reaction: '你蹲下来研究那盘下到一半的残棋和散落的痕迹，想弄清这里人去棚空的缘由——比起急着搜刮，先摸清来路，这份谨慎老到。' },
        { text: '继续赶路', effects: { supplies: -2 }, reaction: '你看着这片人去棚空的营地，被那股凄凉勾得心里发堵，没多停留便走了——稳是稳，可遗下的物资也随风飘远了。' },
      ],
      summary: '废弃营地', art: '1dcydmk', gen: 'gemini',
    },
    {
      narrative:
        '你推门走进一家钟表维修铺，迎面而来的不是死寂，而是一片杂乱无章的「滴答」声——墙上、柜里、工作台上，挂着、摆着数十只大小不一的钟表，竟还有不少在走动，可它们彼此之间快慢不一、毫无章法，无数指针错乱地走着，发出此起彼伏、永不同步的声响。在这片诡异的声浪里待得久了，你开始头晕目眩，恍惚间竟分不清此刻究竟是哪一天、哪一刻，仿佛时间本身在这里碎裂、错乱，唯有窗外的废墟提醒着你，世界从未停止崩塌。',
      choices: [
        { text: '搜索柜台', effects: { supplies: 5, sanity: -4 }, reaction: '你顶着满室错乱的滴答声头晕目眩，仍硬撑着把柜台翻了个遍——能在这扰乱心神的声浪里稳住手脚搜刮，定力不一般。' },
        { text: '稍作休息', effects: { supplies: -4, sanity: 2 }, reaction: '你在那片永不同步的钟声里闭眼歇了片刻，反倒奇异地松弛下来——能把诡异之地当成喘息的港湾，这份心宽也是本事。' },
        { text: '立刻离开', effects: { supplies: -2 }, reaction: '那满屋错乱的指针搅得你分不清时日，你赶紧抽身退了出去——不在乱人心神的地方多留，这份清醒，守住了你的脑子。' },
      ],
      summary: '钟表铺', art: '3wibae', gen: 'gemini',
    },
    {
      narrative:
        '一栋学生宿舍楼外，那面曾经贴满社团海报与通知的公告栏，如今密密麻麻覆满了手写的纸条。你凑近细看，鼻头不由得一酸：有的是寻人启事，歪歪扭扭写着失散亲人的名字与特征，附着「看到请联系」的恳求；有的则是诀别的遗言，字迹潦草，墨迹被泪水或雨水晕开。这些纸张大多已经泛黄卷边，在风里轻轻颤动，可透过那一笔一划，你仍能真切地触摸到落笔之人当时的绝望、不舍与最后一线渺茫的期盼，沉重得让人喘不过气。',
      choices: [
        { text: '仔细阅读', effects: { sanity: -6, supplies: -3 }, reaction: '你一张张读着那些寻人启事与诀别遗言，鼻头发酸——肯停下脚步，替那些素不相识的绝望驻足片刻的人，心还没在末世里硬透。' },
        { text: '寻找遗留房间', effects: { supplies: 8 }, reaction: '你照着纸条上的线索摸进对应的宿舍翻找，果然有所斩获——能从满墙悲怆里冷静读出有用的信息，这份务实救了你的命。' },
        { text: '默默离去', effects: { supplies: -2 }, reaction: '你不忍细看那满墙颤动的纸条，默默转身离开——没多停留，可那些落笔人的期盼与绝望，还是悄悄压在了你心上。' },
      ],
      summary: '寻人纸条', art: '1cvrhf7', gen: 'gemini',
      once: true,
    },
    {
      narrative:
        '远处隐隐传来一阵断断续续的犬吠，在这万籁俱寂、活物近乎绝迹的死城里，这声音格外突兀。你循着声音一路找过去，来到一座仓库门前。那扇厚重的铁门内侧，布满了密密麻麻、深可见底的抓痕，门边还摆着一个磕碰得变形的金属食盆——种种痕迹都表明，这里曾有人悉心照料过一只动物，把它当作了乱世里相依为命的伙伴。可隔着这道紧闭的铁门，你既听见了那似哭似嚎的呜咽，却也无从知晓里头那只生灵，如今究竟变成了什么模样。',
      choices: [
        { text: '打开仓库门', effects: { supplies: 7, hp: -4 }, reaction: '你拉开布满抓痕的铁门，门后扑出的东西被你挡了下来——明知里头那只生灵已不知变成何样，还敢开门一探，胆识够硬。' },
        { text: '留下食物离开', effects: { supplies: -5, sanity: 3 }, reaction: '你隔着门把食物推进缝里才走——既没拿命去赌门后的未知，又不忍让那似哭似嚎的生灵饿着，这份心软中带着分寸。' },
        { text: '忽略声音', effects: { supplies: -2 }, reaction: '你听着那似哭似嚎的呜咽，狠下心快步走开了。没人评判你保命，可那道铁门后的生灵成了什么，再没人知道。' },
      ],
      summary: '仓库犬吠', art: '1ay9o9j', gen: 'gemini',
    },
    {
      narrative:
        '黄昏时分，你路过一座立交桥底，瞥见桥墩下有一小堆被火烧过的灰烬。走近一看，那竟是一摞被刻意焚毁的身份证件和笔记本——烧焦卷曲的塑料卡片上还能依稀辨认出半张照片，烧剩的纸页边角写满了字。有人在这里，决绝地、一点不留地抹去了自己过往的一切身份与痕迹，仿佛要与那个旧的自己、旧的世界彻底割裂。究竟是怎样的遭遇，才会让一个人狠下心销毁自己存在过的证明？那焦黑的纸屑在穿堂风里打着旋翻飞，让你莫名地从心底升起一股寒意。',
      choices: [
        { text: '翻找残留物', effects: { supplies: 5, sanity: -5 }, reaction: '你扒开那堆焦黑的灰烬翻找，指尖沾满黑灰也没停手——能从一个人决绝抹去的过往里冷静捡拾可用之物，这份心硬叫人发怵。' },
        { text: '原地休整', effects: { supplies: -4 }, reaction: '你在那堆焚毁证件旁坐下歇脚，任焦黑的纸屑打旋。没人扰你，可那个销毁一切痕迹的人究竟遭遇了什么，成了挥不去的寒意。' },
        { text: '离开现场', effects: { supplies: -2 }, reaction: '你被那股决绝割裂过往的气息激得发寒，没多停留便离开了——不去深究别人的伤疤，这份分寸，也是一种体面。' },
      ],
      summary: '焚毁证件', art: '1g8bx9z', gen: 'gemini',
    },
    {
      narrative:
        '走进一条商业步行街，你发现某家店铺整面的镜墙竟奇迹般地完好无损，一尘不染。空旷死寂的街道在镜中无限延伸、重叠，映出无数个自己和无数条荒废的长街，那景象诡异得让人心慌。更要命的是，在这昏暗摇曳的光线下，你的眼角余光总会捕捉到镜中似乎有别的人影在身后一闪而过，可猛地回头，身后却空无一物。一次、两次、三次……那挥之不去的「有人跟踪」的错觉，像藤蔓一样缠上你的神经，你不知道那究竟是镜子的把戏，还是你濒临极限的精神出了岔子。',
      choices: [
        { text: '靠近检查', effects: { sanity: -7, supplies: 3 }, reaction: '你逼近那面诡异的镜墙，想看清身后一闪而过的人影——明知精神已濒临极限还偏要自证清醒，这股较真劲，把自己推到了崩溃边缘。' },
        { text: '遮挡镜面', effects: { supplies: -4 }, reaction: '你扯下布条把镜面一块块盖住，断了那挥之不去的错觉——懂得在快撑不住时主动掐掉幻象的源头，这份自救的清醒难得。' },
        { text: '快速通过', effects: { supplies: -2 }, reaction: '你别开眼，不去理会镜中那些一闪而过的人影，快步穿了过去——不和将崩的神经较劲，这份果断，守住了你的理智。' },
      ],
      summary: '镜墙倒影', art: '1lng8vx', gen: 'gemini',
    },
    {
      narrative:
        '你沿着高架桥下方探路，撞见一条半淹在水里的地下步行通道。浑浊发臭的积水没过了膝盖，水面上漂浮着各色杂物和油污，一股呛人的腐败气息直冲鼻腔，让人忍不住作呕。借着尽头透进的一线天光，你隐约看见通道深处的高台上，搁着几个还没被水泡坏的储物箱，那是切实可见的物资。可脚下的积水黑得看不见底，深浅难测，更让你脊背发凉的是，黑暗的深处偶尔会传来一两声极轻的「啪嗒」拍水声——那不是你弄出的动静，水里，还有别的东西。',
      choices: [
        { text: '摸黑涉水检查', effects: { supplies: 10, hp: -6, sanity: -2 }, reaction: '你蹚进齐膝的黑水朝高台摸去，水里那声拍打也没让你回头——敢往看不见底、还藏着活物的浊水里走的，胆子练到了头。' },
        { text: '绕路离开', effects: { supplies: -5 }, reaction: '你听着水里那声不属于你的拍打，毫不犹豫绕道而行——分得清哪些物资碰不得，这份壮士断腕的果决，救了你的命。' },
        { text: '只捞近处漂物', effects: { supplies: 2, sanity: -3 }, reaction: '你只在水边捞了几样漂近的杂物便收手，没敢往深处趟——拿得不多，却也聪明地没把腿伸进黑暗里那东西的口边。' },
      ],
      summary: '积水隧道', art: '1hkuhia', gen: 'gemini',
    },
    {
      narrative:
        '一座加油站的便利店看着还算完整，货架上多半还有存货，这让饥肠辘辘的你眼前一亮。可你刚要迈步进门，一个独眼的精瘦男人就端着一把猎枪先你一步堵在了门口，枪口直直对准你，沙哑着嗓子吼道：「站住！这是我的地盘，识相的就滚！」可你眼尖地注意到，他握枪的手在微微发抖，眼窝深陷、嘴唇发白——这副模样分明也是个弹尽粮绝、强撑着虚张声势的可怜人。横在你们之间的，是一店的物资和两条都快走到头的命。',
      choices: [
        { text: '亮出物资提议交换，和平共处', effects: { supplies: -8, sanity: 6 }, reaction: '独眼男人握枪的手慢慢松了，眼里的凶光化成动容：「这年头……还有人肯讲个换字。」两个走投无路的人，竟谈成了一桩活路。' },
        { text: '佯装退走，绕到后窗潜入', effects: { supplies: 5, hp: -6, sanity: -4 }, reaction: '独眼男人守着前门虚张声势，压根没料到你会从后窗摸进来——等他反应过来，你早已得手抽身，被他啐了句：「狡猾的家伙！」' },
        { text: '硬碰硬，赌他不敢开枪', effects: { hp: -16, supplies: 8, sanity: -8 }, reaction: '你看穿了他色厉内荏，真就硬闯过去。独眼男人握枪的手抖到最后也没扣下扳机，眼里又惊又惧——你赌赢了他那点虚张的胆。' },
      ],
      summary: '加油站对峙', art: '80asbt', gen: 'gemini',
      weight: 1.2,
    },
    {
      narrative:
        '一座高耸的信号塔孤零零地矗立在城郊的山丘上，钢架直插灰蒙蒙的天际。你眯眼望去，塔顶似乎缠着一面褪色的布条，在风里招展，像是某个幸存者留下的求救信号，又像是指引方向的路标。爬上塔顶，整座城市的全貌、感染者聚集的区域、乃至通往安全地带的路线，都将一览无余，这对在废墟里盲目摸索的你来说太重要了。可那条通往塔顶的爬梯锈迹斑斑，不少踏脚的横档已经断裂、变形，在风中摇摇欲坠——每往上爬一步，都是在拿命赌。',
      choices: [
        { text: '攀塔登顶，俯瞰寻路', effects: { hp: -10, supplies: -4, sanity: 10 }, reaction: '你攀着摇摇欲坠的锈梯一步步登顶，脚下横档不时断裂坠落。若有人远远望见这一幕，定会倒抽冷气——这是把命押在风里的搏法。' },
        { text: '绕塔搜寻基站机房', effects: { supplies: 6, sanity: -3 }, reaction: '你没去赌那道要命的爬梯，转而摸进塔下的基站机房翻找——舍得放下登顶的诱惑、稳稳取实利，这份不逞强的算计聪明。' },
      ],
      summary: '信号塔', art: '3ktq9d', gen: 'gemini',
      minTurn: 6,
    },
    {
      narrative:
        '你的储水彻底告罄了，背包里只剩下半瓶前几天接来的、浑浊得泛黄的雨水，沉淀着细碎的杂质。嗓子干得几乎要冒出火来，每一次吞咽都像在吞咽砂纸。前方那个小区里的消防水箱，或许还封存着没漏光的存水，那是你眼下唯一的指望。可你扒开箱盖凑近一闻，一股浓烈刺鼻的铁锈味便扑面而来，水面上浮着一层可疑的暗红色锈垢——这水喝下去，是解一时的燃眉之急，还是给本就脆弱的肠胃再补上致命一刀，全是未知数。',
      choices: [
        { text: '冒险饮用，先解燃眉之急', effects: { hp: -6, supplies: 6, sanity: -2 }, reaction: '你捏着鼻子灌下那口锈味刺鼻的水。没人拦你，可弄坏了肠胃才来后悔的滋味，这片缺医少药的废土向来不留情面。' },
        { text: '取水煮沸，多耗些时间', effects: { supplies: 5, hp: 4, sanity: 1 }, reaction: '你忍着干渴架火把水煮开了才喝——明明渴到嗓子冒烟还沉得住气，这份不拿肠胃赌命的耐心，是老练幸存者的智慧。' },
      ],
      summary: '净水危机', art: '1bgehc4', gen: 'gemini',
      requires: 'supplies<=20',
      weight: 1.4,
    },
    {
      narrative:
        '一座废弃工厂阴暗的角落里，蹲着一台落满灰尘的柴油发电机，外壳锈迹斑斑，看着像是被遗忘了许久。你抱着一丝希望拧开油箱盖晃了晃，里头竟还荡着小半箱柴油！心头顿时一热——若是能把它发动起来，至少今夜你就能拥有久违的光明和暖意，不必再蜷在黑暗与寒冷里瑟瑟发抖。可这机器一旦轰鸣运转起来，那巨大的噪声会像晚餐铃一样，把方圆几条街的感染者统统招到这工厂来。光明与温暖，是用暴露行踪的巨大风险换来的。',
      choices: [
        { text: '发动取暖，赌一夜安稳', effects: { hp: 6, supplies: -6, sanity: 6 }, reaction: '机器轰鸣里，久违的光与暖裹住了你。这赌注引来的危险被你提前防住，若有同伴在旁，会就着这点光暖，头一回睡得安稳。' },
        { text: '抽走柴油另作他用', effects: { supplies: 8, sanity: -2 }, reaction: '你忍着寒意没去发动那台等于报警器的机器，只悄悄抽走了柴油——舍得放弃一夜温暖换长久的安全，这份克制不简单。' },
      ],
      summary: '废弃发电机', art: '1tz6idj', gen: 'gemini',
    },
    {
      narrative:
        '一个背着白色医药箱的女人拦住了你的去路，她神色焦急却努力维持着镇定，自称是医院里跑出来的护士。她说看你身上有伤，她能帮你妥善处理、上药包扎，可作为交换，她恳求你护送她穿过前面两个街区——她那走散的小女儿，最后被人看见就在那个方向。她说话时眼眶泛红，紧紧攥着医药箱的指节都泛了白。专业的救治对伤痕累累的你诚然是救命稻草，可那两个街区一路上凶险莫测，这趟护送，谁也不知道会搭上什么代价。',
      choices: [
        { text: '答应护送，结伴而行', effects: { hp: 8, supplies: -8, sanity: 6 }, reaction: '护士眼眶一热，郑重地替你处理好伤口：「我没看错人。」往后两个街区，她替你包扎，你替她挡刀——这份并肩，抵得过千言万语。' },
        { text: '只换包扎，不愿冒险', effects: { hp: 5, supplies: -4, sanity: -3 }, reaction: '护士默默替你上完药，没再勉强，只是望向那两个街区的眼神黯了下去——你没欠她什么，可那点失望还是扎了你一下。' },
        { text: '怀疑是陷阱，转身离开', effects: { supplies: -3, sanity: -5 }, reaction: '护士攥着医药箱怔在原地，看你头也不回地走远，轻声道：「连帮忙都怕了吗……」这世道把人逼成这样，谁也怪不得。' },
      ],
      summary: '路遇护士', art: '1ks40tm', gen: 'gemini',
      weight: 1.1,
    },
    {
      narrative:
        '一座大型商场的卷帘门后头传来一阵激烈的嘈杂与争吵——一群幸存者据守在商场内，似乎正为分配仅剩的最后一批食物而起了内讧，骂声、推搡声、东西摔碎的声音乱成一团。你贴门细听之际，门缝里忽然递出一道警惕的目光和一句压低的话：「想进来可以，先把你身上的家伙都交出来，我们才敢放你进。」交出武器，等于把自己的命门交到一群素不相识、还在窝里斗的人手上；可商场厚重的防御和成群的同伴，又是孤身在外的你梦寐以求的庇护。',
      choices: [
        { text: '交械入伙，抱团取暖', effects: { supplies: 6, sanity: 4, hp: -2 }, reaction: '门内那道警惕的目光见你坦然交出武器，迟疑片刻还是放了你进去——肯把命门交给一群陌生人，这份豁出去的信任也是种胆量。' },
        { text: '趁乱不入，远远绕开', effects: { supplies: -5, sanity: -3 }, reaction: '你听着门内为最后一口吃食吵得不可开交，摇头退开了——分得清哪些抱团是取暖、哪些是火坑，这份冷眼，救了你的命。' },
      ],
      summary: '商场困守', art: '1bw8h8d', gen: 'gemini',
      minTurn: 5,
    },
    {
      narrative:
        '连日的辗转奔逃让你几乎没能合眼，长久的缺觉与高度紧绷彻底打乱了你的生物钟，你开始分不清窗外究竟是白天还是黑夜，时间在你眼里糊成了一片黏稠的灰。耳边总有人在贴着你低声呢喃，一遍遍唤着你的名字，那声音真切得就在耳后，可你猛地回过头，身后永远只有空荡荡的废墟和你自己粗重的喘息。理智这根弦已经被绷到了极限，随时可能「啪」地断掉——你能清晰地感觉到，自己正一步步滑向某个再也回不来的深渊边缘。',
      choices: [
        { text: '掐着秒表强迫自己规律作息', effects: { sanity: 12, supplies: -5, hp: -2 }, reaction: '没人见证你掐着秒表跟濒临断裂的神经死磕，可那份在崩溃边缘还肯给自己立规矩的狠劲，是从悬崖边把人拽回来的硬本事。' },
        { text: '不管不顾继续赶路', effects: { hp: -8, sanity: -8, supplies: -4 }, reaction: '你顶着糊成一团的昼夜硬往前挪，耳边的呢喃越来越响——那根绷到极限的弦，正一点点滑向再也回不来的深渊。' },
      ],
      summary: '分不清昼夜的幻听', art: 'mxmtgg', gen: 'gemini',
      requires: 'sanity<=20',
      weight: 1.8,
    },
    {
      narrative:
        '夜色里，河对岸那栋高楼的某一扇窗户引起了你的注意——它正以一种极有规律的节奏，一明一灭，明、灭、明灭明……那分明不是偶然，倒像是有人在用灯光打着摩斯电码，绝望地向外界发送求救信号。你的心猛地揪紧了，那或许是另一个和你一样苦苦支撑的同类，在向你伸出最后的手。可你也清楚，这世道里，最毒的陷阱往往就裹着最动人的求救外衣。渡过那条又宽又急的河需要耗费大量的时间和体力，去，还是不去，是一场豪赌。',
      choices: [
        { text: '扎筏渡河，循光而去', effects: { hp: -12, supplies: -8, sanity: 8 }, reaction: '你扎起破筏拼命横渡急流朝那束求救光划去——肯为一个素未谋面的同类拿命去赌一条宽河，这份赤诚，茫茫死城里寥寥无几。' },
        { text: '隔岸观察一夜再定', effects: { supplies: -6, sanity: -2 }, reaction: '你蹲在岸边盯着那明灭的灯守了整夜，没急着冒进——这世道最毒的陷阱常裹着求救的外衣，先看清再说，这份沉稳救命。' },
      ],
      summary: '对岸的灯', art: '1cy045l', gen: 'gemini',
      minTurn: 10,
    },
    {
      narrative:
        '在地下停车场避难的间隙，你盘算着该给自己弄件趁手的兵器了——光靠这根撬棍，对上感染者实在太过被动。你捡来一根细长结实的钢管，又找了块磨利的废铁片，靠着撬棍当工具，叮叮当当地敲打、捆绑、固定，硬是拼凑出了一柄简陋却堪用的长矛。掂着这件勉强成形的家伙，矛尖虽不算锋利，可那一臂的攻击距离，让你悬了许久的心总算落下几分，胆气也跟着壮了起来。是花上半天工夫把它打磨得更顺手，还是凑合着先用上路，你得权衡这宝贵的时间。',
      choices: [
        { text: '花半天精心打磨', effects: { hp: -2, supplies: -6, sanity: 8 }, reaction: '你耐着性子把矛尖磨得寒光凛凛。没人在旁，可这份肯沉下心打磨家伙的耐性，往后每一次对敌都会替你说话——好刀，养命。' },
        { text: '凑合能用就上路', effects: { supplies: -3, sanity: 2 }, reaction: '你掂了掂这柄半成品长矛，觉得能用就够了，扛上便走——求快是求快，只盼真到了拼命那刻，这凑合的家伙别掉链子。' },
      ],
      summary: '自制长矛', art: '1io8gm7', gen: 'gemini',
      requiresItem: '撬棍',
      once: true,
      itemsGained: ['简陋长矛'],
    },
    {
      narrative:
        '电台里那个公事公办的声音反反复复播报着同一条消息：城郊设有临时疫苗接种点，凡持有健康证明者，均可前往登记并安排撤离。这消息的真假实在难以分辨，传闻里那种官方机构特有的流程感，既让人觉得郑重可信，又让人嗅出一丝可疑的诱饵味道。可无论如何，这是你陷落至今听到过的、最具体、最像样的一线希望——有明确的地点，有撤离的承诺。是立刻动身奔赴那渺茫的生路，还是先按捺住激动，多方打探虚实再决定，关乎你接下来每一步的方向。',
      choices: [
        { text: '动身前往接种点', effects: { supplies: -8, sanity: 10 }, reaction: '你攥紧了那条渺茫的消息，毅然上路。若有同伴在旁，会被你这股抓住一线希望就敢押上一切的决断点燃，跟着燃起斗志。' },
        { text: '先打探虚实，不贸然行动', effects: { supplies: -4, sanity: -2 }, reaction: '你按捺住激动，先多方探听接种点的虚实——这年头官腔越足越可疑，不被一纸承诺冲昏头脑，这份冷静往往就是活路。' },
      ],
      summary: '疫苗传闻', art: '1h0vpj6', gen: 'gemini',
      once: true,
      minTurn: 12,
    },
    {
      narrative:
        '一架军用直升机毫无征兆地从低空轰鸣着掠过头顶，巨大的螺旋桨气浪掀得地面尘土飞扬，那震耳欲聋的声响震得你心头发颤，几乎要喜极而泣——这是文明世界还没彻底抛弃这里的铁证！可它并没有停留，只在不远处的空地上抛下了几个挂着降落伞的补给箱，便头也不回地飞远了。空投箱落地的轰然巨响，瞬间像捅了马蜂窝，四面八方的废墟里、街角处，无数道人影骤然涌动起来，朝着补给箱疯狂奔去——一场你死我活的争夺，已经无可避免。',
      choices: [
        { text: '拼速度抢一个补给箱', effects: { hp: -10, supplies: 18, sanity: -4 }, reaction: '你从一片疯抢的人潮里杀出血路抢到一箱，旁人眼睁睁看你抱箱冲出重围，又是眼红又是忌惮——这股拼命的速度没人追得上。' },
        { text: '尾随取箱的人，伺机捡漏', effects: { supplies: 6, hp: -4, sanity: -3 }, reaction: '你没去挤那要命的人堆，而是悄悄缀在抢到箱子的人身后伺机捡漏——避开锋芒、坐收渔利，这份不显山露水的精明老到。' },
      ],
      summary: '空投补给', art: '1hm5dv6', gen: 'gemini',
      keyMoment: true,
      minTurn: 3,
      weight: 1.2,
    },
    {
      narrative:
        '和你出生入死、同行了许多天的伙伴，昨夜在掩护你撤退时被感染者咬伤了胳膊。今晨，你颤抖着掀开他的眼皮——那双眼睛的眼白里，已经爬满了浑浊死灰的纹路，皮肤也开始泛起青黑。转化只是时间问题了。可此刻的他还认得你，还能说话，他抓着你的手，气息微弱却异常清醒地哀求：「求你……别让我变成那种东西……到处咬人……拜托了，趁我还是我。」他眼里没有恐惧，只有恳求与托付，把这个残忍至极的决定，交到了你手上。',
      choices: [
        { text: '含泪了结，留他最后的体面', effects: { sanity: -14, hp: 2, supplies: 4 }, reaction: '他在你怀里咽气前，用尽最后力气挤出一个释然的笑：「谢谢……是你，我才敢闭眼。」这份成全比千言万语都重，也刺得你心口生疼。' },
        { text: '把他锁进房间，独自离开', effects: { sanity: -8, supplies: -4 }, reaction: '你反锁上门，身后传来他渐渐变调的、不再像人的低吼。你没能下手，也没能救他——这份逃避，会在往后的夜里一遍遍审判你。' },
      ],
      summary: '伙伴感染', art: '1ax4oas', gen: 'gemini',
      once: true,
      keyMoment: true,
      requires: 'sanity>=30',
      weight: 1.3,
    },
    {
      narrative:
        '你撬开一所小学的储藏室，眼前的景象让你瞳孔骤缩、呼吸都急促起来——成箱成箱的瓶装矿泉水整齐地码到了天花板，旁边堆着一摞摞军训时配发的真空包装干粮，足够一个人吃上大半个月。这是你陷落以来见过的，最丰厚、最唾手可得的一笔物资，简直像做梦一样。可就在你心跳加速、几乎要冲上去的瞬间，储藏室外那条空荡荡的走廊尽头，传来了一阵缓慢的、沉重的拖行声，「嚓……嚓……」一步步，正不紧不慢地朝这边挪来。',
      choices: [
        { text: '迅速搬运，能拿多少是多少', effects: { supplies: 20, hp: -6, sanity: -4 }, reaction: '拖行声逼近的当口，你抱起箱子就往外冲，擦着那东西的指缝逃了出来——这份贪到极致又快到极致的胆量，赌赢了一笔横财。' },
        { text: '设好退路再分批搬', effects: { supplies: 12, sanity: 2 }, reaction: '你先摸清退路才分批往外搬，没被满屋物资冲昏头——明明唾手可得也不贪命，这份临危不乱的盘算，是老手才有的稳。' },
      ],
      summary: '学校储藏室', art: '1f665tz', gen: 'gemini',
      itemsGained: ['军训干粮'],
    },
    {
      narrative:
        '一处隐秘的黑市，设在废弃地铁站幽深的站台上，几十支蜡烛在风里明灭摇曳，把人影投在贴满瓷砖的墙上，幢幢晃动，透着一股末世特有的诡谲。蒙面的商人们盘踞在摊位后，用物资交换情报，也交换着更要命的东西——你的目光被一个摊位牢牢吸住：一把上了膛的真枪，正在那里公开叫价。在这步步杀机的世道里，一把能保命、能威慑的火器，价值连城；可它的标价同样高得吓人，几乎要掏空你赖以活命的全部家当。换，还是不换，得想清楚。',
      choices: [
        { text: '倾尽物资换那把枪', effects: { supplies: -24, sanity: 8, hp: 4 }, reaction: '蒙面商人接过你掏空家底换来的物资，把枪推过来时多看了你一眼——肯倾家荡产去搏一份保命的威慑，这魄力他见得不多。' },
        { text: '只换一张安全区地图', effects: {}, outcomes: [
          { weight: 1, effects: { supplies: -10, sanity: 6 }, itemsGained: ['安全区地图'], reaction: '你绕过那把诱人的枪，只换了张安全区地图。蒙面商人意味深长地点点头——比起一时的火力，看清出路的人，往往才走得到头。' },
        ] },
        { text: '什么都不换，速速离开', effects: { supplies: -3, sanity: -2 }, reaction: '你在步步杀机的烛影里没敢久留，空手退了出去——稳是稳，可那把能保命的枪和那张地图，都随摇曳的烛火留在了身后。' },
      ],
      summary: '站台黑市', art: '1hluxlt', gen: 'gemini',
      minTurn: 7,
    },
    {
      narrative:
        '一场突如其来的暴风雪把天地搅成了白茫茫的一片，鹅毛大雪夹着刺骨的寒风疯狂抽打在脸上，能见度骤降到只剩几米，每一步都得深一脚浅一脚地从齐膝深的积雪里艰难拔出来。就在你几乎要被这风雪困住、辨不清方向时，雪地上一串还没被新雪填平的、清晰的新鲜脚印闯入了你的视线——是人留下的。这意味着附近就有活物：或许是个能搭伙取暖、互通有无的同类，给你绝境里添一分指望；可也可能是危险的掠夺者，甚至是别的什么，正埋伏在这白色的死寂里。',
      choices: [
        { text: '循脚印追踪，或有同伴', effects: { hp: -8, supplies: -6, sanity: 4 }, reaction: '你顶着暴雪深一脚浅一脚地追着那串脚印——明知前头可能是掠夺者，还肯为一线同伴的指望去闯白色死寂，这胆色不一般。' },
        { text: '反向避开，独自寻路', effects: { hp: -6, supplies: -5, sanity: -4 }, reaction: '你警觉地反向避开那串新鲜脚印，独自在风雪里另寻活路——分得清雪地里的脚印是机缘还是杀机，这份谨慎稳妥。' },
      ],
      summary: '雪地脚印', art: '1lvz6zt', gen: 'gemini',
      weight: 1.1,
    },
    {
      narrative:
        '又一次，你蜷缩在一座加油站便利店的角落里躲避风头，目光死死锁住了货架上那唯一的一罐午餐肉。你伸手取下来，借着微光看清了罐底的生产日期——它已经过期整整三年了，罐身甚至有一处微微鼓胀。理智在你脑海里疯狂拉响警报：过期且胀罐的肉制品，吃下去极可能引发严重的食物中毒，在这缺医少药的世道，那几乎是九死一生。可你的胃此刻正绞成一团，发出尖锐的、压倒一切的抗议，那股饥饿带来的眩晕，让你几乎要把警告抛到脑后。',
      choices: [
        { text: '吃下去，赌肠胃扛得住', effects: { hp: -4, supplies: 6, sanity: 2 }, reaction: '你撕开胀罐就着饥饿狼吞虎咽。没人拦你，可拿肠胃赌一罐胀气的过期肉，在这缺医少药的世道，本就是九死一生的豪赌。' },
        { text: '忍住，继续找更安全的食物', effects: { hp: -6, supplies: -2, sanity: -4 }, reaction: '你盯着那罐胀气的午餐肉咽了咽口水，终究把它放了回去——饿到发昏还能压住嘴，这份不拿命冒险的清醒，救过太多人。' },
      ],
      summary: '过期罐头', art: '1kviurz', gen: 'gemini',
      requires: 'supplies<=20',
    },
    {
      narrative:
        '一座残破的教堂里，竟聚集着十几个幸存者，烛光摇曳中，一个自封「先知」的男人站在讲台上，挥舞着手臂高声布道。他声嘶力竭地宣称，这场末日是上天降下的惩罚，唯有献出身上的全部物资以示虔诚、向神「赎罪」，才能换得活命与救赎的资格。台下，信徒们一个个仰着头，眼神狂热而空洞，齐声应和着，仿佛溺水的人抓住了最后一根稻草。你站在门口，一眼就看穿了这是裹着信仰外衣的盘剥骗局，可这庇护所的安稳，和那群人危险的虔诚，都让你不敢轻举妄动。',
      choices: [
        { text: '献出物资，混入这个庇护所', effects: { supplies: -16, sanity: 6, hp: 4 }, reaction: '「先知」满意地收下你的物资，信徒们朝你投来狂热的接纳目光——你看穿了这是盘剥，却也用一份家底，换来了一处暂避风雨的瓦。' },
        { text: '揭穿骗局，鼓动众人散去', effects: { sanity: -6, supplies: -2, hp: -4 }, reaction: '你当众戳破谎言，「先知」气得脸色铁青，几个狂热信徒扑上来要打你——可也有人怔住了，眼里头一回闪过被你唤醒的清明。' },
        { text: '默默退出，不沾是非', effects: { supplies: -3, sanity: -2 }, reaction: '你一眼看穿这裹着信仰的骗局，没声张便退了出去——不被狂热裹挟，也不去捅那个马蜂窝，这份明哲保身的分寸，稳。' },
      ],
      summary: '末日布道', art: '1eoq8bv', gen: 'gemini',
      minTurn: 9,
    },
    {
      narrative:
        '走投无路时，你拐进一条死胡同，下一秒便陷入了绝境——一群瘦骨嶙峋的野犬不知从哪儿冒了出来，呈半圆形把你死死围在中央。漫长的饥荒早已让它们眼里布满血丝，褪去了对人类最后的畏惧。为首那只体格最壮的黑犬压低身子，喉咙深处滚动着低沉而危险的咆哮，獠牙森森，涎水滴落，随时会扑上来撕咬。你的后背紧紧抵着冰冷的砖墙，已经退无可退——这群被饿疯了的畜生，此刻和那些感染者一样致命。',
      choices: [
        { text: '抛出干粮引开，趁机脱身', effects: { supplies: -10, hp: -4, sanity: -2 }, reaction: '群犬扑向你撒出的干粮乱抢成一团，你趁隙贴墙溜了——没硬碰那排獠牙，用一把口粮换条命，这脑子转得比腿快。' },
        { text: '挥矛硬拼，杀出血路', effects: { hp: -14, supplies: -2, sanity: -4 }, reaction: '你挺矛迎上扑来的头犬，死战中竟真撕开一道口子杀了出去——被逼到绝境还敢以一敌群的狠劲，连那群饿犬都怵了。' },
      ],
      summary: '野犬围攻', art: '1l8ce7i', gen: 'gemini',
      weight: 1.2,
    },
    {
      narrative:
        '攀上一栋高楼的天台，你被眼前的景象怔住了：在这满目死灰的世界里，竟有人在天台的菜畦里，奇迹般地种出了一小片郁郁葱葱的青菜，那刺眼的、生机勃勃的绿意，看得你眼眶发热，几乎以为自己出现了幻觉。菜畦的主人此刻并不在，只在田埂边立着一块用木板做的牌子，上面是娟秀却有力的字迹：「留给需要的人，请别全拿。」短短一行字，是这残酷末世里残存的、滚烫的善意与信任，此刻正沉甸甸地考验着饥肠辘辘的你。',
      choices: [
        { text: '只摘一把，留下大半', effects: { supplies: 6, sanity: 10 }, reaction: '你只摘了一小把，郑重地把那块「请别全拿」的木牌扶正。这片菜畦的主人若知此事，定会红了眼——废土上肯守一句约定的人，太稀罕了。' },
        { text: '连根拔起，全部带走', effects: { supplies: 14, sanity: -8 }, reaction: '你把那片菜连根拔了个精光，木牌上「留给需要的人」几个字被你踩在脚下——往后再有谁爬上来，只能对着空菜畦凉了心。' },
      ],
      summary: '天台菜畦', art: '1ch7i68', gen: 'gemini',
    },
    {
      narrative:
        '历经千辛万苦，你终于跌跌撞撞地摸到了城郊的检查站。望见铁丝网后那些荷枪实弹、身着制式军装的、活生生的军人时，你紧绷了多日的神经几乎当场崩断，泪水夺眶而出——这是真正的秩序，是文明伸出的手。可通往安全区内的流程却并不轻松：登记需要排成长队，逐一接受严格的体检筛查。就在你排队等待时，前头队伍里一个被检出发烧的男人突然被两名士兵架起，不顾他凄厉的哭喊与挣扎，硬生生拖离了队伍——那冰冷的一幕，让你心头骤然一沉。',
      choices: [
        { text: '如实登记，接受体检', effects: { supplies: -4, sanity: 12, hp: 4 }, reaction: '士兵核验完你的资料，神色缓和地朝里一引：「过了，进去吧。」排在身后的人投来羡慕的目光——按规矩来的人，终究走得最稳。' },
        { text: '设法插队，尽快入内', effects: { sanity: 4, supplies: -2, hp: -2 }, reaction: '你钻空子挤到了前头，身后响起一片不满的咒骂，几个士兵也警惕地多瞟了你两眼——抢得了一时的快，却落了人眼里的刺。' },
      ],
      summary: '检查站登记', art: '1bc1l5a', gen: 'gemini',
      keyMoment: true,
      minTurn: 24,
      once: true,
      weight: 2,
    },
    {
      narrative:
        '弹尽粮绝、孤立无援的第二十多天，你在翻找背包的最底层时，摸出了那张早已被汗水和雨水浸得发皱、卷了边的全家福。照片里，所有人都笑得那样灿烂、那样无忧无虑，定格在灾难降临之前某个再寻常不过的团圆时刻。可如今，照片里的这些人——你的父母、爱人、孩子——一个个生死未卜，杳无音信。鼻子毫无预兆地一酸，滚烫的热流瞬间涌上眼眶，眼前这片冰冷的、看不到尽头的废墟，竟在泪光里渐渐模糊、扭曲，连呼吸都被一股巨大的悲怆死死攥住。',
      choices: [
        { text: '把照片贴胸口，逼自己振作', effects: { sanity: 12, supplies: -3 }, reaction: '你把照片贴在胸口，擦干眼泪重新站了起来。没人看见这一幕，可把思念熬成往前走的力气，是濒临崩溃的人最了不起的自救。' },
        { text: '任由情绪决堤，哭一场', effects: { sanity: 6, hp: -2, supplies: -3 }, reaction: '你抱着照片在废墟里嚎啕大哭，把憋了许久的悲怆全哭了出来——没人笑你软弱，有时痛痛快快哭一场，才哭得动往后的路。' },
      ],
      summary: '一张全家福', art: '1hk2oj6', gen: 'gemini',
      weight: 0.8,
    },
    {
      narrative:
        '一座经过加固的独栋别墅赫然出现在你眼前，大门竟敞开着。你警惕地探身进去，里头的景象更让你浑身一激灵：陈设干净完好，毫无被劫掠的痕迹，餐桌上甚至还整整齐齐地摆着几盘饭菜，碗筷俱全，像是主人刚要开饭就被什么打断，匆忙离开了，只是那饭菜早已凉透、长出了霉斑。这反常的「完好」本该是天大的好事，可空气里却弥漫着一股说不清、道不明的、令人头皮发麻的诡异气息，仿佛有无数双眼睛正藏在某个角落，静静地注视着误入此地的你。',
      choices: [
        { text: '速速搜刮厨房与药柜', effects: { supplies: 12, hp: -4, sanity: -6 }, reaction: '你顶着那股令人头皮发麻的诡异气息，把厨房药柜搜了个遍才撤——明知有无数双眼睛在暗处盯着还敢动手，这胆子练到了头。' },
        { text: '只在门口探看，不进深处', effects: { supplies: -3, sanity: 2 }, reaction: '你只在门口探了探，被那反常的「完好」激得后背发凉，没敢深入——读得懂诡异背后的杀机并及时止步，这份直觉救命。' },
      ],
      summary: '诡异别墅', art: '1k8lheg', gen: 'gemini',
      weight: 1.1,
    },
    {
      narrative:
        '当初被你用那瓶退烧药从死亡边缘拉回来的那个孩子，如今像影子一样寸步不离地跟着你。他用还带着奶气的、怯生生的嗓音一声声唤你「叔叔」或是「阿姨」，每当遇到危险，便本能地往你身后躲、紧紧攥着你的衣角。这个稚嫩的小生命，给你冰冷孤寂的逃亡之路添了一丝久违的暖意和牵挂，让你重新有了「想为谁而活」的念头。可你也无比清醒地知道，在这吃人不吐骨头的乱世里，多一张要喂的嘴，多一个跑不快、护不住自己的拖累，便意味着多一道随时可能要命的软肋。',
      choices: [
        { text: '把最后的口粮先紧着孩子', effects: { supplies: -8, sanity: 12, hp: -2 }, reaction: '孩子捧着你省下的口粮，小口小口地吃，抬头怯生生喊了声「谢谢」。这世道还肯饿着自己喂别人孩子的人，稀罕得像奇迹。' },
        { text: '教他自己觅食、自保', effects: { sanity: 6, supplies: -4 }, reaction: '孩子学得认真，小手笨拙地照你说的设套、藏身。授人以鱼不如授人以渔——这份冷静的远见，比一时的口粮更能护他长久。' },
      ],
      summary: '同行的孩子', art: 'jbmbfe', gen: 'gemini',
      requiresItem: '退烧药',
      weight: 1.1,
    },
    {
      narrative:
        '收音机的电池终于还是耗尽了。那个陪你熬过无数个漫漫长夜的电台声音——无论是循环的求救录音，还是断续的人声——在「滋啦」一声拖长的杂音后，戛然而止，再也没能响起。你反复拍打、拧动着旋钮，换来的只有死一般的沉寂。整个世界仿佛在这一刻被人猛地按下了静音键，安静得令人窒息、令人发慌。那份维系了许久的、与外界尚有联系的渺茫念想被彻底掐断，铺天盖地的孤独感如冰冷的潮水般汹涌袭来，几乎要将你整个人吞没。',
      choices: [
        { text: '翻遍废墟寻找新电池', effects: { supplies: -6, sanity: 8, hp: -2 }, reaction: '你翻箱倒柜地满世界找电池，只为重新接通那点与外界相连的念想——这份死也不肯让希望熄灭的执拗，撑住了将崩的神经。' },
        { text: '接受寂静，靠自己撑下去', effects: { sanity: -6, supplies: -2 }, reaction: '你放下了拍打的手，任那片死寂吞没自己。没人能替你扛这铺天盖地的孤独，可肯独自咽下它继续走的人，内里其实硬得很。' },
      ],
      summary: '电台熄灭', art: '1gqql17', gen: 'gemini',
      minTurn: 14,
    },
    {
      narrative:
        '军方地图上用红圈郑重标注的那处安全区，就在前方的山谷尽头，胜利仿佛触手可及。可越是靠近那道山口，你的脚步就越是沉重——沿途横陈的尸体一具接着一具，越来越密集，焦黑的土地、深陷的弹坑、扭曲的金属残骸触目惊心，空气里至今还残留着硝烟与血腥混杂的气味。这里分明曾爆发过一场惨绝人寰的争夺与厮杀，多少人为了挤进那道生门，把命永远丢在了门外。希望就在眼前，可通往希望的这最后一段路，却铺满了前人的尸骨与凶险。',
      choices: [
        { text: '硬着头皮走进山谷', effects: { hp: -10, supplies: -6, sanity: 6 }, reaction: '你踏着前人的尸骨一步步走进山谷，没被那满地凶险吓退——希望就在眼前，敢从死人堆里趟出一条生路的人，值得活下去。' },
        { text: '绕行高地，远远观察', effects: { hp: -6, supplies: -8, sanity: -2 }, reaction: '你没急着扎进那片焦土，而是绕上高地先看清局势——希望近在咫尺也不冲动送命，这份沉得住气，正是熬到此刻的本钱。' },
      ],
      summary: '焦土山谷', art: '1g4xnde', gen: 'gemini',
      requiresItem: '安全区地图',
      minTurn: 18,
      weight: 1.5,
    },
    {
      narrative:
        '你正蹑手蹑脚地在一座废墟超市里搜寻，头顶那台落满灰尘的应急广播却毫无征兆地「滴」一声自检启动，紧接着，一个甜腻欢快得近乎诡异的机械女声响彻了整个空旷的卖场：「叮咚——恭喜您！您是本店第10000位幸运顾客！请立即前往一楼服务台，领取您的神秘大奖！」那荒诞的播报声经过音响放大，在死寂的街区里炸开了锅，余音嗡嗡回荡。话音未落，远处便骤然爆发出此起彼伏、被彻底点燃的嘶吼——这「大奖」的喇叭，无异于一记敲响的丧钟。',
      choices: [
        { text: '将错就错冲去服务台抢东西', effects: { supplies: 12, hp: -10, sanity: 2 }, reaction: '你索性踩着这记丧钟般的播报冲向服务台，在尸群涌来前抢空了货架——别人慌着逃命，你却敢逆着险情捞一把，这胆识够野。' },
        { text: '砸了广播器，亡羊补牢', effects: { supplies: -3, sanity: -4 }, reaction: '你一把砸烂了那台胡乱播报的广播器，荒诞的声浪戛然而止——虽已招来嘶吼，可懂得第一时间掐断祸源，这反应不慢。' },
      ],
      summary: '便利店中奖', art: '1oesdpj', gen: 'gemini',
      weight: 0.4,
      once: true,
    },
    {
      narrative:
        '正当你疲惫地穿行废墟时，「扑棱」一声，一只羽毛脏污凌乱的鹦鹉竟稳稳落在了你的肩头，吓了你一跳。还没等你反应过来，它便歪着脑袋，用一种字正腔圆、模仿得惟妙惟肖的腔调，复述起前主人显然反复念叨过的「遗言」：「冰箱、密码、6657、别信老王……冰箱、密码、6657、别信老王……」念完，它眨巴着圆溜溜的眼睛端详你片刻，又一字不差地从头重复了一遍。这没头没尾的几句话里，分明藏着某个不知所踪的人留下的、价值难料的秘密。',
      choices: [
        { text: '带上它，这情报值钱', effects: { supplies: 6, sanity: 6, hp: -2 }, reaction: '你把鹦鹉揣进怀里，默念着那串「6657」。能从一只鸟的胡言乱语里嗅出值钱的秘密，这份机敏，旁人想都想不到。' },
        { text: '逗它玩一会儿，难得的慰藉', effects: { sanity: 10, supplies: -4 }, reaction: '你逗着鹦鹉笑出了声，这是陷落以来头一次发自心底的轻松——肯在末世里给自己留一刻温柔的人，心还没被磨死。' },
      ],
      summary: '会说话的鹦鹉', art: '1h3enk3', gen: 'gemini',
      weight: 0.4,
      once: true,
      itemsGained: ['鹦鹉'],
    },
    {
      narrative:
        '转过街角，一幅诡异到极致的画面让你猛地刹住了脚步，连大气都不敢出：十几个感染者竟一反常态，整整齐齐地排成了两列纵队，迈着僵硬笨拙却出奇一致的步子，缓慢地向前挪动着。更令人头皮发麻的是，走在队首的那两只，一只身上还挂着早已污脏破败的婚纱残片，另一只则套着同样残破的西装外套——它们一前一后，宛如一对正步入礼堂的新人，演绎着一场永不散场、荒诞而凄艳的「婚礼」。地上还散落着「宾客」们掉落的物资，唾手可得，可这「喜事」，惊动不得。',
      choices: [
        { text: '屏息绕行，谁也别惊动这场「喜事」', effects: { sanity: -6, supplies: -2 }, reaction: '你屏着气，一步一挪地绕开了那支诡异的「迎亲」队伍——读得懂何时该当一缕影子悄然退场，这份分寸，在废土上就是命。' },
        { text: '趁它们「出神」摸走宾客掉落的物资', effects: { supplies: 10, hp: -8, sanity: -4 }, reaction: '你贴着地面，从那群「宾客」脚边一点点把物资摸了回来——敢在感染者鼻子底下顺东西，这心理素质叫人头皮发麻地佩服。' },
      ],
      summary: '感染者婚礼', art: '91fahe', gen: 'gemini',
      weight: 0.4,
      once: true,
    },
    {
      narrative:
        '你在废墟里捡到一部还剩半格电的智能手机，屏幕一亮，前置摄像头便毫不留情地映出了你自己的脸——蓬头垢面，胡子拉碴，眼窝深陷，活脱脱一个野人。盯着那张陌生又熟悉的脸看了半晌，鬼使神差地，你竟点开了录像功能，对着镜头扯出一个比哭还难看的笑，用沙哑的嗓子干巴巴地开了口：「还有人在听吗……今天是第几天了，我也记不清了，就……看看这破地方还能扒拉出点什么……」荒诞的自嘲背后，是几乎要被孤独压垮的灵魂，在拼命寻找一个倾诉的出口。',
      choices: [
        { text: '认真「运营」，苦中作乐当个末世主播', effects: { sanity: 12, supplies: -5 }, reaction: '你对着镜头自说自话，竟把自己逗笑了。没有观众，可在末世里还能拿绝境开涮、给灵魂找个出口的人，韧性强得吓人。' },
        { text: '录完就删，别留下行踪证据', effects: { sanity: 4, hp: -2 }, reaction: '你录完便利落地删了视频，不留半点行踪。哪怕孤独到想找人说话，也清醒地不给敌人留破绽——这份冷静，救命。' },
      ],
      summary: '末世直播', art: '1ekw8sn', gen: 'gemini',
      weight: 0.4,
      once: true,
    },
    {
      narrative:
        '走投无路之际，你被一座戒备森严的幸存者营地「收留」了。高墙、岗哨、严整的巡逻，乍看之下确实安稳得让人心安。可没过多久你就看清了真相：这里被一个铁腕的「营主」用赤裸裸的恐惧牢牢统治着——所有人必须上缴一切私人物资，必须对营主绝对服从，稍有违逆或异议者，便会被当众绑了去喂养圈养的感染者，以儆效尤。安稳是真真切切的安稳，可这安稳的代价，是沦为任人宰割、毫无尊严的奴隶。营地里压抑的空气，让每一次呼吸都透着窒息。',
      choices: [
        { text: '忍辱蛰伏，伺机推翻暴君', effects: { sanity: -6, supplies: 4, hp: -4 }, reaction: '你低眉顺眼地咽下屈辱，暗中却把营主的破绽一一记在心里——能忍常人不能忍、磨刀不出声的人，往往才是最可怕的对手。' },
        { text: '煽动幸存者反抗营主', effects: { hp: -10, sanity: 6, supplies: 2 }, reaction: '你压低声音点起众人胸中的火，起初没人敢应，可你眼里的光一点点烧到了他们脸上——敢在暴君眼皮下举旗的，是天生的领头人。' },
        { text: '连夜逃离这座人间炼狱', effects: { supplies: -6, sanity: 4, hp: -4 }, reaction: '你趁着夜色翻墙逃了出去，把那座吃人的炼狱甩在身后——分得清安稳和奴役的区别，宁可重回风雨也不跪着活，这份骨气难得。' },
      ],
      summary: '暴君营地', art: '1ejev7t', gen: 'gemini',
      minTurn: 8,
      weight: 1.2,
    },
    {
      narrative:
        '命运的重担毫无征兆地压在了你肩上——一个对病毒天生免疫的孩子，机缘巧合落入了你的护佑之下。据说，流淌在他血管里的血液，是各方研究机构梦寐以求、用以研制疫苗的唯一希望，是足以扭转整个末世的钥匙。正因如此，这孩子成了所有势力眼中的肥肉：军方、科研组织、武装匪帮、人贩子……无数双贪婪的眼睛都在暗中追杀、抢夺着他。护他周全，就等于以你一己之力，与这片疯狂废土上几乎所有的强权为敌，每走一步都凶险万分。',
      choices: [
        {
          text: '誓死护送孩子去研究所',
          effects: {},
          outcomes: [
            { weight: 4, effects: { sanity: 10, hp: -10, supplies: -6 }, reaction: '你把孩子紧紧护在身后，迎着各路追兵杀出一条血路——以一己之力同满世界的强权为敌，只为护一个孩子，这份孤勇足以载入传说。' },
            { weight: 1, effects: {}, endTone: '以命护苗·废土微光', reaction: '你押上了自己的一切，一路躲过军方、匪帮与人贩子的围猎，把那个对病毒免疫的孩子亲手送进了还在运转的研究所——你用一条命，替所有人守住了希望。' },
          ],
        },
        { text: '把他交给出价最高的势力', effects: { supplies: 18, sanity: -14 }, reaction: '买家清点完物资，牵走了那个回头望你的孩子。你换来了泼天的家底，却也换来了那双眼睛——它会在你往后的每个梦里追问你。' },
        { text: '只护他一程，到安全处便分别', effects: { sanity: 4, supplies: -4 }, reaction: '你把孩子送到了相对安全的地方便转身。既没拿他换钱，也没赔上自己的命——量力而行的善意，虽不耀眼，却也问心无愧。' },
      ],
      summary: '免疫之子', art: '1bfhgk8', gen: 'gemini',
      once: true,
      keyMoment: true,
      minTurn: 3,
      weight: 1.3,
    },
    {
      narrative:
        '兜兜转转，你回到了那个曾经被称作「家」的地方，把自己反锁进经过加固的屋子里，仿佛缩回了最后的壳。日子一天天过去，你抱着那台早已收不到任何回应的收音机，对着空荡荡的频率喃喃自语，一遍遍重复着没人听的话。某天，你瞥见镜子里那个形容枯槁、眼神涣散的人，张了张嘴，却发现自己竟有些记不起该如何与另一个活人开口交谈了。一个可怕的念头在脑中盘旋不去：或许，你就是这座死城里，硕果仅存的、最后一个活人了。',
      choices: [
        { text: '强迫自己规律作息、坚守理智', effects: { sanity: 12, supplies: -6 }, reaction: '没有一个活人见证你和遗忘、和疯癫死磕的这一程，可在举世皆寂里硬撑着不让自己散架的人，守住的是人之所以为人的最后一线。' },
        { text: '出门去寻找哪怕一个活人', effects: { hp: -6, sanity: 6, supplies: -4 }, reaction: '你推开那扇加固的门重新走进废墟，只为证明自己不是最后一个人——肯为一声回应踏出壳的人，心里那点火，还没灭。' },
      ],
      summary: '最后的人', art: '1ekyl75', gen: 'gemini',
      requires: 'sanity<=35',
      minTurn: 10,
      weight: 1.1,
    },
    {
      narrative:
        '你拼死挤上了一列还能开动的列车，满心以为找到了逃离疫区的生路，谁知厄运早已埋伏在车厢里——病毒已经混了进来。在这密闭狭长、首尾不通的车厢通道中，一场无处可逃的灾难骤然引爆：尖叫着四散奔逃的恐慌人群、张牙舞爪扑咬过来的感染者、还有那些为了自保不惜把他人推向死亡当作肉盾的自私之徒……生死时速之下，文明的伪装被撕得粉碎，人性最不堪、最赤裸的一面在这节车厢里暴露无遗。是奋起组织众人结成屏障，还是踩着别人逃命，全凭你一念之间。',
      choices: [
        { text: '组织幸存者，守住车厢节节后退', effects: { hp: -8, sanity: 6, supplies: -2 }, reaction: '你一声吼镇住了乱作一团的人群，带着众人结成屏障节节后退。混乱里有人含泪喊你「头儿」——危难时挺身而出的人，自带让人追随的光。' },
        { text: '只顾自己，挤过人群往前逃', effects: { hp: -4, sanity: -8, supplies: 2 }, reaction: '你把别人挤开往前冲，身后接连响起被你撞倒的人绝望的尖叫——你逃出来了，可那一路踩着的哭喊，会牢牢钉在你心上。' },
      ],
      summary: '死亡列车', art: '1f8feqm', gen: 'gemini',
      once: true,
      keyMoment: true,
      minTurn: 3,
      weight: 1.2,
    },
    {
      narrative:
        '你找到了一处官方设立的「安全区」——高耸的混凝土围墙，墙顶通了电的铁丝网，墙内秩序井然，有热食、有干净水、有床铺，是末世里近乎天堂般的存在，承诺接收所有前来投奔的幸存者。可这份安稳的代价，却赤裸裸地写在入区须知上：你必须无条件交出身上的全部物资与所有武器，登记造册、按指纹、编入管理名册，从此一举一动都受管控，再不能随意进出。换言之，跨进这道门，你便用全部的自由，换取了一份被圈养起来的太平——是金丝笼，也是避风港。',
      choices: [
        { text: '交械入区，换一份安稳', effects: { supplies: -10, hp: 8, sanity: 4 }, reaction: '你交出武器物资，按下指纹，跨进了那道门。门内的人递来一碗热食和一句「辛苦了」——用自由换一份太平，这取舍冷暖自知。' },
        { text: '只在墙外讨生活，不入牢笼', effects: { sanity: 4, supplies: -4, hp: -2 }, reaction: '你望着那堵高墙摇了摇头，转身回到风雨里。守门的兵都愣了：这年头，还有人嫌避风港是牢笼——这份对自由的执拗，稀罕。' },
      ],
      summary: '安全区代价', art: '1cekkui', gen: 'gemini',
      minTurn: 11,
    },
    {
      narrative:
        '黑压压的尸潮如决堤的洪水般从街道尽头汹涌而来，瞬间淹没了一切，那密密麻麻、铺天盖地的躯体和震耳欲聋的嘶吼，绝望得让人头皮发炸。你和零星几个幸存者仓皇退守进一栋楼里，死死顶住那扇唯一的大门。门外，是成千上万只腐烂的手在疯狂拍打、抓挠，门板被撞得剧烈震颤、嘎吱作响，铰链一点点松动，门缝里伸进来无数扭曲的手指——所有人都心知肚明，这道脆弱的门，撑不了多久了，再不做决断，所有人都得葬身于此。',
      choices: [
        { text: '设路障、引爆油桶，殊死阻击', effects: { hp: -12, supplies: -8, sanity: 4 }, reaction: '油桶在尸潮里轰然炸开，火光冲天。同伴们望着你布下的这一手死局，眼里燃起了希望——绝境里还能镇定排兵的人，是众人的主心骨。' },
        { text: '弃楼走天台，跨楼逃生', effects: { hp: -8, supplies: -4, sanity: -2 }, reaction: '你当机立断带众人弃楼跨向天台，把尸潮甩在身后——分得清什么时候该守、什么时候该撤，这份决断，救了不止你一条命。' },
        {
          text: '把同伴推出去断后，独自逃',
          effects: {},
          outcomes: [
            { weight: 4, effects: { hp: -2, sanity: -16, supplies: 4 }, reaction: '你猛地把同伴推向尸潮，趁着他凄厉的惨叫脱身。活下来的你，从此再不敢回头看任何人的眼睛——这条命，沾着同伴的血。' },
            { weight: 1, effects: {}, endTone: '弃众独生·孤鬼游荡', reaction: '你把信任你的同伴一个个推向尸潮，踩着他们的惨叫独自杀出生天——那一刻起，你保住了命，却把自己活成了一缕谁也不敢靠近的孤魂。' },
          ],
        },
      ],
      summary: '尸潮围城', art: '1d2alf1', gen: 'gemini',
      keyMoment: true,
      once: true,
      minTurn: 3,
      weight: 1.3,
    },
    {
      narrative:
        '一伙身穿白大褂、戴着口罩的人在你毫无防备时将你制服、捆缚起来，强行拖进了一处隐秘的实验室。为首者推了推眼镜，用一种冷漠到近乎虔诚的语气宣称，要拿你的身体做疫苗活体实验，「这是为了全人类的存续，是崇高的牺牲」。实验室里阴森森的，泛着刺鼻的消毒水味，冰冷的金属手术台、明晃晃的器械和插着不明液体的针管让人遍体生寒。更让你魂飞魄散的是，隔壁的房间里，正不断传来前一个「志愿者」凄厉绝望、却又渐渐微弱下去的惨叫。',
      choices: [
        { text: '配合实验，赌它真能造出疫苗', effects: { hp: -12, sanity: -6, supplies: 2 }, reaction: '为首者推了推眼镜，难得露出一丝近乎敬意的神色：「你比上一个识大体。」可隔壁渐弱的惨叫提醒着你，这份「崇高」赌的是命。' },
        { text: '拼死反抗，砸开实验室逃生', effects: { hp: -10, sanity: 4, supplies: 4 }, reaction: '你挣开束缚砸翻器械夺路而逃，白大褂们惊得四散躲避——被绑上手术台还能反咬一口杀出去的，他们头回见这么硬的「材料」。' },
      ],
      summary: '活体实验', art: '1fi6fa5', gen: 'gemini',
      requires: 'hp>=30',
      minTurn: 14,
      weight: 1.2,
    },
    {
      narrative:
        '循着电台里那个反复播报的「友善社区」广播，你终于找到了它——一处宛如世外桃源的营地：篱笆整齐，炊烟袅袅，居民们个个面色红润、热情好客，又是端酒又是上肉，殷勤得让你受宠若惊，几乎以为重新找回了乱世里失落已久的人情温暖。可就在你卸下心防、准备饱餐一顿时，无意间瞥见了后厨阴影里挂着的那几扇还来不及处理干净的、形状熟悉得令人作呕的「肉」——那一瞬间，你浑身的血液都凉透了：这群笑容可掬的「好人」，吃的根本就是人。这顿盛宴，你或许就是下一道菜。',
      choices: [
        { text: '不动声色，连夜逃出生天', effects: { sanity: -8, supplies: 4, hp: -2 }, reaction: '你强压住翻涌的恐惧，陪笑着熬到深夜才悄然溜走——识破了惊天的杀机还能稳住脸不露半分破绽，这份镇定，从虎口里捡回了命。' },
        { text: '揭穿真相，与食人族火并', effects: { hp: -14, sanity: 6 }, reaction: '你掀翻饭桌当众戳穿那几扇人肉，与食人者拼得头破血流——明知寡不敌众也不肯坐着当下一道菜，这份血性叫人胆寒又敬服。' },
        {
          text: '装作不知，蹭一顿饱饭再走',
          effects: {},
          outcomes: [
            { weight: 4, effects: { supplies: 6, sanity: -12 }, reaction: '你装作什么都没看见，就着那几扇「肉」吃了顿饱饭。东西是吃下去了，可那滋味连同自己默许的恶，会在胃里翻搅许久。' },
            { weight: 1, effects: {}, endTone: '同流合污·食人自保', reaction: '你不仅咽下了那几口明知是什么的「肉」，还在心底悄悄认下了这条「活路」——从这一口起，你与那群笑容可掬的食人者，再没了分别。' },
          ],
        },
      ],
      summary: '末日食人', art: '1ep0j1q', gen: 'gemini',
      keyMoment: true,
      once: true,
      minTurn: 3,
      weight: 1.2,
    },
    {
      narrative:
        '情报指向旧城区一座医院的药库，那里囤积着堪称救命的抗生素和成箱的净水片，对眼下缺医少药的你而言，价值连城。可当你举着捡来的辐射计数器靠近那片区域时，仪器立刻发出了刺耳的、密集到连成一片的警报声——读数早已爆表。原来这一带，正是某座核反应堆熔毁后遗留的辐射禁区，那些救命的药品就泡在足以致命的辐射尘里。进去取药，每多停留一分钟，就是在拿自己的寿命和未来的健康，去换眼前这批续命的物资。这笔账，怎么算都让人胆寒。',
      choices: [
        { text: '速进速出，抢一批救命药', effects: { supplies: 16, hp: -14 }, reaction: '计数器尖叫成一片，你却冲进辐射禁区抢出了成箱的救命药——拿往后的寿命去换眼前的活路，这破釜沉舟的狠劲，叫人既佩服又心惊。' },
        { text: '简易防护、只取最近的一批', effects: { supplies: 8, hp: -6 }, reaction: '你裹上简易防护只取了最近的一批就撤，没贪多——既要药也惜命，在贪婪和理智之间卡住那条线，这份分寸救命。' },
        { text: '辐射太凶，放弃这批物资', effects: { supplies: -4, sanity: -2 }, reaction: '你盯着爆表的读数，终究把那批救命药留在了辐射尘里——舍得放下唾手可得的诱惑，不拿命换物的人，才活得长久。' },
      ],
      summary: '辐射禁区', art: '1ksh1pw', gen: 'gemini',
      minTurn: 13,
      weight: 1.1,
    },
    {
      narrative:
        '你颤抖着退出弹匣看了一眼——枪膛里只剩下孤零零的最后一颗子弹了。而此刻，你已被逼入了绝境：身后是无路可退的死墙，面前，三五只感染者正发出渴求的低吼，迈着踉跄却执着的步子，一步步朝你逼近，腐臭的气息扑面而来。时间一分一秒地流逝，命悬一线。这最后一颗珍贵的子弹，是该咬紧牙关，瞅准时机射向最近的敌人，搏那一线杀出重围的生机；还是……抵住自己的太阳穴，在被啃噬殆尽、痛苦地变成怪物之前，给自己留一个最后的、体面的解脱？',
      choices: [
        { text: '一弹搏命，杀出最后生机', effects: { hp: -12, sanity: 4 }, reaction: '你瞅准最近那只感染者的眉心扣下最后一弹，趁它倒地的空当杀出了重围——绝境里这一枪稳得叫人发怵，命是自己一颗子弹挣回来的。' },
        { text: '虚晃示警、靠双腿赌一把', effects: { hp: -8, sanity: -6, supplies: -2 }, reaction: '你没舍得开那一枪，虚晃一记便撒腿狂奔。把生死押在双腿上是险了点，可留着子弹、绝不轻言放弃的人，总能多搏一线。' },
      ],
      summary: '最后一颗子弹', art: 'tau20j', gen: 'gemini',
      requires: 'hp<=30',
      minTurn: 16,
      weight: 1.2,
    },
    {
      narrative:
        '为躲避追兵，你避进了一座废弃的图书馆。一排排高大的书架在厚厚的尘埃里沉默矗立，无数书籍静静地躺在那里，无人翻阅。在这个连一口干净水都重逾黄金的末世里，这些印满文字的纸页早已一文不值，没人会用宝贵的物资去换一本书。可当你的指尖拂过那一道道书脊时，心头却莫名地一沉、一颤——它们承载的，是整个旧世界的智慧、记忆与文明的火种，是人类之所以为人的证明。它们一文不值，却又重逾千钧。是带走几本要紧的留作希望，还是撕了它们生火取暖、活在当下？',
      choices: [
        { text: '取几本要紧的，留作火种与知识', effects: { sanity: 10, supplies: -4 }, reaction: '你挑了几本要紧的小心揣进怀里。在连水都重逾黄金的世道还肯为知识留一席之地的人，守的是人类不肯熄灭的那点火种。' },
        { text: '只撕书页生火取暖，活着要紧', effects: { hp: 4, sanity: -6 }, reaction: '你一页页撕下书烧火取暖，火光里字句化成灰烬。活下去当然要紧，可亲手烧掉文明火种的那一刻，你心里也空落落地疼了一下。' },
      ],
      summary: '文明遗迹', art: '1eg0siy', gen: 'gemini',
      minTurn: 10,
      weight: 0.9,
    },
    {
      narrative:
        '一个谁也没料到的消息在营地里炸开了锅：你的同伴，竟在这朝不保夕的末世里怀上了孩子。一个新生命的到来，本该是天大的喜事，是黑暗里燃起的希望之火，可在这随时可能丧命、连成年人都自身难保的世道，一个嗷嗷待哺的婴儿，更意味着沉重的拖累、巨大的风险和九死一生的分娩。营地里为「生还是不生」吵成了一团，各执一词，互不相让，焦灼的目光最终齐刷刷地汇聚到了你这个主心骨身上——这个关乎生命与存续的艰难抉择，等着你来一锤定音。',
      choices: [
        { text: '护住这个孩子，守住希望', effects: { sanity: 10, supplies: -8, hp: -2 }, reaction: '你一锤定音要护住这个孩子，营地里争执的众人安静下来，你的同伴泪眼婆娑地握住你的手——敢在绝境里赌一个新生的人，给了所有人活下去的盼头。' },
        { text: '末世不宜生育，忍痛劝阻', effects: { sanity: -6, supplies: 4 }, reaction: '你忍着心痛说出那番冷硬的现实，营地里一片沉默，你的同伴别过脸去——没人说你错，可这份不近人情的清醒，扎得人生疼。' },
      ],
      summary: '末世新生', art: '1ekt7hx', gen: 'gemini',
      requires: 'sanity>=30',
      minTurn: 18,
      weight: 1.1,
    },
    {
      narrative:
        '断粮的日子已经熬过了十多天，营地里的存粮彻底见了底，体弱的人一个接一个地饿死，蜷缩的饿殍在角落里渐渐多了起来，连空气里都弥漫着死亡与绝望的气息。今天，又一个同伴咽了气，尸体还没凉透，你却惊恐地发现，几个饿得双眼通红的人正死死盯着那具遗体，喉结上下滚动，终于有人压低了声音，颤抖着说出了那个所有人都心知肚明、却谁也不敢宣之于口的字眼。维系人之所以为人的最后那道堤坝，在无边无际的饥饿面前，正发出不堪重负的呻吟，摇摇欲坠。',
      choices: [
        { text: '严守底线，绝不食人', effects: { sanity: 8, hp: -8, supplies: -2 }, reaction: '你死死按住那道堤坝，宁可饿着也不肯逾矩。有人红着眼骂你迂腐，可也有人默默退到了你身边——守住人之为人最后底线的人，本身就是一面旗。' },
        { text: '默许此事，活下去最大', effects: { hp: 6, sanity: -16, supplies: 4 }, reaction: '你别过脸，默许了那件谁也不敢说出口的事。肚子是填上了，可从这一夜起，营地里每个人的眼神都变了——包括你自己的。' },
        { text: '拼死外出觅食，为众人寻一线生路', effects: { hp: -10, sanity: 6, supplies: 8 }, reaction: '你在所有人都盯着那具遗体时，独自抄起家伙冲进了死城觅食。带着粮回来的那一刻，众人望你的眼神，是绝望里重新燃起的敬服与生机。' },
      ],
      summary: '饥荒人性', art: '1mgmym5', gen: 'gemini',
      requires: 'supplies<=20',
      minTurn: 15,
      weight: 1.2,
    },
    {
      narrative:
        '风向忽然变了，一股甜腻里裹着刺鼻的怪味顺着街道漫了过来，呛得你眼睛刺痛、喉咙发紧，远处一座化工厂的烟囱正歪斜着往外吐着黄绿色的浓雾。你拿衣袖死死捂住口鼻，泪水还是不受控地往下淌。这毒雾一旦弥漫开来，整片街区都将成为活人的禁地。可就在你慌乱后退时，眼角扫到路边一具穿着防化服的工人遗骸，他脸上那只完好的防毒面具，在毒雾的黄绿色里泛着一点冷光——那东西，眼下比任何东西都金贵。',
      choices: [
        { text: '屏息冲过去摘下防毒面具', effects: { hp: -8, supplies: -4, sanity: 2 }, reaction: '你憋着一口气扑过去扯下面具扣在脸上，灼人的毒雾被隔在了外头——敢在毒云里抢一具尸体的家当，这份豁出去的果决救了你往后的命。' },
        { text: '捂住口鼻拼命逆风逃离', effects: { hp: -12, supplies: -6, sanity: -4 }, reaction: '你呛着咳着踉跄逃出毒雾区，肺里像灌了火——逃是逃了，可那只能救命的面具，也被你连同那具遗骸一起丢在了黄绿色的死地里。' },
      ],
      summary: '化工厂毒雾', art: '7vye4y', gen: 'gemini',
      once: true,
      itemsGained: ['防毒面具'],
      weight: 1.3,
    },
    {
      narrative:
        '逃命的路被生生堵死，唯一的去向是那条灌满了沉积毒气的废弃地铁隧道——黄绿色的雾气贴着地面缓缓流动，凡是钻进去过的人都再没出来。你下意识摸了摸脸上那只防毒面具，滤罐还能再撑一阵。隧道深处的站台，据说曾是一处地下避难所的入口，里头囤着搬不完的给养，正因这毒气封路，才一直无人敢取。面具勒得脸颊生疼，每一次呼吸都带着滤芯的橡胶味，可它此刻是你唯一的底气。',
      choices: [
        { text: '戴稳面具，深入毒气隧道搜刮', effects: { supplies: 30, hp: -6, sanity: -4 }, reaction: '你罩着面具一头扎进黄绿色的雾里，把那处无人敢碰的给养搬了个空——别人光是闻着这味就腿软，你却靠一只面具走进了死人堆里的宝库。' },
        { text: '只在隧道口探一截就退', effects: { supplies: 8, hp: -3, sanity: -2 }, reaction: '你贴着隧道口拣了些近处的东西就退了出来，没敢深入——面具虽在，可拿命去赌滤罐能撑多久，终究不是稳妥人会做的事。' },
      ],
      summary: '毒气隧道', art: '1ffit51', gen: 'gemini',
      requiresItem: '防毒面具',
      once: true,
      weight: 1.4,
    },
    {
      narrative:
        '满载而归的代价很快找上门来。你这一趟从毒气隧道里搬出的给养堆成了小山，可消息像血腥味一样在废土上飘散开去。今夜，藏身处外头的黑暗里挤满了贪婪的眼睛——拾荒的、火并的、走投无路的，全冲着你这份惹眼的家底来了。你蜷在堆得满满当当的库房角落，听着四面八方逼近的脚步声，第一次真切地体会到那句老话：在末世，囤得太多，本身就是一桩催命的罪。',
      choices: [
        { text: '据守库房，与围攻者拼死一搏', effects: { hp: -16, supplies: -10, sanity: -6 }, reaction: '你背靠着粮山红了眼地厮杀，硬是把成片的围攻者一一逼退——守着这么大一份家底还能不丢半分地杀出来，连最凶的劫掠者都倒抽了凉气。' },
        { text: '主动抛出大批物资引开众人', effects: { supplies: -34, sanity: 4 }, reaction: '你把成箱的给养往街心一推，贪婪的人群顷刻扑上去争抢撕咬，谁也没空再盯着你——用一半家底换一条活路，这笔账算得比谁都清醒。' },
      ],
      summary: '粮山引来的围攻', art: '930tkh', gen: 'gemini',
      requires: 'supplies>=70',
      weight: 1.6,
    },
    {
      narrative:
        '连日的奔波让你嗓子干得快要冒烟，你终于在一座社区净水站的废墟里翻到了希望——主控室的橱柜里，整整齐齐码着几板还没拆封的净水片，铝箔包装在积灰里闪着银光。在这个连一口干净水都能要人命的世道，这玩意儿能把臭水沟里的浊水变成救命的清流，价值远胜罐头。可净水站的铁门早被人用链条反锁，门上还潦草地喷着一个褪色的骷髅警告——上一个守着这里的人，显然没能守住。',
      choices: [
        { text: '砸开铁门，把净水片全数收入囊中', effects: { supplies: 6, hp: -4, sanity: 2 }, reaction: '链条在你的猛砸下崩断，你把那几板净水片一扫而空塞进怀里——这年头能囤上一把净水片的人，等于在背包里揣了一条命。' },
        { text: '只取一板，留些给后来的渴死鬼', effects: { supplies: 3, sanity: 5 }, reaction: '你只抠下一板就收了手，把剩下的留在原处。后来摸进来的人对着那几板净水片愣了半晌——这死城里，竟还有人肯替陌生人留一口活命的清水。' },
      ],
      summary: '社区净水站', art: '1jrv356', gen: 'gemini',
      once: true,
      itemsGained: ['净水片'],
      minTurn: 4,
    },
    {
      narrative:
        '一连几天没沾过水，你的嘴唇裂出了血口子，头晕得几乎站不稳，眼前一阵阵发黑。终于，你在一处地势低洼的院落里寻到一汪积水，可凑近一看心又凉了半截——水面浮着一层油膜，泛着诡异的暗绿，底下隐约沉着不知名的腐物，分明是被污染过的脏水。喝下去多半要上吐下泻，可不喝，你撑不过今天。你颤抖着摸出怀里那几板净水片，铝箔包装在掌心被攥得发烫——它能不能把这摊毒水，变成救命的清泉？',
      choices: [
        { text: '投入净水片，澄清后大口痛饮', effects: {}, outcomes: [
          { weight: 1, effects: { hp: 14, supplies: 8, sanity: 4 }, itemsLost: ['净水片'], reaction: '净水片在浊水里嘶嘶冒泡，半晌过后那汪绿汤竟澄清了大半。你捧水痛饮，干裂的喉咙活了过来——会用一片药救自己一命的人，废土淘汰不了。' },
        ] },
        { text: '舍不得净水片，强咽几口脏水', effects: { hp: -12, supplies: -2, sanity: -4 }, reaction: '你攥着净水片没舍得用，硬灌了几口脏水。当晚便绞着肚子吐了个昏天黑地——省下了药，赔进去的却是大半条命，这账亏到了家。' },
      ],
      summary: '污染水源', art: '1fhnnz1', gen: 'gemini',
      requiresItem: '净水片',
      once: true,
      weight: 1.3,
    },
    {
      narrative:
        '深夜守着收音机调频时，仪表盘的指针忽然剧烈跳动起来，杂讯里钻出一段奇异的、规律到不似人为的电子脉冲——「滴、滴滴、滴」，一遍遍循环，像某种加密的坐标信标。你屏住呼吸，凭着退役通讯兵教过的那点皮毛，一笔一画把这串脉冲译成了一组方位数字，抄在了掌心。这信标太规整、太执着了，绝不会是哪台坏掉的旧设备在空转——它的源头，一定还藏着活人，或者，藏着某种刻意要被人找到的东西。',
      choices: [
        { text: '反复校验，记下这串信号源坐标', effects: { sanity: 8, supplies: -5 }, reaction: '你把那串脉冲来回译了三遍才敢确信，掌心的坐标被汗水洇得发涨——能在满是杂讯的死城里抠出一组真坐标的人，脑子比枪还好使。' },
        { text: '心烦意乱，关机不再理会', effects: { sanity: -4, supplies: -3 }, reaction: '那执拗的脉冲听得你心头发毛，伸手关了收音机。死寂重新罩下来，可那串没记下的坐标，往后会像根刺一样时不时扎你一下。' },
      ],
      summary: '神秘信标', art: '1hjn9qb', gen: 'gemini',
      once: true,
      itemsGained: ['信号源坐标'],
      minTurn: 7,
    },
    {
      narrative:
        '掌心那组信号源坐标，把你引到了城郊一座爬满藤蔓的废弃水塔下。塔身锈迹斑斑，可塔顶却架着一台明显是新近装上的太阳能天线，正一闪一闪地工作着——那执拗的脉冲，就发自这里。塔下一扇钢门虚掩着，门后黑黢黢的，隐约传来设备运转的低鸣。是谁在这与世隔绝的地方，日复一日地朝外发送着信标？是等待救援的幸存者，是某个组织的秘密据点，还是一个早已无人、只剩机器在空转的坟墓？你站在钢门前，心跳得厉害。',
      choices: [
        { text: '推门而入，弄清信标的源头', effects: { sanity: 6, supplies: 10, hp: -4 }, reaction: '门后竟是个小小的幸存者电台站，守台的老人见你循着信标找来，浑浊的眼睛一下亮了：「真有人收到了……我守这台机器，总算没白守。」' },
        { text: '爬上塔顶拆走天线设备就走', effects: { supplies: 6, sanity: -6 }, reaction: '你撬走了塔顶那台还在工作的天线，脉冲应声断绝。背着设备走远时你忽然心里一沉——若塔里真有人靠它等救援，这一下，等于亲手掐断了别人的盼头。' },
      ],
      summary: '信标源头', art: '1b5lett', gen: 'gemini',
      requiresItem: '信号源坐标',
      itemsLost: ['信号源坐标'],
      once: true,
      weight: 1.2,
    },
    {
      narrative:
        '一处坍塌的天桥下，传来微弱的呻吟。你扒开碎石，看见一个被横梁压住腿的年轻人，脸色惨白，却还死死护着怀里一个磨得发亮的旧怀表。见你过来，他抓住你的手，气若游丝地央求你帮他从废墟里脱身。救他要费不少力气，还得分他物资，可那双求生的眼睛里，又透着一股不愿连累人的倔强。末世里，伸手拉人一把是最奢侈的善意——这善意会变成同行的暖意，还是甩不掉的拖累，谁也说不准。',
      choices: [
        { text: '合力撬开横梁，救他脱困', effects: { hp: -6, supplies: -6, sanity: 10 }, reaction: '横梁撬开的一刻，年轻人攥着怀表泣不成声，硬把那块旧表塞进你手里：「这是我爹留的，往后……它替我跟着你。」从此你身边多了个肯把后背交给你的人。' },
        { text: '心有不忍，留下些水和药就走', effects: { supplies: -4, sanity: -3 }, reaction: '你搁下水和药转身离去，年轻人没有挽留，只是望着你的背影低声道谢——你没欠他什么，可那块没能护住的旧怀表，会在你梦里晃上几回。' },
      ],
      summary: '废墟下的人', art: '1sz7z7w', gen: 'gemini',
      once: true,
      itemsGained: ['旧怀表'],
      weight: 1.2,
    },
    {
      narrative:
        '漂泊了太久，居无定所地在废墟间东躲西藏，让你身心俱疲。这日你撞见一处难得的好地方——一栋只剩单一入口的半塌公寓，地势高、易守难攻，里头还封存着没被翻动过的物资。唯一的麻烦是，楼里盘踞着几个游荡的感染者，且门窗破败、需要大动干戈地封堵加固。是咬牙清场、把这里据为自己的第一处巢穴，还是嫌麻烦、继续做个无根的流浪者？',
      choices: [
        { text: '清场加固，据为巢穴', effects: { hp: -8, supplies: -6, sanity: 6 }, flagsSet: ['落脚点'], reaction: '你清掉了游荡的感染者、堵死了多余的门窗，头一回有了一处能安睡的角落——脚下有了根，飘了太久的心也定了几分。' },
        { text: '不敢久留，继续流浪', effects: { hp: -4, supplies: -4, sanity: -3 }, reaction: '你终究没敢在一处久待，又背起行囊钻进无边的废墟——连个能踏实喘口气的地方都没有，这日子何时是个头。' },
      ],
      summary: '觅一处栖身', art: '1uyogz3', gen: 'gemini',
      keyMoment: true,
      minTurn: 2,
      weight: 1.2,
    },
    {
      narrative:
        '在落脚点安顿下来后，你愈发觉得「能守」还不够，得让它「养得起人」。你盘算着把这处巢穴扩建成一个真正的据点：清出一间干燥的库房囤粮储水，在必经的过道上设几道简陋却致命的陷阱，再辟出一小块能种点东西的地方。这是一笔不小的投入，要耗去你眼下大半的体力与物资去经营；可一旦建成，你便不再是手到口的流浪汉，而有了一份能层层囤积、细水长流的家底。',
      choices: [
        { text: '扩建据点，囤粮储水', effects: { supplies: 10, hp: -6 }, flagsSet: ['据点'], reaction: '库房一点点充实起来，过道的陷阱也布置停当，你抚着囤起的粮水长出一口气——从今往后，你总算有了能囤得住的家底，不必再为下一顿发愁。' },
        { text: '维持现状，不折腾扩建', effects: { supplies: -4, sanity: -2 }, reaction: '你嫌扩建太耗精力，只守着这处巢穴将就过活——日子是省心，可家底始终薄薄一层，攒不下、也囤不久。' },
      ],
      summary: '加固扩建', art: '1bhqujm', gen: 'gemini',
      requires: 'has(落脚点)',
      keyMoment: true,
      minTurn: 9,
      weight: 1.1,
    },
    {
      narrative:
        '据点的名声渐渐传开，引来的不止是投奔者，还有觊觎你这份家底的掠夺者，以及偶尔成群涌来的感染者。仅靠门闩和陷阱，迟早要被撞破。你意识到，要想真正立住，必须把据点筑成一座堡垒：垒起高墙、清出射界、囤备应对围攻的水火之械。这是脱胎换骨的一步，要押上大量物资与心血，还要冒着筑墙期间防备空虚、被人趁虚而入的风险；可一旦墙成，寻常的尸潮与匪帮便再难撼动你分毫。',
      choices: [
        { text: '筑墙设防，严阵以待', effects: { hp: -8, supplies: -6, sanity: 4 }, flagsSet: ['堡垒'], reaction: '高墙垒起的那天，你站在墙头俯瞰自己一手撑起的天地，心头第一次有了底气——往后再有尸潮或匪帮撞来，这堵墙，扛得住。' },
        { text: '只小修小补，不大兴土木', effects: { supplies: -4, hp: -2 }, reaction: '你只给据点小修小补、加了几道门闩，没敢大动干戈——墙是没筑起来，可往后每逢风吹草动，你都得提心吊胆地睡。' },
      ],
      summary: '筑墙设防', art: '1hp42bj', gen: 'gemini',
      requires: 'has(据点)',
      keyMoment: true,
      minTurn: 16,
      weight: 1.1,
    },
    {
      narrative:
        '堡垒立稳之后，慕名前来投奔的幸存者越来越多——拖家带口的、身怀一技的、走投无路的，都聚到你的墙下，眼里是劫后余生的渴望。人多了是力量，也是麻烦：要分配口粮、要排班守夜、要调解纷争、更要立下一套人人遵从的规矩，否则乌合之众转眼就会内讧崩散。是敞开大门、把这堡垒经营成一座号令一方、自有秩序的营地，还是怕担风险、只守着自己这一小撮人闭门过活？',
      choices: [
        { text: '接纳幸存者，立规矩号令一方', effects: { supplies: 8, sanity: 6, hp: -4 }, flagsSet: ['营地'], reaction: '你立下规矩、排定分工，墙内渐渐有了集市、有了哨岗、有了孩子的笑声——一座有秩序的营地在废土上拔地而起，众人提起你的名字，眼里是信服。' },
        { text: '闭门自守，不愿担这摊子', effects: { supplies: -4, sanity: -2 }, reaction: '你婉拒了大多数投奔者、只守着自己这一小撮人——是省了管一大摊子的烦难，可墙外那些失望离去的背影，也带走了你本可号令一方的可能。' },
      ],
      summary: '聚拢幸存者', art: 'mvkohi', gen: 'gemini',
      requires: 'has(堡垒)',
      keyMoment: true,
      minTurn: 26,
      weight: 1.1,
    },
    {
      narrative:
        '在一处幸存者集市的角落，一个独眼的掮客拦住了你，他眯着眼盯着你怀里那块旧怀表看了许久，忽然压低声音：「这表……我认得。它原主人的兄弟，眼下就在城南的据点里，悬了赏要寻这块表和带它的人。」他的话半真半假，可你心头还是猛地一跳。这块同伴托付的信物，竟成了一条牵向某个未知据点的线——那头，可能是失散亲人的重逢与酬谢，也可能是一个精心编织、专钓痴心人的圈套。',
      choices: [
        { text: '凭信物寻到城南据点赴约', effects: { supplies: 14, sanity: 8, hp: -2 }, reaction: '城南据点的人验过怀表，神色骤变，当即奉上一大批给养致谢：「我哥临终前最惦记的就是它……你替他守住了，这份情我们认。」一块表，换来了一处可投靠的善缘。' },
        { text: '疑心是圈套，攥紧信物不去', effects: { supplies: -3, sanity: -4 }, reaction: '你掂量着掮客那半真半假的话，终究没敢去赴这趟约。信物还在你怀里，可那扇或许真能为你敞开的门，也被你的谨慎一并关上了。' },
      ],
      summary: '信物的来历', art: '1u3jbry', gen: 'gemini',
      requiresItem: '旧怀表',
      once: true,
      minTurn: 9,
    },
    {
      narrative:
        '一伙劫掠者把你逼到了死墙下，明晃晃的刀就架在面前，为首的搜着你的背包，眼睛却死死盯住了你脸上那只防毒面具。「就要这个。」他啐了一口——在毒雾越来越频繁的近来，一只完好的防毒面具，早已是比子弹还硬的通货。他咧嘴冷笑，意思再明白不过：要么乖乖交出面具滚蛋，要么连命一起留下。你的手不自觉地按住了脸上的滤罐，那是你从死人堆里抢来、护着你穿过毒气的命根子。',
      choices: [
        { text: '舍命不舍面具，拼死护住它', effects: { hp: -14, supplies: -4, sanity: 4 }, reaction: '你宁可挨刀也死死扣着面具不撒手，那股要命不要面具的疯劲，竟把劫掠者唬得退了半步——他想不通，一只破面具凭什么让人豁出命去护。' },
        { text: '交出防毒面具，保命要紧', effects: { hp: -2, supplies: -3, sanity: -8 }, reaction: '你颤抖着摘下面具递了过去，为首的掂着战利品满意地挥手放行。命是保住了，可往后再遇上毒雾，你就只剩一条湿毛巾和一身的运气了。' },
      ],
      summary: '面具劫掠', art: '1lxd409', gen: 'gemini',
      requiresItem: '防毒面具',
      itemsLost: ['防毒面具'],
      weight: 1.1,
    },
    {
      narrative:
        '连日的厮杀与饥寒终于把你掏空了，你瘫在一栋居民楼的楼道里，伤口发炎，高烧不退，神志一阵阵模糊。就在你以为要交代在这儿时，那块同伴托付的旧怀表从怀里滑了出来，冰凉的表壳贴上手心，那个被你救下、又把信物交给你的年轻人的脸忽然浮上心头——他说过，要替你盯着后背。这一点念想像根细线，把你从昏沉里硬生生拽住了一截。是攥着这点暖意咬牙撑下去，还是干脆松手，让自己沉下去？',
      choices: [
        { text: '攥紧怀表，靠这点念想撑住自己', effects: { hp: 10, sanity: 12, supplies: -4 }, reaction: '你把怀表死死贴在胸口，那滴答的错觉像有人在耳边催你别倒下。高烧退去时你长出一口气——撑着你熬过这一夜的，不是药，是有人惦记着你的那点暖。' },
        { text: '高烧中神志涣散，几近放弃', effects: { hp: -10, sanity: -6, supplies: -3 }, reaction: '你的手一点点松开，怀表「当啷」落地。半梦半醒间你听见有人在喊你的名字，恍惚里又攥回了它——这一回，是侥幸没沉下去，下次未必还有这运气。' },
      ],
      summary: '信物的暖意', art: '1u3j96g', gen: 'gemini',
      requiresItem: '旧怀表',
      requires: 'hp<=40',
      weight: 1.3,
    },
    {
      narrative:
        '你撞进一间废弃的诊所避雨，意外在里间发现了一只上锁的医药冷藏箱，撬开后竟是大半箱完好的抗生素和消炎针剂——在这缺医少药的世道，这无异于一座金山。可正当你欣喜若狂地往背包里塞时，门外传来了杂沓的脚步声和咳嗽声：一群染了病、走投无路的幸存者循着药味摸了过来，挤在门口，一双双布满血丝的眼睛里，是恳求，是渴望，也是被绝望逼到极致的危险。这箱药，独吞能保你很久平安，分出去却能换一群人的命与情。',
      choices: [
        { text: '匀出大半箱药，救济门外病患', effects: { supplies: -10, sanity: 16, hp: -2 }, reaction: '你把成把的药分发出去，病患们捧着救命药跪地痛哭，连声唤你恩人——肯在金山面前舍出大半的人，赢的不是物资，是一群肯为你拼命的伙伴。' },
        { text: '抱箱从后门溜走，独吞这批药', effects: { supplies: 8, sanity: -10 }, reaction: '你抱着冷藏箱从后门猫腰溜了，身后门口那群病患的咳嗽声渐渐变成了咒骂。药是你的了，可那一声声诅咒，会在往后的夜里反复爬上你的心头。' },
      ],
      summary: '诊所药箱', art: '1k8w95b', gen: 'gemini',
      once: true,
      minTurn: 11,
    },
    {
      narrative:
        '一连数日不见天日的搜寻，让你的精神绷到了断裂的边缘。此刻你蹲在一间黑屋里，眼前的景象开始诡异地扭曲：墙壁在呼吸，地板像水面般荡漾，那些早已死去的面孔从阴影里一张张浮出来，温柔地朝你伸手。你分不清哪句话是真，哪个影是幻。背包带子勒着肩膀的真实痛感，是你与现实之间最后一根没断的线。你死死盯着自己沾血的指甲，拼命想抓住一点确凿无疑的东西，可那些低语越来越响，几乎要把你整个人吞没进去。',
      choices: [
        { text: '反复默念自己的名字，把神志拽回来', effects: { sanity: 14, hp: -2, supplies: -3 }, reaction: '你一遍遍念着名字、掐着掌心，那些浮动的面孔终于一点点退回了阴影里。能在崩溃的悬崖边把自己生生拉回来的人，整片废土也找不出几个。' },
        { text: '伸手去够那些「故人」的手', effects: { hp: -10, sanity: -4, supplies: -4 }, reaction: '你伸出的手只攥住了一把冰冷的空气和墙皮的碎屑，幻象「轰」地碎开。回过神时你正站在窗台边缘——再往前半步，就再没有醒来的机会了。' },
      ],
      summary: '幻象深渊', art: '1djlu58', gen: 'gemini',
      requires: 'sanity<=30',
      weight: 1.5,
    },
    {
      narrative:
        '电台里突然插进一段断续而亢奋的广播：据说军方在邻市还留着最后一处撤离点，今夜子时是登机窗口，逾时不候。消息真假难辨，可那点「或许还有救」的指望，仍让你心脏狂跳。撤离点远在数条街区之外，沿途必是被惊动的感染者与同样拼命赶路的人。这一程，是把命押在一个未必为真的传言上——可万一是真的呢？',
      choices: [
        { text: '不顾一切，朝撤离点狂奔', effects: { hp: -10, supplies: -6, sanity: -6 }, reaction: '你拼了命地赶到，等来的却是空荡的停机坪和遍地狼藉——撤离队早已撤走，或者根本从未存在。你瘫坐在地，赌上半条命换来的，只有「再没有人会来救你」这一个砸实在心口的事实。' },
        { text: '将信将疑，远远探看再说', effects: { hp: -4, supplies: -5, sanity: -2 }, reaction: '你择暗巷谨慎靠近，远远望见那片空无一物、只有风卷照明弹残壳的广场——传言到底是假的。你没白送命，却也彻底断了「等人来救」的最后一点念想，转身回了自己的路。' },
      ],
      summary: '最后撤离窗口', art: 'wlvnpo', gen: 'gemini',
      keyMoment: true,
      once: true,
      minTurn: 27,
      weight: 2,
    },
    {
      narrative:
        '又一次缺粮的当口，你这便利店老店员的脑子忽然转了起来——总店有一处只有值夜班的人才晓得的暗格仓库，当初为了防盗藏得极深，外人翻烂了货架也找不着。三年过去，那批货十有八九还压在夹层里。这是一笔足以让你缓口气的横财，可若声张出去，难保不引来红了眼的同类。是摸黑独吞，还是匀出来分给同伴、聚一聚人心？',
      choices: [
        { text: '摸黑独吞这批隐藏库存', effects: { supplies: 10, sanity: -3 }, reaction: '你借着夜色搬空了只有你记得的暗格，背包鼓了起来——可这秘密压在心头，也让你愈发提防身边的每一双眼睛。' },
        { text: '拿出来分给同伴，聚拢人心', effects: { supplies: 2, sanity: 6 }, reaction: '你把那批别人找不到的库存匀给了同伴，换来一圈感激的眼神——废土上，肯分粮的人，才聚得起一群肯替你拼命的人。' },
      ],
      summary: '熟门熟路', art: '1gi1usa', gen: 'gemini',
      requires: 'has(店员)',
      minTurn: 4,
      weight: 0.9,
    },
    {
      narrative:
        '一个重伤的幸存者被同伴七手八脚抬到你面前——腹部一道深可见骨的撕裂伤，血已经浸透了半边衣裳，气息奄奄。旁人都束手无策，唯有你这退役军医看得明白：这伤还有救，但要赌上你珍藏的抗生素与缝合器械，还得让你那条旧伤的左腿在手术台前硬撑上几个钟头。救，要掏空本就紧张的药品家底；不救，眼睁睁看一条命在你专长的领域里流逝。',
      choices: [
        { text: '倾尽药品器械，救他一命', effects: { hp: -4, supplies: -8, sanity: 8 }, reaction: '你赌上珍贵的药品和自己的旧伤把人从鬼门关拽了回来——他攥着你的手发誓此生跟定你，这份医者仁心，在废土上重过千金。' },
        { text: '保住药品，只做简单包扎', effects: { supplies: -2, sanity: -4 }, reaction: '你只草草包扎便收起了药箱——药是省下了，可那人渐渐没了气息时望你的那一眼，让你往后许多夜都翻来覆去。' },
      ],
      summary: '军医本色', art: '1bbkhg9', gen: 'gemini',
      requires: 'has(军医)',
      minTurn: 4,
      weight: 0.9,
    },
    {
      narrative:
        '一处高处的断桥彼端，吊着半箱没人够得着的救命物资。围观的老幸存者都摇头说太险，可你这年轻人腿脚利索、又一身使不完的劲，眼睛一下就亮了——凭你的身手，攀过那道朽断的桥梁去够它，未必不能成。心口那股不服输的冲动嗡嗡作响，撺掇着你去露一手、搏一把；可理智又在角落里发虚地提醒你：这世道，逞强的少年，往往死得最快。',
      choices: [
        { text: '逞勇涉险，赌一把身手', effects: { hp: -10, supplies: 8, sanity: -2 }, reaction: '你仗着年轻腿快攀过朽桥险险够到了物资，落地时后怕得腿发软——这次赌赢了，可这股莽劲，早晚要让你吃个大亏。' },
        { text: '按住冲动，稳妥行事', effects: { supplies: -3, sanity: 6 }, reaction: '你死死按住了逞强的念头、稳稳退了开——一个年轻人能在废土上学会「忍」字，是比跑得快、跳得高更要命的本事。' },
      ],
      summary: '少年血气', art: '1cyw5ha', gen: 'gemini',
      requires: 'has(高中生)',
      minTurn: 3,
      maxTurn: 18,
      weight: 0.9,
    },
    {
      narrative: '废墟里一条瘦骨嶙峋的流浪狗一直跟在你身后，浑身脏污、肋骨根根分明，眼神里却没有末世该有的凶戾，只剩一片怯生生的依赖。你停下，它也停下，远远地冲你摇了摇尾巴。在这连人都自顾不暇、一口粮就是一条命的世道，多养一张嘴是天大的奢侈；可那点活生生的、贴上来的暖意，又让你一时狠不下心转身就走。',
      choices: [
        { text: '收下阿黄，相依为命', effects: { sanity: 6, supplies: -2 }, flagsSet: ['阿黄'], reaction: '你拍了拍腿，那狗便颠颠地扑进你怀里。你给它取名阿黄——从此这条忠心的狗成了末世里少有的慰藉，夜里有它支着耳朵替你守着，心也安稳几分，只是那口粮，又得从牙缝里匀出一份。' },
        { text: '狠心赶走，自顾不暇', effects: { supplies: 2 }, reaction: '你硬起心肠捡起块碎砖朝它扬了扬，它呜咽着退开，却还在原地不肯走。省下的那一份口粮压在背包里沉甸甸的，你低头快步离开，到底没敢回头再看它一眼。' },
      ],
      summary: '收留阿黄', art: '1eguxkn', gen: 'gemini', minTurn: 4, weight: 1,
    },
    {
      narrative: '阿黄如今总寸步不离地跟着你，鼻子灵、耳朵尖，废墟里有人摸近，它老远就压低喉咙警觉地呜呜起来；翻找物资时，它还能凭着嗅觉刨出埋在瓦砾下的罐头。你看着这通人性的家伙，心里盘算开了——若肯多匀些口粮、好好把它喂壮，再花些工夫调教，它未必不能成为你在末世里一个得力的帮手。',
      choices: [
        { text: '悉心喂养，倚为帮手', effects: { hp: 2, sanity: 4, supplies: -4 }, reaction: '你把省下的口粮一口口喂给阿黄，它一天天壮实起来，毛色也亮了。寻物觅食、守夜护院样样拿手，人和狗的命，就这么紧紧拴在了一处——只是粮食也跟着耗得更快了。' },
        { text: '有口饭吃，不必费心', effects: { supplies: 2 }, reaction: '你只把吃剩的残食留给阿黄，懒得再多费心思调教。省了力气，人和狗之间也就少了那份生死与共、心意相通的默契，它依旧跟着你，却终究只是条跟着混口饭的狗。' },
      ],
      summary: '喂养阿黄', art: '1bww6w5', gen: 'gemini', requires: 'has(阿黄)', minTurn: 12, weight: 1.1,
    },
    {
      narrative: '一伙劫匪循着气味摸进了你的藏身处，刀光在月色里闪着冷意。混乱中，阿黄毫不犹豫地扑了上去，死死咬住一个匪徒的手腕，替你挡下了致命的一击，自己却被狠狠踹翻在地，发出凄厉的哀鸣，再爬不起来。劫匪的刀已重新举起，对准了倒地的它。救它，你得拼着重伤再冲进去；不救，你眼下就能脱身。',
      choices: [
        { text: '拼死也救，不弃伙伴', effects: { sanity: 8, hp: -8 }, reaction: '你嘶吼着扑回去，拼着挨了一刀也要把阿黄抢出重围。这份不弃伙伴的执拗，让你在这吃人的末世里，到底守住了一点没被磨尽的人味——只是自己也伤得不轻，淌着血搂着它喘了半宿。' },
        { text: '放它自去，求个心安', effects: { supplies: 2, sanity: 4 }, reaction: '你含泪冲它喊「走——快走」，替它和自己都求个两全。阿黄挣扎着爬起，一瘸一拐地回头望了你最后一眼，便钻进了茫茫废墟。它是死是活你再不得而知，可那点相伴过的暖，却长长久久地留在了心底。' },
        { text: '舍狗保命，绝境无情', effects: { supplies: 2 }, flagsClear: ['阿黄'], reaction: '绝境之中，你终究背过身，踩着阿黄的哀鸣保全了自己。你活下来了，背包还在，命还在，可那点末世里仅剩的、靠一条狗焐热的暖意，也跟着那声渐弱的哀鸣，一并熄了。' },
      ],
      summary: '阿黄赴险', art: '1m49sat', gen: 'gemini', requires: 'has(阿黄)', minTurn: 22, weight: 1.2,
    },
    {
      narrative:
        '据点立稳了名头，麻烦也跟着上门。一支盘踞在邻近三个街区的武装幸存者团伙派人捎来口信：他们的头目想见你。会面定在两片势力交界的废广场，对方头目独眼缠着旧疤，开门见山——眼下废土上零散的据点撑不过下一个寒冬，要么并作一家、互通有无共御尸潮，要么就在春天来临前，看谁先把谁吞掉。他把一柄解下的猎刀横在桌上，算是诚意，也算是威胁。结盟能让你的据点一夜壮大，可把后背交给一个野心勃勃的枭雄，谁也说不准明天的刀会不会捅向你。',
      choices: [
        { text: '歃血结盟，两家据点连成一片', effects: { supplies: 16, sanity: 8, hp: -3 }, flagsSet: ['盟约'], reaction: '独眼头目割破手掌按在桌面，咧嘴一笑：「成。从今往后你我两家，一根绳上的蚂蚱。」两片势力连成一气，墙更高了，巡哨更密了，废土上头一回有人要正眼掂量你的分量。' },
        { text: '婉拒结盟，只谈一纸互不侵犯', effects: { supplies: 4, sanity: -3 }, reaction: '你压住了并家的诱惑，只与他划清了井水不犯河水的界。头目眯眼盯了你半晌，收刀起身：「精明……但精明人，在这世道也未必活得长。」话里的机锋，让你往后多排了两班守夜。' },
        { text: '设局反吞，趁会面拿下对方头目', effects: {}, outcomes: [
          { weight: 2, effects: { supplies: 24, hp: -14, sanity: -8 }, reaction: '你抢先发难，废广场上刀光骤起。一场血拼过后，你踩着对方头目的尸首吞下了他整片地盘——从此废土上再没人敢小觑你，可这条以诈取胜的路，也让投奔者望你的眼神里多了一丝忌惮。' },
          { weight: 1, effects: { hp: -20, supplies: -16, sanity: -10 }, reaction: '你的杀机被对方先一步识破，反被打了埋伏。你浴血杀出重围捡回半条命，却折了大半人手、丢了不少家底——枭雄之间的算计，赌输了便是伤筋动骨。' },
        ] },
      ],
      summary: '据点会盟', art: '1e5yyjp', gen: 'gemini', minTurn: 36, requires: 'has(据点)', weight: 1.2,
    },
    {
      narrative:
        '熬到这一步，墙内聚起的人多了，新的祸患却从里头长了出来。昨夜库房少了三箱口粮，今晨又有人举报：几个老资格的住户私藏物资、克扣新来者的配给，甚至动手打伤了分粮时多嘴的少年。墙外是感染者，墙内若也烂了，这堡垒迟早从根上塌掉。众人吵作一团，目光齐刷刷落到你这立墙之人身上——是当众立下铁律、以儆效尤，还是设个由众人推选的议事桌，让规矩从此不只系于你一人之口？',
      choices: [
        { text: '当众立威，铁腕整肃秩序', effects: { sanity: 6, hp: -2, supplies: 4 }, reaction: '你把私藏的物资尽数追回、当众罚了带头闹事的人，立下白纸黑字的铁律。墙内一时鸦雀无声，秩序是稳住了，只是有人服气，也有人在背地里嘀咕你越来越像那些「营主」。' },
        { text: '设议事桌，让规矩出自众人之手', effects: { sanity: 10, supplies: -4, hp: -2 }, flagsSet: ['秩序'], reaction: '你没把刀攥在自己手里，而是请各家推举代表，围坐定下了分粮、守夜、断案的章程。头一回，墙内的人不再只是仰你鼻息的难民，而成了肯为这片家园说话、出力的伙伴——一套不靠你一人撑着的秩序，悄悄生了根。' },
        { text: '查清主谋，私下铲除隐患', effects: { sanity: -6, hp: -4, supplies: 6 }, reaction: '你不动声色地查清了带头作乱的人，趁夜把麻烦悄悄抹平了。隐患是除了，物资也追了回来，可那桩没人说破的事压在心头，往后你看谁的眼神里，都难免多了三分提防。' },
      ],
      summary: '墙内秩序', art: '1cdo7ob', gen: 'gemini', minTurn: 37, requires: 'has(堡垒)', weight: 1.1,
    },
    {
      narrative:
        '一场尸潮过后，几十名失了据点的幸存者拖家带口涌到你墙下——老的老、小的小，伤的伤、病的病，跪在墙根一声声哀求收留。墙头的人都看着你：开门收人，你这点家底要被几十张嘴生生分薄，万一里头混进了细作或染了病的，更是引狼入室；可若闭门不纳，眼睁睁看着这群活人在墙外被夜里的尸群吞掉，那一夜的惨叫，你这辈子怕是都忘不掉。曙光还远，人心却已在天平上摇晃。',
      choices: [
        { text: '开门收容，分薄家底也要救人', effects: { supplies: -14, sanity: 14, hp: -2 }, reaction: '你下令开门，把饿殍般的人群一个个接进墙内、匀出口粮、腾出铺位。粮仓肉眼可见地瘪了下去，可墙内多了几十条肯为这片家园拼命的人，多了孩子重新响起的笑——这笔账，用粮食算是亏的，用人心算，是赚的。' },
        { text: '只收老弱孩童，余者发些干粮遣走', effects: { supplies: -6, sanity: 4 }, reaction: '你留下了跑不动、护不住自己的老人和孩子，又给余下的人塞了几日干粮指条活路。算不上铁石心肠，也谈不上倾囊相助——在这要命的世道，这点掐着分寸的善意，已是许多人给不出的了。' },
        { text: '闭门拒纳，守住自己这一摊', effects: { supplies: 2, sanity: -12, hp: -2 }, reaction: '你硬起心肠下令封门，任墙外的哭求一声声撞在铁门上。那一夜，墙外的动静从哀求变成了惨叫，又渐渐归于死寂。你守住了家底，也守住了人，可墙头那些不忍再看的眼神，和你自己堵在胸口的那块石头，久久没能散去。' },
      ],
      summary: '墙外难民潮', art: 'z515dd', gen: 'gemini', minTurn: 38, weight: 1.2,
    },
    {
      narrative:
        '你以为早已退出你势力范围的旧敌，又回来了。当年被你打退、折了威风的那个匪帮头目，这些时日纠集了散落各处的亡命徒，组成一支几十人的队伍，连夜逼到了你据点外的高地上，扎下营盘，篝火连成一片。他要的不只是物资，是当众踩碎你这块在废土上越来越响的招牌，找回那口憋了许久的恶气。哨兵慌张来报时，你站上墙头远眺那一片晃动的火光——这一仗躲不过去了，是死守、是先发制人，还是另寻第三条路。',
      choices: [
        { text: '据墙死守，把围攻者耗在墙下', effects: { hp: -12, supplies: -12, sanity: 6 }, reaction: '你下令熄灯、上墙、备好滚石檑木，把一波波扑墙的亡命徒尽数砸了回去。鏖战三日，对方丢下满地伤亡悻悻退走——你用一道墙和满身伤，把这块越打越硬的招牌，又往废土上钉深了一寸。' },
        { text: '夜袭敌营，先发制人斩首', effects: {}, outcomes: [
          { weight: 2, effects: { hp: -10, supplies: 10, sanity: -4 }, reaction: '你挑了死士摸黑摸进敌营，一刀结果了正在篝火旁醉酒的头目。群龙无首的乌合之众天没亮就作鸟兽散，还丢下了搬不完的辎重——这一手快、狠、准的斩首，叫整片废土都记住了你的名字。' },
          { weight: 1, effects: { hp: -22, supplies: -10, sanity: -10 }, reaction: '夜袭却撞进了对方的圈套，营盘里早有埋伏。你拼死杀出重围，赔进去几个并肩多年的弟兄，自己也险些没能爬回墙内——逞先发制人之勇，赌输了便是血淋淋的代价。' },
        ] },
        { text: '联络他方势力，里应外合夹击', effects: { hp: -8, supplies: -6, sanity: 8 }, reaction: '你连夜知会了相熟的另一片势力，约定里应外合。当亡命徒扑上你的墙头时，援军的人马正好从他们背后杀出——前后夹击之下，旧敌溃不成军。这一仗打完，两家从此结下了用血换来的、实打实的过命交情。' },
      ],
      summary: '旧敌反扑', art: '1ehmzva', gen: 'gemini', minTurn: 39, requires: 'has(堡垒)', weight: 1.2,
    },
    {
      narrative:
        '据点终于经营成了一座有模有样的营地，可入夜后墙内仍是一片漆黑——人心里那点对光的渴望，比对粮食的还要深。这日，一个曾在供电局干过的住户找上你，说城郊那座小型变电站或许还能抢修，只要凑齐线缆、零件，再冒险跑一趟，说不定能让整座营地重新亮起灯。这是一桩耗费物资、还得派人深入险地的大工程，可一旦灯亮起来，那将不只是照明——那是文明的火种在废土上重新被点燃的信号，是给所有人「日子能越过越好」的指望。',
      choices: [
        { text: '倾力抢修变电站，让营地重见光明', effects: { supplies: -10, sanity: 14, hp: -4 }, flagsSet: ['灯火'], reaction: '忙了整整数日，当那串灯泡终于「啪」地次第亮起、把营地的夜照得暖黄一片时，墙内爆发出一阵难以置信的欢呼，有人当场红了眼。孩子们追着灯光跑，老人们怔怔望着久违的电灯掉泪——这一夜，废土上重新有了一座不肯熄灭的灯塔，文明的火，又烧了起来。' },
        { text: '先攒够余粮再说，眼下不冒这险', effects: { supplies: 4, sanity: -4 }, reaction: '你掂量着抢修要搭进去的物资和人手，终究把这桩心愿先压了下来，把粮仓填得更实些。日子是稳当了，可入夜后那片浓得化不开的黑，和众人眼里悄悄黯下去的那点指望，也实实在在压在了你心上。' },
      ],
      summary: '点亮营地', art: '1g1e5dt', gen: 'gemini', minTurn: 40, requires: 'has(营地)', weight: 1.1,
    },
    {
      narrative:
        '一支自称来自外省幸存者联盟的小队，循着废土上的口碑找到了你的据点。带队的女人风尘仆仆，却难掩眼底的振奋——她说东边数百里外，几座大型安全区已经连成了一片稳固的辖区，电力、医疗、农耕都在恢复，更要紧的是，对病毒的疫苗已经过了试制、开始小批量接种。她此行是来联络废土上仍在坚守的据点，邀你们派人随她回去对接，把这片孤悬的土地，也接进那张正在重新铺开的文明之网。这消息太大、太好，好得几乎让人不敢信。',
      choices: [
        { text: '派人随队东去，对接联盟', effects: { supplies: -6, sanity: 16 }, flagsSet: ['曙光'], reaction: '你挑了几个机灵可靠的人，备足干粮随她上路对接。送他们出门那天，你站在墙头望着东方——三年了，第一次，「熬出头」这三个字不再是哄自己的空话，而成了一个有方向、有期限、看得见的盼头。整座据点的人都觉得，那片天，好像真的要亮了。' },
        { text: '谨慎查证，先不轻信这泼天好消息', effects: { sanity: 4, supplies: -3 }, reaction: '你按捺住翻涌的激动，没急着应承，只多方盘问、暗中查证她的来路与说辞。这年头，越是天大的好消息越要防着是套——这份在喜讯面前都不肯失了分寸的冷静，正是你熬过三年的本钱。查证之下，那联盟竟大半是真，只是路途比她说的更凶险些。' },
      ],
      summary: '联盟来使', art: '1ile83g', gen: 'gemini', minTurn: 41, weight: 1.2,
    },
    {
      narrative:
        '东去对接的人马终于回来了，还带回一支挂着联盟臂章的接应队——疫苗是真的，重建的辖区也是真的。可天大的好事偏偏卡在最后一道坎上：联盟划定的安置点能容下的人手有限，迁过去意味着舍下你一砖一瓦垒起来的这座营地，把所有人连根拔起，长途跋涉穿过尚未肃清的废土；留下不走，则要永远做这文明之网外、自生自灭的孤岛。墙内为「走还是留」吵翻了天，几十双眼睛、几十条性命的去向，最终又压回了你这个主心骨肩上。',
      choices: [
        { text: '率众迁往安置点，融入重建的新世界', effects: { supplies: -10, sanity: 16, hp: -6 }, reaction: '你拍板带众人启程。临行前你最后一次站在亲手砌的墙头，把这座熬过尸潮与寒冬的营地，郑重地交给了风。一路虽折损了些气力与给养，可当安置点的灯火与穿白大褂的医护撞进眼帘时，所有的疲惫都化作了泪水——你带着托付给你的人，一个不少地，走进了那个重新有光的世界。' },
        { text: '留守这片亲手挣下的疆土，做废土上的灯塔', effects: { supplies: 8, sanity: 10, hp: -2 }, reaction: '你谢绝了内迁，选择留守。你与联盟立约，让这座营地成为他们伸向废土深处的一个补给点、一座驿站。墙没拆，灯还亮，往来的人越来越多——你没去投奔那个新世界，而是把新世界的边界，亲手向你脚下这片土地延伸了过来。' },
        { text: '送走老弱妇孺，自领死士殿后清路', effects: { supplies: -6, sanity: 12, hp: -10 }, reaction: '你把跑不动、护不住自己的人尽数托付给接应队先行，自己却带着一队敢拼命的人留下断后、替大队清扫沿途的尸群与匪患。送别时有孩子哭着不肯松手，你笑着把他推上车——能在曙光将至时仍把生路先让给旁人的，整片废土上没有几个。' },
      ],
      summary: '迁徙抉择', art: '1ku04cx', gen: 'gemini', minTurn: 43, requires: 'has(营地)', once: true, weight: 1.6,
    },
    {
      narrative:
        '三年将尽，废土上的风里第一次没了那股化不开的死气。你登上据点最高处守着黎明，远处接二连三亮起的灯火、隐隐传来的人声与犬吠，都在告诉你这片死城正一寸寸活过来。怀里那块同伴托付的旧物贴着胸口，那些没能走到这一天的人的脸，一张张从记忆深处浮上来。你忽然想为他们、也为这座据点，留下一点东西——一座碑，一行字，一段让后来人记得「有人曾在这死城里认真活过」的念想。可立碑费时费力，而眼下，还有太多活人的生计等着你操持。',
      choices: [
        { text: '立一座碑，刻下逝者的名字', effects: { sanity: 16, supplies: -6 }, reaction: '你领着众人在据点中央垒起一座石碑，把那些没能熬到曙光的名字一个个刻了上去。落成那日，墙内的人默默围拢过来，有人献上一束从天台菜园里掐的野花。这座碑不产一粒粮、不挡一只感染者，却让活着的人头一回觉得，他们守的不只是一条命，而是一个值得被记住的家。' },
        { text: '把心思先用在活人的春耕上', effects: { supplies: 10, sanity: 6, hp: -2 }, reaction: '你把缅怀压在心底，转身扎进了开春的营生里——翻地、育苗、修渠，张罗着让这据点在新的一年能多打几担粮。逝者长已矣，活人的日子还得往前奔。看着墙内重新有了春耕的忙碌与盼头，你知道，这才是对那些故去之人最实在的告慰。' },
      ],
      summary: '废土立碑', art: '1dblpwf', gen: 'gemini', minTurn: 42, requires: 'has(据点)', weight: 1,
    },
  ],
}
