...
> 社交分享入口页（`public/s/`）已预生成并随仓库提交，`npm run build` 会一并打包；改动内置剧本标题/开局后，重跑 `npx vite-node scripts/gen-og-pages.mjs` 刷新 OG 预览页。
...
> 另有 `scripts/gen-og-pages.mjs` 预生成社交分享入口页 → `public/s/`，题材 OG 封面置于 `public/og/`。
