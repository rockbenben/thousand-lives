...
**10 个剧本 · 28 家 AI 服务商预设 · 31 枚成就 · 纯前端 · 无需 Key 即玩**
...
内置 **10 个剧本**，覆盖末世、官场、谍战、仙侠、武侠、科幻等题材，每个均自带本地事件池，**无需 API Key 即可试玩**：

| 剧本 | 题材 | 简介 |
|------|------|------|
| ☢️ 末世求生 | 末世 | 病毒爆发后的现代都市，撑过三十天等待军方救援 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | string | 唯一标识符，不能与内置剧本的 id 相同（`wasteland` / `book` / `officialdom` / `spy` / `xian` / `wuxia` / `scifi` / `voyage` / `liyuan` / `sanguo`） |
| `title` | string | 剧本名称 |
| `emoji` | string | 剧本图标 |
| `intro` | string | 开场介绍文字 |
...
- **成就系统**：31 枚成就，覆盖局数里程碑、结局收集、S 评级、各剧本通关与传说结局、无伤通关、剧本创作等维度
